...
class FusedMagics(Magics):
    ...
    def _generate_ai_udf(self, line: str, *, operation: str) -> None:
        udf_text = line.strip()

        output_text = ""
        fused_udf_line = f"# {udf_text}\n"
        output_text += fused_udf_line

        # Print code
        api = get_api()
        udf_code = api._generate_udf(udf_text, operation=operation)
        output_text += udf_code

        self.fused_shell.set_next_input(output_text, replace=True)

    @line_magic
    def ai_udf(self, line: str) -> None:
        self._generate_ai_udf(line, operation="map")

    @line_magic
    def ai_join_udf(self, line: str) -> None:
        self._generate_ai_udf(line, operation="join")
