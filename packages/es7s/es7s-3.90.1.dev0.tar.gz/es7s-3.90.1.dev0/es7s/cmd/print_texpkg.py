...
class action(_BaseAction):
    ...
    def _print_summary(self):
        ...
        # fmt: off
        lines = (
            ("\x1b[1m",     f"Files with .sty extension:", self._files),
            ("\x1b[1;90m",  "  ├─>" + f" Classified as non-package:", len(self._files) - len(self._pkgs)),
            ("\x1b[1m",     "  └─>" + f" Packages detected:", self._pkgs),
            ("\x1b[1;34m",  "      ├─>" + f" Versions recognised:", [*filter(lambda p: p.ver, self._pkgs)]),
            ("\x1b[1;90m",  "      ├─>" + f" Marked as unversioned:", [*filter(lambda p: not p.ver and p.match, self._pkgs)]),
            ("\x1b[1;33m",  "      └─>" + f" Chocked on:", [*filter(lambda p: p.nonmatch, self._pkgs)]),
        )
        # fmt: on
        for line in lines:
            fmt, desc, val = line
