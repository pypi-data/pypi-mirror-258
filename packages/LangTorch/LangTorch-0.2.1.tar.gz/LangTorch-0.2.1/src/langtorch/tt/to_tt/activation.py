...
class OpenAI(torch.nn.Module):
    def __init__(self,
        ...
        if self.tool_jsons is not None:
            self.tool_jsons = [json.dumps(tool, ensure_ascii=False) if isinstance(tool, dict) else langtorch.api.tools.generate_schema_from_function(tool) if callable(tool) else str(tool) for tool in self.tool_jsons]
    ...
    def forward(self, input: TextTensor):
        ...
        result = chat(input, system_messages, model=self.model, cache=self.cache, as_str=True, tools = self.tool_jsons, **self.kwargs)
