...
class Text(str):
    ...
    @classmethod
    def parse(cls, s):
        parsed_result = cls.Parser.parseString(s)
        parsed_output = [(res.key if "key" in res else "", res.value if "value" in res else "") if isinstance(res,
                                                                                                              ParseResults) else res
                         for res in parsed_result]
        return cls(*parsed_output)
    ...
    def constructors(cls, *args, parse = True):
        """Reformats a wide array of construtor patterns into a unified form"""
            ...
            if parse:
                try:
                    ...
                    for arg in args:
                        ...
                        arg = [(res.key if "key" in res else "", res.value if "value" in res else "") if isinstance(res,   ParseResults) else res
                                for res in parsed_result]
                        result += arg
                    args = result
        ...
        if any([isinstance(arg, torch.Tensor) for arg in args]):
            raise ValueError("You cannot initialise Text from a Tensor subclass. Use tensor.item() or change the choice of classes.")
    ...
    def set_key(self, keys):
        """
        Modify or set new keys for the textual entries, enabling renaming or restructuring
        of the content. Useful for substituting the key right before passing a Text.

        Args:
            keys (Union[Text, str, List[str]]): The new keys to apply.
        ...
        if isinstance(keys, Text):
            if len(keys.values()) == len(self.values()):
                return self.__class__(*[(k,v) for k,v in zip(keys.values(), self.values())], parse = False)
            else:
                return self.__class__((Text._str_formatter(keys), Text._str_formatter(self)), parse = False)
        elif isinstance(keys, str):
            return self.__class__((keys, Text._str_formatter(self)), parse = False)
        else:
            return self.__class__(*[(k,v) for k,v in zip(keys, self.values())], parse = False)
    ...
    def __mul__(self, other):
        ...
        for j, (k_, v_) in enumerate(other.items()):
            if k_ == "" and j not in formatted_indices: # Positional arguments
                for i, (k, v) in enumerate(content):
    ...
    def format(self, *args, **kwargs):
        """A temporarily dodgy implementation"""
        def remove_shared_end(list1, list2):
            # Reverse the lists to compare from the end
            ...
            for a, b in zip(list1_reversed, list2_reversed):
                if a == b:
                    shared_length += 1
                else:
                    break

            # Return the list without the shared end entries
            return list1[:-shared_length] if shared_length > 0 else list1

        other = self.__class__(*args, parse = False) + Text(kwargs, parse = False)
