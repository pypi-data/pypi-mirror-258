...
class COGRaster(COGReader):
    ...
    def read_data_under_aoi(self, gdf:GeoDataFrame):
        """
        geojson in wgs84
        """
        ...
        # Use convex_hull directly
        gdf = GeoDataFrame(geometry=gdf.convex_hull.simplify(tolerance=0.001), crs='EPSG:4326')

        # Check validity of the geometry
        print("Is geometry valid?", gdf.geometry.is_valid)

        # Inspect envelope geometry
        print("Envelope Geometry:", gdf.geometry.envelope)

        # Use the feature method with the GeoDataFrame

        data, mask = self.feature(shape=gdf.__geo_interface__)
        print("Feature read successfully!")
        return data, mask
