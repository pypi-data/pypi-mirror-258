...
def get_func_modules() -> dict[str, FuncModuleInfo]:
  ...
  if not func_util_module_names:
    print(f'\tWarning: No functional modules found! Add `general_utils` at least')
    module_info = FuncModuleInfo.from_module(general_utils)
    loaded_module_info_map[general_utils.BINDING_KEYWORD] = module_info
  else:
