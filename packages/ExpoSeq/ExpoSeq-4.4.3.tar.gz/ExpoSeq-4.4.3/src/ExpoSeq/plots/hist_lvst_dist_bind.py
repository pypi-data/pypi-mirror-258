...
class PrepareData:
    ...
    def check_input(sample, batch_size, max_cluster_dist, antigens):
        assert type(sample) == str, "You have to give a string as input for the sample" # only one sample
        assert type(
            max_cluster_dist) == int, "You have to give an integer as input for the maximum levenshtein distance"
        assert max_cluster_dist != 0, "Plot does not make sense if you do not investigate at least SNPs"
    ...
    @staticmethod
    def tidy_data(sequencing_report, sample, batch_size, region_of_interest, antigens, binding_data):
    ...
    def label_bind_seqs(mix, region_string, aa_clustered, antigens):
        values = mix.loc[mix[region_string].isin(aa_clustered), antigens] # get the binding values for the sequences which were clustered
        #values = pd.DataFrame(values, columns = ["binding"])
        filtered_df = values[antigens][values[antigens] > 1].dropna(how='all') # filter out the binidng values which are 0
        indices = filtered_df.index.tolist()
        assert len(indices) !=0, "The cluster does not contain binding values. Please increase the lev. distance or the batch size."
        key_sequences = []
        key_values = []
        ...
        seq_val.columns = ["binding_seqs", "binding_values"] # seq_val object contains only the sequences from sanger - this is important for labelling
    ...
    def process(self, sequencing_report, sample, batch_size, region_of_interest, antigens, binding_data, max_cluster_dist, sample_column_name = "Experiment"):
        assert sample in list(sequencing_report[sample_column_name].unique()), "The provided sample name does not exist"
        self.check_input(sample, batch_size, max_cluster_dist, antigens)
        mix, aa_all = self.tidy_data(sequencing_report, sample, batch_size, region_of_interest, antigens, binding_data)
        ...
        condensed_matrix = squareform(levenshtein_distance_matrix, checks=False) # linearize, now only 1 dim array
        linked = linkage(condensed_matrix, 'single')
        aa_clustered, binding_seqs, seq_val = self.label_bind_seqs(mix,region_of_interest, aa_clustered, antigens)
        return linked, aa_clustered, binding_seqs, seq_val

        
class DendroBind:
    def __init__(self, sequencing_report, sample,region_of_interest, antigens,
                 batch_size, max_cluster_dist, binding_data,ascending = True, fig = None, font_settings = {},) -> None:
        linked, aa_clustered, binding_seqs, seq_val = PrepareData().process(sequencing_report, 
                                                                            sample,
                                                                            batch_size, 
                                                                            region_of_interest,
                                                                            antigens,
    ...
    def set_dendrogram_labels(self, font_settings, sample,):
    ...
    def create_bar_plot(self, binding_seqs, seq_val, ascending):
        binding_seqs_sorted, binding_values_sorted = self.sort_binding_seqs(binding_seqs, seq_val, ascending)
