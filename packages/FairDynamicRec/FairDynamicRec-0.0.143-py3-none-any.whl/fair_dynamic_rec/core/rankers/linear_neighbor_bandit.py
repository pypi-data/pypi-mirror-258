...
class NeighborUCB(AbstractRanker):
    ...
    def load_UV(self, config):
        if os.path.isfile(str(config.get_data_path()) + '/item_mapping') and os.path.isfile(config._target + '/U') and os.path.isfile(config._target + '/V'):
            item_mapping = pd.read_csv(str(config.get_data_path()) + '/item_mapping', sep='\t')
            U = pd.read_csv(config._target + '/U', names=['itemid', 'emb'], sep='\t')
            V = pd.read_csv(config._target + '/V', names=['itemid', 'emb'], sep='\t')
            for index, row in item_mapping.iterrows():
                oldID, newID = row['oldID'], row['newID']
                self.U[newID] = list(map(float, U[U['itemid'] == oldID]['emb'].values[0].split(',')[:-1]))
                self.V[newID] = list(map(float, V[V['itemid'] == oldID]['emb'].values[0].split(',')[:-1]))
    ...
    def get_ranking(self, batch_users, sampled_items=None, round=None):
        ...
        for i in range(len(batch_users)):
            ...
            self.writeCB((round*len(batch_users))+i, user, cbV, cbU)
