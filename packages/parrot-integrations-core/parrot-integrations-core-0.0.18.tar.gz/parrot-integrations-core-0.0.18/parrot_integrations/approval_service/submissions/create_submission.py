...
def get_schema():
    return generate_create_schema(object_type='submission', object_schema=OBJECT_SCHEMA)


def process(inputs, integration, token, account_uuid, **kwargs):
    return create_object(integration=integration, object_type='submission', data=inputs, token=token,
