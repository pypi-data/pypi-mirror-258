...
def get_schema():
    return generate_get_schema(object_type='account', object_schema=OBJECT_SCHEMA)


def process(inputs, integration, token, account_uuid, **kwargs):
    return get_object(integration=integration, object_type='account', object_uuid=inputs['account_uuid'], token=token,
