...
def get_or_create_test_cluster(
    ...
    if cluster_exist_with_name_and_runtime(db_client, cluster_name, runtime):
        print(f"Cluster {cluster_name} with runtime {runtime} exists")
        ...
        if status == DatabricksClusterStates.TERMINATED:
            print("Cluster is terminated, restarting it")
        ...
        if status == DatabricksClusterStates.RUNNING:
            print(f"Cluster {cluster_name} with runtime {runtime} is running")
            return cluster_id
        else:
            print(
        ...
        print(
            f"Could not find {cluster_name} with runtime {runtime} creating a new one"
        )
