...
def create_dict(pathlike, rootname=None, device=None, os_type=None, session=None, objtype='auto', nickname=None, do_hash=True, include_uuid=False):
    try:
        ...
        return False
    objtype = (('dir' if pathlike.is_dir() else 'file') if objtype=='auto' else objtype)
    ...
    if objtype in ['rootdir', 'dir']:
        contents_list = [pathlike/entry for entry in os.listdir(pathlike)]
        out['dir_list'] = [{'name' : entry.name, 'type' : 'unrecorded'} for entry in contents_list if Path(entry).is_dir()]
        out['file_list'] = [{'name' : entry.name, 'type' : 'unrecorded'} for entry in contents_list if not Path(entry).is_dir()]
...
def print_basic_info(dictin):
    ...
    def print_thing(label, key, default='N/A'):
        dictin_get = dictin.get(key, None)
        print(label + ': ' + (str(dictin_get) if dictin_get!=None else default))
