...
from dicomselect.logger import Logger
...
class Database(Logger):
        ...
        try:
            ...
            try:
                with ThreadPoolExecutor(max_workers=max_workers) as executor, \
                    ...
                    for root, dirs, filenames in os.walk(data_dir, onerror=lambda err: self.log(traceback.format_exception(err))):
                        root = Path(root)
            ...
            self.print(f"Database {db_action['n']}d at {self.path} in {hours}:{minutes:02d}:{seconds:02d} "
    ...
    def _scan_for_readers(self, root: Path, *filenames: str, **kwargs) -> Iterator[DICOMImageReader]:
        ...
        if len(files_dcm) > 0 and len(files_zip) <= 1:
            if len(files_zip) == 1 and self.prefer_mode & PreferMode.PreferZipFile:
    ...
    def _create_or_assert_db(self, db_path: Path, max_init: int, *readers: DICOMImageReader):
        """
        Obtain columns given any (uninitialized) DICOMImageReader
        """
        def create_columns(reader: DICOMImageReader):
            ...
            return set(sorted(dicom_columns + custom_columns))

        db_path_exists = db_path.exists()
        column_sets = []
        create_column_error: Exception | None = None
        for reader in tqdm(readers[:max_init], leave=None, desc='Verifying' if db_path_exists else 'Initializing'):
            try:
                column_sets.append(create_columns(reader))
            except Exception as e:
        ...
        columns = {column for column_set in column_sets for column in column_set}
        ...
        with sqlite3.connect(db_path) as cursor:
            if db_path_exists:
                ...
                missing_columns = columns.difference({f'{c[1]} {c[2]}' for c in columns_existing[2:]})
