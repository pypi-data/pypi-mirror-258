...
from dexteritysdk.dex.sdk_context import SDKContext, SDKTrader, SDKEventType, SDKPosition, SDKOrder, SDKFill
...
async def trade_loop(
    ...
    while True:
        ...
        price = price_map["Crypto.SOL/USD"][0]
...
async def main():
    ...
    await subscribe_to_prices(price_map, ["Crypto.SOL/USD"], loop=False)

    def positions_callback(e: List[SDKPosition], t: SDKEventType):
        print(f"positions_callback: {e} {t}")

    def orders_callback(e: List[SDKOrder], t: SDKEventType):
        print(f"orders_callback: {e} {t}")

    def fills_callback(f: SDKFill, t: SDKEventType):
        print(f"fills_callback: {f} {t}")

    await asyncio.gather(
        subscribe_to_prices(price_map, ["Crypto.SOL/USD"]),
        trader._monitor_accounts(
            sdk,
