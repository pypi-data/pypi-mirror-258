...
from databricks_genai.types.common import ObjectList, ObjectType
from databricks_genai.version import __version__
...
class MAPIConnection:
    ...
    def _load_from_environment(self, api_key: Optional[str] = None, endpoint: Optional[str] = None) -> None:
        ...
        conf: MCLIConfig = config.MCLIConfig.load_config()
        if api_key is None:
        ...
        if endpoint is None:
            if conf.endpoint is None:
                raise MCLIConfigError('No endpoint specified')
            final_endpoint = conf.endpoint
        self.endpoint = endpoint or final_endpoint
