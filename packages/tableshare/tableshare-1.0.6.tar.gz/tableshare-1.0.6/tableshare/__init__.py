        ...
        if not tables:  # 判断网站有没有任何表格
            ...
            for i, table in enumerate(tables):
                try:
                    ...
                    # 如果表头列表仍然为空，设置默认列名,len(table.find_all('tr')[1])是获取第2个tr标签里td的数量
                    if not headers:
                        headers = ['Column' + str(i) for i in range(1, len(table.find_all('tr')[0]) + 1)]
                    ...
                    for row_index, row in enumerate(table.find_all('tr'), start=1):  # 从1开始计数
                        ...
                        if cols:
                            ...
                            # 2.如果当前行的<td>数量小于表头列数，补齐空值（这部分代码已在之前的回答中提供）
                                ...
                                print(f"警告：在处理Table {i + 1}，第 {row_index} 行时，发现列数少于表头定义，已补齐空值处理。")
                            ...
                            # 3.如果当前行的<td>数量大于表头列数，忽略多余的<td>
                                ...
                                for i in range(len(headers)):
                                    ...
                                    if col.name == 'td':
                                        row_data.append(col.text.strip())
                                print(f"警告：在处理Table {i + 1}，第 {row_index} 行时，发现列数超出表头定义，已只保留与表头对应的列。")
                        ...
                        # 2.如果当前行的<td>数量小于表头列数，补齐空值（这部分代码已在之前的回答中提供）
                            ...
                            for col in cols:
                                if col.name == 'td':
                                    row_data.append(col.text.strip())
                            row_data += [''] * (len(headers) - len(row_data))
                            print(f"警告：在处理Table {table_index+1}，第 {row_index} 行时，发现列数少于表头定义，已补齐空值处理。")
