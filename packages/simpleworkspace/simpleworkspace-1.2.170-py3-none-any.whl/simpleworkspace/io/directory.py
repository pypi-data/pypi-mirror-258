...
def Scan(
    ...
    :param includeDirs: When true yields directories. Keep in mind even if this is set to false it will scan the directory tree but simply not yield its path
    :param includeFiles: When true yields files.
    :param followLinks: Specifies wheter to yield symlinks, when this is set to false, symlink directories wont even be scanned.
    :param filter: Callback or Glob string, when the filter condition returns true, the path is yielded
        * callback             : takes fullpath as arg and returns true for paths that should be yielded
        * glob string or list  : Matches a pattern against the full path, if any matches are found yields the path. Example: "*.js" or ["*.js", "*.txt"]
            ...
            with _os.scandir(path) as entries:
                for entry in entries:
                    ...
                    if filter is None:
                        ...
                        pathMatchesFilter = regex.Glob(filter, entryPath)
                    elif isinstance(filter, (list, tuple)):
                        pathMatchesFilter = any([regex.Glob(x, entryPath) for x in filter])
                    else: #callback
