...
def Scan(
    ...
    :param filter: Callback or Regex string, when the filter condition returns true, the path is yielded
        * callback      : takes fullpath as arg and returns true for paths that should be yielded
        * regex string  : Matches a pattern against the full path, if any matches are found yields the path. Example: "/mySearchCriteria/i"
            ...
            with _os.scandir(path) as entries:
                for entry in entries:
                    ...
                    if(filter is None):
                        ...
                        pathMatchesFilter = regex.Test(filter, entryPath)
                    else: #callback
