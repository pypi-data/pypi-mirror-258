...
class FBGcom:
    ...
    def read(self, Targets):
        ...
        if self._on_boradCalculation:
            now_data = [int.from_bytes(line[8 * i:8 * i + 4], byteorder='little', signed=True)/10000 for i+1 in Targets]
        else:
            now_data = [int.from_bytes(line[8 * i:8 * i + 4], byteorder='little', signed=True) for i+1 in Targets]
        return now_data
