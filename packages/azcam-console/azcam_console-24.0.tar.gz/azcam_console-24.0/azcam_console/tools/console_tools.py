...
class ServerConnection(object):
    ...
    def command(self, command):
        ...
        try:
            ...
            if e.error_code == 2:
                raise
                # raise exceptions.AzcamError("could not connect to server")
            else:
                raise
        ...
        if reply[0] == "ERROR":
            # raise exceptions.AzcamError(f"command error: {' '.join(reply[1:])}")
            raise exceptions.AzcamError(f"command error: {reply}")
        elif reply[0] == "OK":
            if len(reply) == 1:
                return None
            elif len(reply) == 2:
            ...
            raise exceptions.AzcamError(f"invalid server response: { ' '.join(reply)}")
