(function (nx) {
    ...
    topo.registerIcon("dead_node", "/static/nautobot_ui_plugin/img/dead_node.png", 49, 49);
    ...
    showHideRoles = function(role, isVisible) {
        let devicesByRole = []
        topologyData['nodes'].forEach(function(node){
            if (node['role'] == role) {
    ...
    topo.on('topologyGenerated', function(){
        ...
        undisplayedRoles.forEach(function(role){
            showHideRoles(role, false);
        });
