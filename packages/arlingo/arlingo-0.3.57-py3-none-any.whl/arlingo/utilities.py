...
def get_phrase_translation_output(phrases, given_translations=[], source='fr', target='fa', save_root_path='', github_root_path='', output_type='markdown', header = True):
    ...
    if source in ['de', 'fr']:        
        writer = PhraseTranslationWriter(phrases, translations, github_filepaths, part_of_speechs, genders, header)
    elif source == 'ko':
        writer = PhraseTranslationWriter(phrases, translations, github_filepaths, part_of_speechs, genders, header, no_gender=True, no_role=True)
    else:
        raise NotImplementedError('Only French, Korean, and German are supported at the moment.')
    if failure == False:
        clipboard.copy(writer.write(output_type))