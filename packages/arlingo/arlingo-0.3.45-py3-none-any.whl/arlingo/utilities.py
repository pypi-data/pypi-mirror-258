...
class Conjugator:
    ...
    def get_conjugation(self, verb, tense='present', lang='fr', **kwargs):
        ...
        if self.lang in ['fr', 'de']:
            ...
            if self.source == 'verbix':
                self.pronouns = []
                self.verbs = []
                parts = [item for item in self.page.findAll('table', {'class': 'verbtense'}) if 'nospeech' not in item.get('class')]
                for part in parts:
                    curr_pronouns = [item.text for item in part.findAll('span', {'class':'pronoun'})]
                    curr_verbs = [item.text for item in part.findAll('span') if item.get('class')==['orto'] or item.get('class')==['normal']]
                    self.pronouns.append(curr_pronouns)
                    self.verbs.append(curr_verbs)
            elif self.source == 'koreanverb':
                pass
        else:
