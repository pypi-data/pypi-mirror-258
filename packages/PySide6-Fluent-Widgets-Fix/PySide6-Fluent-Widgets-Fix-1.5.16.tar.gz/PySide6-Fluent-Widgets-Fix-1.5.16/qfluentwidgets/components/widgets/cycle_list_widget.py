...
class CycleListWidget(QListWidget):
    ...
    def __init__(self, items: Iterable, itemSize: QSize, align=QtEnum.Qt.AlignCenter, parent=None):
        """
        Parameters
        ----------
        items: Iterable[Any]
        ...
        self.vScrollBar = SmoothScrollBar(QtEnum.Qt.Vertical, self)
        ...
        # hide scroll bar
        self.setHorizontalScrollBarPolicy(QtEnum.Qt.ScrollBarAlwaysOff)
        self.setVerticalScrollBarPolicy(QtEnum.Qt.ScrollBarAlwaysOff)
        self.upButton.hide()
    ...
    def _addColumnItems(self, items, disabled=False):
        for i in items:
            ...
            item.setTextAlignment(self.align | QtEnum.Qt.AlignVCenter)
            if disabled:
    ...
    def setSelectedItem(self, text: str):
        ...
        items = self.findItems(str(text), QtEnum.Qt.MatchExactly)
        if not items:
            return
        ...
        super().scrollToItem(self.currentItem(), QtEnum.QListWidget.ScrollHint.PositionAtCenter)

    def scrollToItem(self, item: QListWidgetItem, hint=QtEnum.QListWidget.ScrollHint.PositionAtCenter):
        """ scroll to item """
    ...
    def eventFilter(self, obj, e: QEvent):
        if obj is not self or e.type() != QtEnum.QEvent.KeyPress:
            return super().eventFilter(obj, e)

        if e.key() == QtEnum.Qt.Key_Down:
            self.scrollDown()
            return True
        elif e.key() == QtEnum.Qt.Key_Up:
            self.scrollUp()
            return True
