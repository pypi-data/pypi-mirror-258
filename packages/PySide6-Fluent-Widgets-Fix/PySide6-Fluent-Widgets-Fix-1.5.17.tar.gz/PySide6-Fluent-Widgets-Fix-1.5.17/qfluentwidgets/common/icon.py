...
class FluentIconEngine(QIconEngine):
    ...
    def paint(self, painter, rect, mode, state):
        ...
        if mode == QtEnum.QIcon.Disabled:
            painter.setOpacity(0.5)
        elif mode == QtEnum.QIcon.Selected:
        ...
        icon.paint(painter, rect, QtEnum.Qt.AlignCenter, QtEnum.QIcon.Normal, state)
        painter.restore()
...
def drawIcon(icon, painter, rect, state=QIcon.Off, **attributes):
    ...
    **attribute:
    ...
    if isinstance(icon, FluentIconBase):
        ...
        icon = QIcon(icon)
        icon.paint(painter, QRectF(rect).toRect(), QtEnum.Qt.AlignCenter, state=state)
