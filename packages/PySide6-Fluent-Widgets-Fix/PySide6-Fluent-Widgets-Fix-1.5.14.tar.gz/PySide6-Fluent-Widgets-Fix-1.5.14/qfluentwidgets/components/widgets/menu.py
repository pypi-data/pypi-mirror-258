...
class MenuItemDelegate(QStyledItemDelegate):
    ...
    def _isSeparator(self, index: QModelIndex):
        return index.model().data(index, QtEnum.Qt.DecorationRole) == "seperator"
...
class ShortcutMenuItemDelegate(MenuItemDelegate):
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        # draw shortcut key
        action = index.data(QtEnum.Qt.UserRole)  # type: QAction
        if not isinstance(action, QAction) or action.shortcut().isEmpty():
            return

        painter.save()

        if not option.state & QtEnum.QStyle.State_Enabled:
        ...
        shortcut = action.shortcut().toString(QtEnum.QKeySequence.NativeText)
        ...
        painter.drawText(rect, QtEnum.Qt.AlignLeft | QtEnum.Qt.AlignVCenter, shortcut)
