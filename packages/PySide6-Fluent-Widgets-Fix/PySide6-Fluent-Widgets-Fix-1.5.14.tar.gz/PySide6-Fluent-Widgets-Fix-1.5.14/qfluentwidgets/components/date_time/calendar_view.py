...
class ScrollItemDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        painter.setRenderHints(QtEnum.QPainter.Antialiasing | QtEnum.QPainter.TextAntialiasing)
    ...
    def _drawBackground(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        if index == self.currentIndex:
            ...
            elif option.state & QtEnum.QStyle.State_MouseOver:
            ...
            if index == self.pressedIndex:
                ...
                painter.setBrush(QtEnum.Qt.transparent)
    ...
    def _drawText(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        if index == self.currentIndex:
            ...
            if not (self.min <= index.data(QtEnum.Qt.UserRole) <= self.max or option.state & QtEnum.QStyle.State_MouseOver) or \
                    index == self.pressedIndex:
                painter.setOpacity(0.6)

        text = index.data(QtEnum.Qt.DisplayRole)
        painter.drawText(option.rect, QtEnum.Qt.AlignCenter, text)
        painter.restore()
...
class ScrollViewBase(QListWidget):
    ...
    def wheelEvent(self, e):
        if self.vScrollBar.ani.state() == QtEnum.QPropertyAnimation.Running:
            return
    ...
    def mousePressEvent(self, e):
        ...
        if e.button() == QtEnum.Qt.LeftButton and self.indexAt(e.pos()).row() >= 0:
...
class YearScrollView(ScrollViewBase):
    ...
    def _initItems(self):
        ...
        for i, year in enumerate(years):
            item = self.item(i)
            item.setData(QtEnum.Qt.UserRole, QDate(year, 1, 1))
...
class MonthScrollView(ScrollViewBase):
    ...
    def _initItems(self):
        ...
        for i in range(12 * 201):
            ...
            m = i % 12 + 1
            item = self.item(i)
            item.setData(QtEnum.Qt.UserRole, QDate(year, m, 1))
...
class MonthCalendarView(CalendarViewBase):
    ...
    def currentPageDate(self) -> QDate:
        ...
        month = item.data(QtEnum.Qt.UserRole).month() if item else 1
...
class DayScrollView(ScrollViewBase):
    ...
    def _initItems(self):
        ...
        for i in range(bias):
            ...
            item.setFlags(QtEnum.Qt.NoItemFlags)
            self.addItem(item)
        ...
        for i in range(bias, self.count()):
            item = self.item(i)
            item.setData(QtEnum.Qt.UserRole, dates[i-bias])
            item.setSizeHint(self.gridSize())
...
class CalendarView(QWidget):
    ...
    def exec(self, pos: QPoint, ani=True):
        ...
        self.opacityAni.setEasingCurve(QtEnum.QEasingCurve.OutQuad)
        ...
        self.slideAni.setEasingCurve(QtEnum.QEasingCurve.OutQuad)
        self.aniGroup.start()
