...
class TextBlocks(GenericCRUD):
    ...
    def process_and_update_field(self, profile_id: int, fields_dict: dict, field_id: int, match: str) -> None:
        ...
        try:
            ...
            self.cursor.execute("SELECT `schema`, table_name, profile_mapping_table_id FROM `database`.table_definition_table WHERE table_definition_id = %s", (table_id,))
            schema, table_name, profile_mapping_table_id = self.cursor.fetchone()
            # Get field information
            self.cursor.execute("SELECT `schema`, table_name FROM `database`.table_definition_table WHERE table_definition_id = %s",
            ...
            if profile_id and profile_mapping_table_id:
                ...
                sql = "SELECT %s FROM %s.%s WHERE %s_id = %s" % (database_field_name, schema, table_name, table_name, mapping_id)
                self.cursor.execute(sql)
                field_old = self.cursor.fetchone()
                if field_old is not None:
                    sql = "UPDATE %s.%s SET %s = '%s' WHERE %s_id = %s" % (
                        schema, table_name, database_field_name, match, table_name, mapping_id)
                    if database_sub_field_name and database_sub_field_value:
                        sql = "UPDATE %s.%s SET %s = '%s', %s = '%s' WHERE %s_id = %s" % (
                            schema, table_name, database_field_name, match, database_sub_field_name,
                            database_sub_field_value, table_name, mapping_id)
                    self.cursor.execute(sql)
