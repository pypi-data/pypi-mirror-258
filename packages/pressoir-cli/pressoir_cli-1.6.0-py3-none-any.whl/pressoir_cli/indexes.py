...
def collect_indexes_and_add_ids(target, tocbook, kinds):
    ids = defaultdict(dict)
    refs = defaultdict(dict)
    for toc_page in tocbook:
        html_file = target / f"{toc_page['id']}.html"
        if not html_file.exists():
            break
        html_content = html_file.read_text()
        parsed_html = HTMLParser(html_content)
        for kind, title in kinds.items():
            chapter_id = html_file.name[: -len(".html")]
...
def generate_indexes(repository_path, target_path, configuration):
    ...
    ids, refs = collect_indexes_and_add_ids(target_path, tocbook, kinds)
