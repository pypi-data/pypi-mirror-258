...
class Env(IEnv):
    ...
    def service_subnets(self, subnet_type: SubnetType):
        ...
        print("**** environment.py. self.vpc_subnets(subnet_type):", self.vpc_subnets(subnet_type))
        vpc_subnets = self.vpc_subnets(subnet_type)
        if not self.config.subnet_map:
            print("**** environment.py. self.config.subnet_map:", self.config.subnet_map)
            print("**** environment.py. no config.subnet_map, so vpc_subnets:", vpc_subnets)
            return vpc_subnets

        print("**** environment.py. self.config.subnet_map:", self.self.config.subnet_map)
