    ...
    async def confirm_remote_startup(self):
        ...
        while not ready_to_connect:
            ...
            if status:
                ...
                # phase が running の場合 は ready_to_connect を True にして終了
                if pod_phase == "running":
                   if self.assigned_host != "":
                    ...
                    self.log.debug(f"Kernel container phase: '{pod_phase}'")
                    if 'conditions' in status:
                        for condition in status['conditions']:
                            # check if the podScheduled condition is True
                            # podScheduled が False の場合は以下のケースが考えられる
                            # - NodeSelector が設定されているが、対象の Node が存在しない
                            # - node が pod を実行するのに必要なリソースが不足している
                            # - node に Taint が設定されているが pod が Tolerations を持っていない
                            # - kube-scheduler がおかしい
                            if condition['type'] == "PodScheduled" and condition['status'] != "True":
                                reason = f"Pod is not scheduled: '{condition['message']}'."
                                self.log_and_raise(RuntimeError(reason))

                            # ContainersReady が False の場合は以下のケースが考えられる
                            # - ContainerCreating の状態である
                            # - PodInitializing の状態である(initContainer がないので無視)
                            # - CreateContainerError が発生している
                            # - errImagePull(コンテナのいずれかが pull に失敗) が発生している 
                            # - ImagePullBackOff(コンテナのいずれかが再 pull を待っている) が発生している
                            # - CrashLoopBackOff(コンテナのいずれかが再起動を待っている) が発生している

                            # check if the ContainerCreating
                            if status['containerStatuses'][0]['state']['waiting']['reason'] == "ContainerCreating":
                                reason = f"Container is creating: '{status['containerStatuses'][0]['state']['waiting']['message']}'."
                                self.log_and_raise(RuntimeError(reason))

                            # check if the CreateContainerError
                            if status['containerStatuses'][0]['state']['waiting']['reason'] == "CreateContainerError":
                                reason = f"Container is in CreateContainerError: '{status['containerStatuses'][0]['state']['waiting']['message']}'. Check server logs."
                                self.log_and_raise(RuntimeError(reason))

                            # check if the ErrImagePull
                            if status['containerStatuses'][0]['state']['waiting']['reason'] == "ErrImagePull":
                                reason = f"ImagePull failed: '{status['containerStatuses'][0]['state']['waiting']['message']}'."
                                self.log_and_raise(RuntimeError(reason))

                            # check if the imagePullBackOff
                            if status['containerStatuses'][0]['state']['waiting']['reason'] == "ImagePullBackOff":
                                reason = f"ImagePull failed: '{status['containerStatuses'][0]['state']['waiting']['message']}'."
                                self.log_and_raise(RuntimeError(reason))

                            # check if the CrashLoopBackOff
                            if status['containerStatuses'][0]['state']['waiting']['reason'] == "CrashLoopBackOff":
                                reason = f"Container is in CrashLoopBackOff: '{status['containerStatuses'][0]['state']['waiting']['message']}'. Check server logs."
                                self.log_and_raise(RuntimeError(reason))         

                            # check if the containersReady condition is True
                            # 上記以外で containersReady が False の場合 をハンドルする
                            if condition['type'] == "ContainersReady" and condition['status'] != "True":
                                # containers are not ready, so we should terminate the kernel
                                reason = f"Containers are not ready: '{condition['message']}'."
                                self.log_and_raise(RuntimeError(reason))

                            # check if the podReady condition is True
                            # containersReady が True で podReady が False の場合考えられるのは以下のケース
                            # - schedule された Node の状態がおかしい
                            # - ReadinessProbe が失敗している
                            # - ReadinessProbe が設定されていない
                            # - nerworkPolicy や service の設定
                            if condition['type'] == "PodReady" and condition['status'] != "True":
                                # pod is not ready, so we should terminate the kernel
                                reason = f"Pod is not ready: '{condition['message']}'."
                                self.log_and_raise(RuntimeError(reason))
