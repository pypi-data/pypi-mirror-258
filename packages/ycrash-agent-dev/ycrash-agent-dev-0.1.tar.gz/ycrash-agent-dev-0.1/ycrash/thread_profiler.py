...
def capture_export_thread_data(filepath,config):
    ...
    with open(filepath, 'w') as outfile:
        for thread_id, thread_info in threads.items():
            ...
            thread = {"threads": [thread_events]}  # Dictionary with threadevents list as a value
        thread = {
                "timeStamp": current_time,
                "threads": [thread_events],  # Dictionary with threadevents list as a value
                "process_thread_details": run_command(f" ps | grep {get_current_pid()}")}
        thread_json = json.dumps(thread, indent=4)
        upload_json_data('application/json', getKey(config), getServerUrl(config) + "/python/thread_dump", thread_json)
