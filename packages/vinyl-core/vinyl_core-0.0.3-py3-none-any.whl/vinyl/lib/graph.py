...
from vinyl.lib.utils.context import is_notebook
from vinyl.lib.utils.graph import get_label, rustworkx_to_networkx
from vinyl.lib.utils.graphics import TurntableTextualApp


def find_backend(node: ops.Node):
...
class VinylGraph(rx.PyDiGraph):
    ...
    @classmethod
    def new_init_from_expr(cls, expr, label_func=None):
        return VinylGraph.init_from_expr_helper(expr.op(), VinylGraph(), {}, label_func)

    @staticmethod
    def init_from_expr_helper(
        ...
        if isinstance(u, ops.Node):
            ...
            for v in u.__children__:
                if isinstance(v, ops.Node):
                    ...
                    g = VinylGraph.init_from_expr_helper(v, g, node_dict, label_func)

        node_dict = {}  # reset to ensure no future issues
        return g

    @staticmethod
    def find_unaltered_cols(expr) -> dict[ir.Expr, ir.Expr]:
        g = VinylGraph.new_init_from_expr(expr.select(s.all()).tbl)
            ...
            if isinstance(g[node], ops.Relation):
                ...
                for in_node in in_nodes:
                    for out_node in out_nodes:
                        if g[in_node].name == g[out_node].name:
        ...
        for col in columns_to_check:
            ...
            if len(root) > 1:
                ...
                unaltered[orig_g[col]] = sub[root[0]]

        return unaltered

    @classmethod
    def visualize(cls, expr: ir.Expr):
        g_rust = cls.new_init_from_expr(expr, label_func=get_label)
        g = rustworkx_to_networkx(
            g_rust,
...
def unify_backends(tbl, visited_nodes=set()):
    g = VinylGraph.new_init_from_expr(tbl)
    for i in rx.topological_sort(g):
        ...
        if isinstance(
            ...
            sources = [find_backend(n) for n in nodes]
            ...
            if len(non_null_sources) > 0 and len(set(non_null_sources)) > 1:
                ...
                return unify_backends(tbl, visited_nodes)
