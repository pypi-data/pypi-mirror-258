...
from .resnet import PRM


def fc_resnet50(num_classes: int = 20, pretrained: bool = True) -> nn.Module:
    """FC ResNet50."""
    model = PRM(num_classes, models.resnet50(pretrained))
    return model
...
def prm_visualize(
    ...
    if len(instance_list) > 0:
        ...
        height, width = (
            instance_list[0]["mask"].shape[0],
            instance_list[0]["mask"].shape[1],
        )
        ...
        for idx, pred in enumerate(instance_list):
            ...
            h, s, _ = rgb2hsv(
                palette[idx + 1][0], palette[idx + 1][1], palette[idx + 1][2]
            )
