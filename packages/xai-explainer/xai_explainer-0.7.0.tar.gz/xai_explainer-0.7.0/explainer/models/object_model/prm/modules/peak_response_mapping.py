...
from ..functions import peak_stimulation, pr_conv2d
...
class PeakResponseMapping(nn.Sequential):
    ...
    def instance_seg(
        ...
        for i in range(len(peak_response_maps)):
            ...
            class_response = cv2.resize(
                class_response_maps[class_idx],
                (img_height, img_width),
                interpolation=cv2.INTER_CUBIC,
            )
            bg_response = (
                class_response < bg_threshold_factor * class_response.mean()
            ).astype(np.float32)
            ...
            for j in range(min(proposal_count, len(proposals))):
                ...
                contour_mask = cv2.morphologyEx(
                    raw_mask,
                    cv2.MORPH_GRADIENT,
                    np.ones((contour_width, contour_width), np.uint8),
                ).astype(bool)
                ...
                if (mask_area >= proposal_size_limit[1] * img_area) or (
                    ...
                    val = (
                        balance_factor * peak_response_map[mask].sum()
        ...
        if nms_threshold is not None:
            instance_list = self.instance_nms(
                sorted(instance_list, key=lambda x: x[0], reverse=True),
                nms_threshold,
                merge_peak_response,
            )
        return [dict(category=v[1], mask=v[2], prm=v[3]) for v in instance_list]

    def forward(self, input, class_threshold=0, peak_threshold=2, retrieval_cfg=None):
        ...
        if self.inferencing:
            ...
            for idx in range(peak_list.size(0)):
                ...
                if aggregation[peak_list[idx, 0], peak_list[idx, 1]] >= class_threshold:
                    ...
                    # peak_threshold = 0.2
                    # print(
                    #    "peak_val", peak_val.item(), "   peak_threshold", peak_threshold
                    # )
