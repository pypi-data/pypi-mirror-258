...
class PeakStimulation(Function):
    ...
    def forward(ctx, input, return_aggregation, win_size, peak_filter):
        ...
        element_map = (
            torch.arange(0, h * w)
            .long()
            .view(1, 1, h, w)[:, :, offset:-offset, offset:-offset]
        )
        element_map = element_map.to(input.device)
        _, indices = F.max_pool2d(
            padded_maps, kernel_size=win_size, stride=1, return_indices=True
        )
        ...
        if return_aggregation:
            ...
            return peak_list, (input * peak_map).view(batch_size, num_channels, -1).sum(
                2
