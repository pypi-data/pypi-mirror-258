...
def _load_cell_partionings(
    ...
    files = importlib.resources.files("explainer.resources.cell_partitionings")

    contents = []

    for cell_level in cell_levels:
        content = files.joinpath(f"50_{cell_level}.csv").read_text(encoding="utf-8")
...
_PARTITIONINGS = _load_cell_partionings()


def load_cell_partionings() -> Dict[str, List[str]]:
    """
    Returns:
        Dict[str, List[str]]: Dictionary with keys "files" and "shortnames"
    """
    return copy.deepcopy(_PARTITIONINGS)
