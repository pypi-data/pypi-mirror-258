...
def add_multiplicative_gaussian_noise(df, series_id, column, stddev=0.1, noise_type='proportional'):
    """
    ...
    def gen_query(df, series_id, column, stddev, noise_type):
        """
        ...
        if noise_type == 'proportional':
            formula = f'Y=X1*(1 + {stddev}*1./sqrt(2.)*sqrt(-2.0*ln(RANDOM(1,999999999)/1000000000)) * cos(2.0*3.14159265358979323846 * RANDOM(0,999999999)))'
        elif noise_type == 'additive':
            formula = f'Y=X1 + {stddev}*1./sqrt(2.)*sqrt(-2.0*ln(RANDOM(1,999999999)/1000000000)) * cos(2.0*3.14159265358979323846 * RANDOM(0,999999999))'
    ...
    for i, c in enumerate(columns):
        if c in column:
            columns[i] = f'B{column.index(c)}.MAGNITUDE AS ' + c
        else:
            columns[i] = 'A.' + c
    columns = ', \n'.join(columns)
