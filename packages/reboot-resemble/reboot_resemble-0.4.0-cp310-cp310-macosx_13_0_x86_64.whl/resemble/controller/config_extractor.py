...
from resemble.naming import get_local_application_id
...
class LocalConfigExtractor:
    ...
    def config_from_serviceables(
        ...
        # Because we're running locally we don't know the application name, and
        # therefore can't compute the usual application ID. We'll instead
        # compute an alternative "local" application name based on the services
        # included in this config.
        service_names = [s.service_name() for s in serviceables]
        application_id = get_local_application_id(service_names)
