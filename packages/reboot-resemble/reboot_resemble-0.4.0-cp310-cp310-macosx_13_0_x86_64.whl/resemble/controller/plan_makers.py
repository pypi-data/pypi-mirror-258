...
class PlanMaker:
    ...
    def make_plan(
        ...
        """
        service_plans = []
        # Map service names to their application ID to make sure each service is
        # only served by one application.
        application_id_by_service_name: dict[str, str] = {}
        for application_config in application_configs:
            ...
            for service_name in application_config.spec.service_names:
                ...
                if service_name in _ServiceServer.SYSTEM_SERVICE_NAMES:
                    continue
                if service_name in application_id_by_service_name:
                    raise ValueError(
                        f"Service '{service_name}' registered under multiple "
                        f"applications: "
                        f"'{application_id_by_service_name[service_name]}' "
                        f"and '{application_id}'"
                    )
                ...
                application_id_by_service_name[service_name] = application_id
