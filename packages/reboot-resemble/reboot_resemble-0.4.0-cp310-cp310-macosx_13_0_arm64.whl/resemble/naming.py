...
# The ID of the space that's used when an application is running locally, i.e.
# in a unit test or in `rsm dev`.
LOCAL_SPACE_ID = 'local'
...
def get_local_application_id(service_names: list[str]) -> ApplicationId:
    """
    Produces an application ID for a local application.

    Local applications may not have names, and may therefore not be able to use
    the normal `get_application_id()` method. This function produces an
    alternative application ID for such local applications, based on the
    services they run.
    """
    application_alternative_name = str(service_names)
    return get_application_id(LOCAL_SPACE_ID, application_alternative_name)
