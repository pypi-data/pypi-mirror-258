...
class DBHelper:
    ...
    def __init__(self, config):
        ...
        database = importlib.import_module(self.sql_conns.get(self.config.setdefault('SQL_CREATOR', 'MySQL')))
        if self.sql_conns.get(self.config.setdefault('SQL_CREATOR', 'MySQL')) == "sqlite3":
            self.pool = PooledDB(
                creator=database,
    ...
    def get_placeholder(self):
        ...
        :return:
        """
        dbtype = self.sql_conns.get(self.config.setdefault('SQL_CREATOR', 'MySQL'))
        if dbtype == "MySQL":
            return '%s'
