...
class OpenNucleome:
    ...
    def load_default_settings(self):
        ...
        ideal_param_file = "./parameters/HFF_100KB/ideal_param_file.txt"
        compt_param_file = "./parameters/HFF_100KB/compt_param_file.txt"
        interchr_param_file = './parameters/HFF_100KB/interchr_param_file.txt'
        chr_nuc_param = "./parameters/HFF_100KB/chr_nuc_param.txt"
        chr_spec_param = "./parameters/HFF_100KB/chr_spec_param.txt"
        chr_lam_param = "./parameters/HFF_100KB/chr_lam_param.txt"
    ...
    def load_customized_settings(self, force_field, k = 1.0):
        ...
        if ideal_param_file:
            ideal_param_file = os.path.join(config_path, ideal_param_file)
        compt_param_file = force_field.loc['chromosome']['compt_param_file']
        if compt_param_file:
            compt_param_file = os.path.join(config_path, compt_param_file)
        interchr_param_file = force_field.loc['chromosome']['interchr_param_file']
        if interchr_param_file:
            interchr_param_file = os.path.join(config_path, interchr_param_file)
        chr_nuc_param = force_field.loc['nucleolus']['chr_nuc_param']
        if chr_nuc_param:
            chr_nuc_param = os.path.join(config_path, chr_nuc_param)
        chr_spec_param = force_field.loc['speckle']['chr_spec_param']
        if chr_spec_param:
            chr_spec_param = os.path.join(config_path, chr_spec_param)
        chr_lam_param = force_field.loc['lamina']['chr_lam_param']
        if chr_lam_param:
            chr_lam_param = os.path.join(config_path, chr_lam_param)
        self.add_chromosome_potential(dict_chrom_ff, ideal_param_file, compt_param_file, interchr_param_file)
        self.add_speckle_potential(dict_spec_ff, chr_spec_param)
        self.add_nucleolus_potential(dict_nuc_ff, chr_nuc_param)
        self.add_lamina_potential(dict_lam_ff, chr_lam_param)
