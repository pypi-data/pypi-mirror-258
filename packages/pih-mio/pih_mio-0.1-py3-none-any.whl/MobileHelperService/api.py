...
class MobileHelper(OutputStub):
    ...
    def __init__(self, pih: PIH, stdin: Stdin):
        ...
        self.show_note_node: CommandNode = CommandNode(
            ...
            lambda: self.find_note_handler(False),
            filter_function=lambda: self.is_not_all or self.in_choose_command,
        ...
        with self.output.make_instant_mode():
            if CheckableSections.POLIBASE in section_list:
                with self.output.make_redirection():
                    self.run_command(
                        A.CT_CMDT.POLIBASE, one(A.R_F.find("doctemplets statistics")).text
                    )
                    additional_text[CheckableSections.POLIBASE] = jnl(self.output.redirected_text_list)
    ...
    def find_note_handler(self, show_all: bool = False) -> None:
            ...
            if e(gkeep_item_list_result):
                ...
                for gkeep_item in gkeep_item_list:
                    with self.output.make_loading(1, "Идет загрузка заметки. Ожидайте"):
                        note_result: Result[None] = A.R_N.get_by_id(gkeep_item.id)
