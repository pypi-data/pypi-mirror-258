...
class PolibaseDBApi:
    ...
    def create_dump(file_name: str | None = None, test: bool | None = None) -> None:
        ...
        file_name_result = A.PTH.add_extension(file_name_result, file_in_ext)
        file_database_dump_name_out: str = A.PTH.add_extension(file_name, file_in_ext)
        ...
        polibase_test_folder_path: str = f"//{A.CT_H.POLIBASE_TEST.NAME}/{local_polibase_database_dump_folder_name}"
        ...
        file_out_name = A.PTH.add_extension(file_name, file_out_ext)
        par_file_name = "backpar.txt"
        nas_connection_command = f"net use {nas_mount_point} {nas_folder_path} /user:{nas_user} {nas_password}"
        polibase_test_connection_command = js(
            ("net use", polibase_test_mount_point, polibase_test_folder_path)
        )
        nas_copy_command = (
        ...
        os.system(nas_connection_command)
        os.system(polibase_test_connection_command)
        ...
        # step 6
        polibase_test_previous_file_delete_command: str = (
            f"del {polibase_test_mount_point}/{file_name_result}"
        )
        polibase_file_rename_command: str = f"ren {polibase_test_mount_point}/{file_database_dump_name_out} {file_name_result}"
