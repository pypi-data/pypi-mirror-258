...
if os.name == "nt":
    os.putenv("PATH", os.path.abspath(os.path.join(base_dir, __relative_tesslib_path, "tesslib")))
    lib_path = os.path.abspath(os.path.join(base_dir, __relative_tesslib_path,'tesslib', "libtesseract-5.dll"))
    if not os.path.isfile(lib_path):
        raise 'libtesseract-5.dll not find'
...
def create_ocr(char_whitelist=None, lang="eng"):
    ...
    __api_dict[name] = lib,api
    return lib,api # lib and api need pair for use


def ocr_image(lib,api, image):
...
def ocr_conf(lib,api):
    return lib.TessBaseAPIMeanTextConf(api)


def release_ocr(lib,api):
...
def release_all():
    global __api_dict
    for lib,api in __api_dict.values():
        release_ocr(lib,api)
    __api_dict.clear()
