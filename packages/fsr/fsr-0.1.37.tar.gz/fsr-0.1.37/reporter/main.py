...
def find_images(test_name, base_dir, type, image_folder):
    logging.debug("Searching for images for test %s of type %s in %s", test_name, type, image_folder)
    # Проверяем, является ли image_folder абсолютным путем. Если нет, преобразуем его в абсолютный путь относительно текущей директории.
    if not os.path.isabs(image_folder):
        image_folder = os.path.join(os.getcwd(), image_folder)
    logging.debug("Using image folder: %s", image_folder)
    
    target_dir = os.path.join(os.getcwd(), 'test_report', 'data', type)
    images = []
    for root, dirs, files in os.walk(base_dir):
        if type in root.split(os.sep) and os.path.abspath(root).startswith(os.path.abspath(image_folder)):
            for file in files:
                if file.startswith(test_name) and file.endswith('.png'):
                    ...
                    if source_path != destination_path:
                        ...
                        logging.debug("Image found and copied: %s", file)
    if not images:
        logging.info("For test %s, images not found.", test_name)
    return images
        ...
        try:
            ...
            if item['type'] in ['testStart', 'testDone', 'print']:
                ...
                if item['type'] == 'testStart':
                    ...
                    if test_type == "goldens":
                        images_info = {
                            'goldens': find_images(test_name, image_folder, 'goldens', image_folder),
                            'failures': find_images(test_name, image_folder, 'failures', image_folder)
                        }
                    test_results.append({
                        'id': test_id,
                        'name': test_name,
