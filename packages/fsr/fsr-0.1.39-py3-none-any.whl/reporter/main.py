...
def find_images(test_name, base_dir, type, image_folder):
    ...
    logging.debug("Looking for images in %s of type %s for test %s", image_folder_path, type, test_name)
    
    target_dir = os.path.join(os.getcwd(), 'test_report', 'data', type)
    images = []
    
    if not os.path.exists(image_folder_path):
        logging.warning("Image folder does not exist: %s", image_folder_path)
        return images
            ...
            for file in files:
                if file.startswith(test_name) and file.endswith('.png'):
                    ...
                    if not os.path.exists(destination_path):
                        ...
                        logging.debug("Image copied: %s", source_path)
                    else:
                        logging.debug("Image already exists: %s", destination_path)
    if not images:
        logging.info("No images found for test %s in %s", test_name, type)
    return images
