            ...
            while len(recreate_list) > 0:
                ...
                try:
                    ...
                    if cache_item.refresh_error_count < ERROR_COUNT_MAX:
                        log_exception(f"Cache [{cache_item.cache_key}] konnte nicht neu erstellt werden. Neuer Versuch beim nächsten Durchlauf.")
                        recreate_list.append(cache_item)
                    else:
                        log_exception(f"Cache [{cache_item.cache_key}] konnte nach mehrmaligem Versuch nicht neu erstellt werden und wird daher jetzt ignoriert.")
