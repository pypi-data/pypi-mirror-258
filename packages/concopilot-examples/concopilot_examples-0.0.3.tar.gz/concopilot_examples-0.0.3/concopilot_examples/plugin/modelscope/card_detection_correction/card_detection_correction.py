...
class ModelScopeCardDetectionCorrection(AbstractPlugin):
    def __init__(self, config: Dict):
        super(ModelScopeCardDetectionCorrection, self).__init__(config)
    ...
    def command(self, command_name: str, param: Any, **kwargs) -> Any:
        if command_name=='detect_and_correct':
            param=AssetRef.try_retrieve(param, self.context.assets)
            re=self.detect_and_correct(
                param.get('img_data'),
                param.get('img_url'),
                param.get('img_path'),
            ...
            raise ValueError(f'Unknown command: {command_name}. Only "detect_and_correct" is acceptable.')
