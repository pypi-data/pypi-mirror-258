...
from .letters import (UNSHAPED, ISOLATED, TATWEEL, ZWJ, LETTERS_KURDISH, FINAL,
...
class KurdishReshaper(object):
    """
    A class for Kurdish reshaper, it allows for fine-tune configuration over the
    API.

    If no configuration is passed to the constructor, the class will check for
    an environment variable :envvar:`PYTHON_Kurdish_RESHAPER_CONFIGURATION_FILE`
    ...
    * Python 3: https://docs.python.org/3/library/configparser.html
    ...
    def __init__(self, configuration=None, configuration_file=None):
        super(KurdishReshaper, self).__init__()
    ...
    def reshape(self, text):
        ...
        return bidi.algorithm.get_display(''.join(result))


default_reshaper = KurdishReshaper()
