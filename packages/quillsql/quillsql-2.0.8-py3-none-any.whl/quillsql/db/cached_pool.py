...
class CachedPool:
    def __init__(self, config, cache_config):
        ...
        self.cur = self.pool.cursor(cursor_factory=RealDictCursor)
        self.orgId = None
    ...
    def exec_with_reconnect(self, sql):
        reconnect_count = 0
        while True:
            self.pool = psycopg2.connect(self.config)
            self.cur = self.pool.cursor(cursor_factory=RealDictCursor)
            try:
    ...
    def exec(self, sql):
        try:
            self.exec_with_reconnect(sql)
        except psycopg2.Error as err:
            raise PgQueryError(err, sql, err.diag.statement_position)
        fetch = self.cur.fetchall()
        fields = [
            {"name": desc[0], "dataTypeID": desc[1]} for desc in self.cur.description
        ]
