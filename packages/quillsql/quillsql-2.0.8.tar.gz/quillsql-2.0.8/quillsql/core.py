...
from quillsql.db.cached_pool import CachedPool
...
class Quill:
    def __init__(
        ...
        self.target_pool = CachedPool(database_connection_string, cache)
    ...
    def run_queries(self, queries, metadata=None, runQueryConfig=None):
        ...
        if runQueryConfig and runQueryConfig.get("arrayToMap"):
            array_to_map(
                queries, runQueryConfig.get("arrayToMap"), metadata, self.target_pool
            )
            return {"queryResults": []}
        else:
