...
class InstanceDecoder(DecoderBase):
    ...
    def metrics(self, x: Tensor, y: Tensor, stage: str) -> dict[str, Any]:
        ...
        df = pd.DataFrame({
            "x": torch.cat([data[p].x_instance_coords[:,0] for p in nudata.planes], dim=0),
            "y": torch.cat([data[p].x_instance_coords[:,1] for p in nudata.planes], dim=0),
            "instance": torch.cat([data[p].y_instance for p in nudata.planes], dim=0).numpy().astype(str),
        })
        fig = px.scatter(df, x='x', y='y', color='instance')
        fig
        return metrics
