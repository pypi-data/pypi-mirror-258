...
class EOF:
    ...
    def solve(self, method="eig", st=False, dim_min=10, chunks=None):
        ...
        if method not in ['eig', 'svd', 'dask_svd']:
            raise ValueError(f"method must be 'eig' or 'svd' or 'dask_svd, not {method}")
        # mask data
        self._mask_nan()
        # solve maksed data
        ...
        # ================================= EOF process end===============================
            ...
            if dim_min is None:
                dim_min = np.min([dim0_len, dim1_len])
            U, Sigma, V = dsa.linalg.svd_compressed(Xor, k=dim_min,n_power_iter=3)
            U.compute()
            Sigma.compute()
            V.compute()
            e_vector = np.array(V)
    ...
    def north_test(self, npt=10, perc=False, ddof=None):
        """
        Args:
            north test for EOF
