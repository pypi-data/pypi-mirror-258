...
def field_corr(field1, field2):
    ...
    field2: time, nspace
    """
    if field1.shape[0] != field2.shape[0]:
        raise ValueError("field1 and field2's time dimension must be equal")
...
def autocorr(x: np.ndarray,lags:int):
    """
    compute autocorrelation of x at all lags

    Args:
        x (np.ndarray): data, 2D the first dimension is sample number, the second dimension is feature number
        lags (int): lags
    """
    if x.shape[0] < lags:
        raise ValueError("lags is larger than x's time dimension")
    if lags == 0:
        return np.ones(x.shape[1:])
    else:
        x1 = x[:-lags]
        x2 = x[lags:]
        lag_corrs = field_corr(x1, x2)
        return lag_corrs

# def ddof(data: np.ndarray,):
#     """ Calculate degree of freedom
#     data: time, nspace
#     """
#     max_ddof = data.shape[0] - 1
#     num = data.shape[0]
#     Neff = 1 / num
#     for ii in range(0, num):
#         lag_corrs = autocorr(data, ii)
        


def ddof(data: np.ndarray, axis: int = 0):
    """ Calculate degree of freedom

    Args:
        data (np.ndarray): data
        axis (int, optional): axis to calculate. Defaults to 0.

    Returns:
        degree of freedom
    """

    ddof = data.shape[axis] - 1
