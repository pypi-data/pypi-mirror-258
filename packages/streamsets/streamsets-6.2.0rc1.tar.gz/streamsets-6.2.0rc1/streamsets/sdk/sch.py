...
from .sch_models import (
    ...
    ScheduledTaskActions, ScheduledTaskBuilder, ScheduledTasks, ScheduledTaskStatus, SelfManagedDeployment,
    SelfManagedEnvironment, StPipelineBuilder, SubscriptionAudit, SubscriptionBuilder, Subscriptions, Topologies,
...
class ControlHub:
    ...
    def cleanup_deployment(self, deployment):
        """Deletes all scheduled tasks, jobs and pipelines that belong to that deployment.

        Args:
            deployment (:py:obj:`streamsets.sdk.sch_models.Deployment`): Deployment to be cleaned.

        """

        if not isinstance(deployment, Deployment):
            raise TypeError("Deployment should be of type :py:class:`streamsets.sdk.sch_models.Deployment`")

        logger.info("Cleanup deployment called for deployment id {}...".format(deployment.deployment_id))

        # since a deployment can theoretically have more than one engine we should be checking against all engine ids
        # current tests run with only one engine per deployment
        engine_ids = [engine.id for engine in deployment.registered_engines]
        logger.info("Engine IDS to clean up are: {}.".format(engine_ids))

        # find all pipelines in the deployment
        pipelines_to_delete = [pipeline for pipeline in self.pipelines if pipeline.sdc_id in engine_ids]
        logger.info("Pipelines to delete: {}.".format(pipelines_to_delete))

        # find all jobs based on if their pipeline belongs to the current deployment
        jobs_to_delete = [job for job in self.jobs if job.pipeline.sdc_id in engine_ids]

        # draft runs are jobs too
        draft_runs_to_delete = []
        for draft_run in self.draft_runs:
            try:
                if hasattr(draft_run, 'pipeline') and draft_run.pipeline.sdc_id in engine_ids:
                    draft_runs_to_delete.append(draft_run)
            except HTTPError as e:
                # sometimes the pipeline is deleted for a stopped draft run, we should ignore those
                if e.response.json()['ISSUES'][0]['code'] == 'PIPELINE_STORE_09':
                    logger.debug('Leftover draft run of a stopped pipeline: {}'.format(draft_run.job_id))
                else:
                    raise

        jobs_to_delete.extend(draft_runs_to_delete)

        job_ids = [job.job_id for job in jobs_to_delete]
        logger.info("Dangling jobs: {}.".format(jobs_to_delete))  # this will log for draft runs too

        # find all scheduled tasks based on the jobs
        scheduled_tasks_by_status = {status.value: list() for status in ScheduledTaskStatus}
        scheduled_tasks_to_delete = [
            scheduled_task for scheduled_task in self.scheduled_tasks if scheduled_task.job_id in job_ids
        ]

        # group all tasks by their status i.e, RUNNING, PAUSED, KILLED, DELETED
        for scheduled_task in scheduled_tasks_to_delete:
            scheduled_tasks_by_status[scheduled_task.status].append(scheduled_task)
        logger.info("Dangling Scheduled Tasks by status: {}.".format(scheduled_tasks_by_status))

        logger.info("Killing Scheduled Tasks.")
        for alive_status in [ScheduledTaskStatus.RUNNING.value, ScheduledTaskStatus.PAUSED.value]:
            if scheduled_tasks_by_status[alive_status]:
                self.kill_scheduled_tasks(
                    *scheduled_tasks_by_status[alive_status],
                )
                # add the tasks to killed status
                scheduled_tasks_by_status[ScheduledTaskStatus.KILLED.value].extend(
                    scheduled_tasks_by_status[alive_status],
                )

        if scheduled_tasks_by_status[ScheduledTaskStatus.KILLED.value]:
            logger.info("Deleting Scheduled Tasks.")
            self.delete_scheduled_tasks(*scheduled_tasks_by_status[ScheduledTaskStatus.KILLED.value])
        else:
            logger.info('No scheduled tasks to delete.')

        if jobs_to_delete:
            logger.info("Stopping Jobs.")
            try:
                self.stop_job(*jobs_to_delete, force=True)
            except TimeoutError:
                logger.info('Timeout error when trying to stop jobs')

            self.delete_job(*jobs_to_delete)
        ...
        logger.info("Deployment cleaned.")

    def delete_deployment(self, *deployments, cleanup_deployments=False):
        """Delete deployments.

        Args:
            *deployments (:py:class:`streamsets.sdk.sch_models.Deployment`): One or more deployments.
            cleanup_deployments (:py:obj:`bool`): Whether to run cleanup on the deployments.
        """
        deployments = [deployment for deployment in deployments if not isinstance(deployment, NotImplementedDeployment)]
        if not deployments:
        ...
        if cleanup_deployments:
            for deployment in deployments:
                self.cleanup_deployment(deployment)
