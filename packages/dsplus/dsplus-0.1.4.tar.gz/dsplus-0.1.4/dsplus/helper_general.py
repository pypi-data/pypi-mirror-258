...
#%%
def smoothen_line(y:list|np.ndarray|pd.Series, x:list|np.ndarray|pd.Series = None, method:int = 1, window_frac:float = 0.05, window_count:int = None, span:float = 3, polyorder:int = 2) -> pd.Series:
    ...
    Args:
        ...
        method (int, optional): 1 for Savitzky-Golay Filter ('scipy.signal.savgol_filter()'), 2 for Lowess Smoothing ('statsmodels.nonparametric.smoothers_lowess()'), 3 for Exponential Moving Average ('pd.ewm()'). Defaults to 1.
    ...
    Notes:
        - Method 1 requires polyorder and one of window_frac and window_size.
        - Method 2 requires one of window_frac and window_size.
        - Method 1 requires span. 

    Examples:
        >>> smoothen_line(df['y'])
        >>> smoothen_line(df['y'], method = 1, window_frac = 0.03)
        >>> smoothen_line(df['y'], df['x'], method = 2)
        >>> smoothen_line(df['y'], df['x'], method = 2, window_frac = 0.03, polyorder = 3)
        >>> smoothen_line(df['y'], method = 3)
        >>> smoothen_line(df['y'], method = 3, span = 3)       
    '''
...
#%%
def np_seq(start = None, end = None, by = None, count = None):
    if pd.Series([start, end, by]).notna().all():
        seq = np.arange(start, end, by)
    elif pd.Series([start, end, count]).notna().all():
        seq = np.linspace(start, end, count)
    elif pd.Series([start, by, count]).notna().all():
        seq = np.arange(start, start + by*count, by)
    elif pd.Series([start, end]).notna().all():
