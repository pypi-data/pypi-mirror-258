...
def pd_pivot_wider(df_long: pd.DataFrame, 
    ...
    if len(values) == 1 & ~keep_val:
        df_wide.columns = [col[1] for col in df_wide.columns]
    else:
        if col_first:
            df_wide.columns = [join_sep.join(col[::-1]) for col in df_wide.columns]
        else:
            df_wide.columns = [join_sep.join(col) for col in df_wide.columns]
...
#%%
def pd_print(df: pd.DataFrame) -> pd.DataFrame:
