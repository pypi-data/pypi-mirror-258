...
class MqttSpbEntity:
    ...
    def publish_data(self, send_all=False):
        ...
        if send_all or self.data.is_updated():
            ...
            self._loopback_topic = topic
            self._mqtt.publish(topic, payload_bytes, 1, True)
    ...
    def publish_specific_data(self, topicType: str, name: str, value):
        """
            Send data to specific topic

        :param      topicType: string   'NDEATH', 'DDEATH', 'NBIRHT', 'DBIRHT', 'NDATA', 'DDATA', 'NCMD', 'DCMD'     
                    name: string
                    value: any
        :return:                        result
        """
        ...
        if 'DDEATH' in topicType:
            qos = 1
            retain = True
        elif 'NDEATH' in topicType:
            qos = 0
            retain = True
        elif 'NCMD' in topicType:
            qos = 1
            retain = False
        elif 'DCMD' in topicType:
            qos = 1
            retain = False

        payload = Payload()
        timestamp=int(round(datetime.datetime.now().utcnow().timestamp() * 1000))

        addMetric(payload, name, None, self._spb_data_type(value), value, timestamp=timestamp)
        payload_bytes = bytearray(payload.SerializeToString())
        if self._entity_is_scada:  # If it is a type entity SCADA, change the DEATH certificate
            topic = "spBv1.0/" + self.spb_group_name + "/" + topicType + "/" + self._spb_eon_name
        else:  # Normal node
            if self._spb_eon_device_name is None:  # EoN
                topic = "spBv1.0/" + self.spb_group_name + "/" + topicType + "/" + self._spb_eon_name
            else:
                topic = "spBv1.0/" + self.spb_group_name + "/" + topicType + "/" + self._spb_eon_name + "/" + self._spb_eon_device_name
        self._mqtt.publish(topic, payload_bytes, qos, retain)

        logger.info("%s - Published %s message %s" % (self._entity_domain, topicType, topic))
        return True
    ...
    def connect(self,
        ...
        if self._entity_is_scada:  # If it is a type entity SCADA, change the DEATH certificate
            ...
            if self._spb_eon_device_name is None:  # EoN
                ...
                self._mqtt.will_set(topic, payload_bytes, 1, True)  # Set message
            else:
    ...
    def _mqtt_on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            #################################################################################################
            # Entity DEATH message - reset DEATH message
            if self._entity_is_scada:  # If it is a type entity SCADA, change the DEATH certificate
                topic = "spBv1.0/" + self.spb_group_name + "/STATE/" + self._spb_eon_name
                self._mqtt.publish(topic, "ONLINE".encode("utf-8"), 0, True)  # Set message
            else:  # Normal node
                payload = Payload()
                timestamp=int(round(datetime.datetime.now().utcnow().timestamp() * 1000))

                addMetric(payload, "bdSeq", None, MetricDataType.Int64, 0, timestamp=timestamp)
                payload_bytes = bytearray(payload.SerializeToString())
                if self._spb_eon_device_name is None:  # EoN
                    topic = "spBv1.0/" + self.spb_group_name + "/NDEATH/" + self._spb_eon_name
                    self._mqtt.publish(topic, payload_bytes, 1, True)  # Set message
                # else:
                #     topic = "spBv1.0/" + self.spb_group_name + "/DDEATH/" + self._spb_eon_name + "/" + self._spb_eon_device_name
                #     self._mqtt.publish(topic, payload_bytes, 0, True)  # Set message
            #################################################################################################
...
class MqttSpbEntityEdgeNode(MqttSpbEntity):
    ...
    def publish_command_device(self, spb_eon_device_name, commands):
        ...
        for k in commands:
            timestamp=int(round(datetime.datetime.now().utcnow().timestamp() * 1000))
            addMetric(payload, k, None, self._spb_data_type(commands[k]), commands[k],timestamp)
