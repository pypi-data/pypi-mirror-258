...
def creare_tree_view(names, mean_0, mean_1, directory, threshold_p=0.05, family_colors=None):
    ...
    for node in g.nodes:
        for s in g.succ[node]:
            ...
            # for u test without mimic_da results the name is fixed to the correct version of the taxonomy
            # for the mimic_da results the name is the actual name
            ...
            if s[0][-1] not in T or not any([anc.species == a for anc, a in
                ...
                # taking the node index, if the node is in u_test (without mimic_da results)
                ...
                # nodes in mimic_da results without u-test
...
def calculate_all_imgs_tag_corr(folder, tag, start_i, eval="corr", sis='fdr_bh', correct_first=False,
    ...
    with warnings.catch_warnings():
        ...
        if mode == 'test':
            ...
            if tax != 'nosignificant':
                ...
                # Apply post HOC miMic test
                print("\nRP - post HOC mimic_da on original and addition \n")
                ...
                print("\nSP - post HOC mimic_da on original and addition \n")
