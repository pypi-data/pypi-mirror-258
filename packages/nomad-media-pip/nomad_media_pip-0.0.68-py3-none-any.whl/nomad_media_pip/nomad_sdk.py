...
class Nomad_SDK:
	...
	def create_and_update_event(self, CONTENT_ID: str | None, CONTENT_DEFINITION_ID: str, 
								...
								IS_DISABLED: bool | None, PROPERTIES: dict | None) -> dict:
		try:
			if self.config["apiType"] != "admin":
				raise Exception("This function is only available for admin API type.")
			...
			EVENT = _create_and_update_event(self.token, self.config["serviceApiUrl"], 
				...
				EVENT_TYPE, SERIES, IS_DISABLED, PROPERTIES, self.debug_mode)
	...
	OUTPUT_TYPE (str): The type of the live output. The types are Archive, LiveVodHls, MediaPackage,
	MediaStore, Rtmp, Rtp, RtpFec, and S3.
