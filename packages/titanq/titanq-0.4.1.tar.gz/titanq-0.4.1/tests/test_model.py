...
def test_optimize_no_constraints(model_s3_client, mock_s3_storage, mock_metrics, mock_result):
    ...
    assert mock_s3_storage.array_uploaded['weights_constraints'] == None
    assert mock_s3_storage.array_uploaded['bias_constraints'] == None
...
def test_optimize_with_constraints(model_s3_client, mock_s3_storage, mock_metrics, mock_result):
    ...
    # optimize using sdk
    weights = np.random.rand(10, 10).astype(np.float32)
    weights_constraints = np.random.rand(4, 10).astype(np.float32)
    bias = np.random.rand(10).astype(np.float32)
    bias_constraints = np.random.rand(4).astype(np.float32)

    model.add_variable_vector('x', 10, Vtype.BINARY)
    model.set_objective_matrices(weights, bias)
    model.set_constraints_matrices(weights_constraints, bias_constraints)
    response = model.optimize()
    ...
    assert mock_s3_storage.array_uploaded['bias'] == np_array_to_bytes(bias)
    assert mock_s3_storage.array_uploaded['weights'] == np_array_to_bytes(weights)
    assert mock_s3_storage.array_uploaded['weights_constraints'] == np_array_to_bytes(weights_constraints)
    assert mock_s3_storage.array_uploaded['bias_constraints'] == np_array_to_bytes(bias_constraints)
...
@pytest.mark.parametrize("weights_shape, bias_shape, error", [
    ((2, 10), (2,), None),
    ((10, 10), (10,), None),
    ((2, 10), (3,), ValueError),
    ((2, 10), (2,2), ValueError),
...
def test_set_constraints(model_s3_client, weights_shape, bias_shape, error):
    model_s3_client.add_variable_vector('x', 10, Vtype.BINARY)
...
@pytest.mark.parametrize("weights_dtype, bias_dtype, error", [
    (np.float32, np.float32, None),
    (np.float64, np.float32, ValueError),
    (np.float32, np.float64, ValueError),
    (np.int32, np.int32, ValueError),
])
def test_constraints_dtype(model_s3_client, weights_dtype, bias_dtype, error):
    model_s3_client.add_variable_vector('x', 10, Vtype.BINARY)

    weights = np.random.rand(2, 10).astype(weights_dtype)
    bias = np.random.rand(2).astype(bias_dtype)
