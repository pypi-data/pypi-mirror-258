...
class Model:
    ...
    def set_constraints_matrices(self, weights: np.ndarray, bias: np.ndarray):
        """
        Set the constraints matrices for the model.

        :param weights: The weights_constraints matrix of shape (M, N) where M the number of constraints and N is the number of variables
        :type weights: a NumPy 2-D dense ndarray (must be float32)

        :param bias: The bias_constraints vector of shape (M,) where M is the number of constraints
        :type bias:  a NumPy 1-D ndarray (must be float32)

        :raise MissingVariableError: If no variable have been added to the model.
        :raise ConstraintsAlreadySetError: If a constraints has already been setted in this model.
        :raise ValueError: If the weights shape or the bias shape does not fit the expected shape of this model.
        :raise ValueError: If the weights or bias data type is not f32.
        """
        if self._variables is None:
        ...
        self._constraints = Constraints(self._variables.size(), weights, bias)
