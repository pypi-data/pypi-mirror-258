...
def solve_request():
    return api_model.SolveRequest(
        input=api_model.S3Input(
            ...
            bias_constraints_file_name=None,
            weights_constraints_file_name=None,
...
def temp_storage_raw_response():
    return \
        '{"input": {' \
            ...
            '"weights_constraints.npy": { "upload": "http://weights_constraints-upload.com", "download": "http://weights_constraints-download.com" }, ' \
            '"bias_constraints.npy": { "upload": "http://bias_constraints-upload.com", "download": "http://bias_constraints-download.com" } }, ' \
        '"output": {' \
...
def test_temp_storage(client, baseurl, temp_storage_raw_response):
    ...
    assert temp_storage_response.input.weights_constraints_file.upload == "http://weights_constraints-upload.com"
    assert temp_storage_response.input.weights_constraints_file.download == "http://weights_constraints-download.com"
    assert temp_storage_response.input.bias_constraints_file.upload == "http://bias_constraints-upload.com"
    assert temp_storage_response.input.bias_constraints_file.download == "http://bias_constraints-download.com"
