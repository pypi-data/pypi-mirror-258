...
_BIAS_CONSTRAINTS_FILE = "bias_constraints.npy"
_WEIGHTS_CONSTRAINTS_FILE = "weights_constraints.npy"
...
class S3Storage(StorageClient):
    ...
    def _upload_arrays(self, bias: bytes, weights: bytes, bias_constraints: Optional[bytes], weights_constraints: Optional[bytes]):
        ...
        if bias_constraints:
            upload_tuple.append((self._get_full_filename(_BIAS_CONSTRAINTS_FILE), bias_constraints))

        if weights_constraints:
            upload_tuple.append((self._get_full_filename(_WEIGHTS_CONSTRAINTS_FILE), weights_constraints))
    ...
    def _input(self) -> S3Input:
        bias_constraints = self._get_full_filename(_BIAS_CONSTRAINTS_FILE)
        weights_constraints = self._get_full_filename(_WEIGHTS_CONSTRAINTS_FILE)

        return S3Input(
            weights_file_name=self._get_full_filename(_WEIGHTS_FILE),
            bias_file_name=self._get_full_filename(_BIAS_FILE),
            weights_constraints_file_name=weights_constraints if weights_constraints in self._file_uploaded else None,
            bias_constraints_file_name=bias_constraints if bias_constraints in self._file_uploaded else None,
