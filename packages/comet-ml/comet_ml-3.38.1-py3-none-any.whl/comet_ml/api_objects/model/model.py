...
class Model:
    ...
    def download(
        ...
        remote_assets = is_remote_model(raw_assets)

        LOGGER.debug(
            "Model assets download - is remote assets: %r, direct S3 assets download enabled: %r",
            remote_assets,
    ...
    def _start_remote_asset_download(
        ...
        if parsed_url.scheme not in ["s3", "gs"]:
            raise RemoteModelDownloadException(
                logging_messages.UNSUPPORTED_URI_SYNCED_REMOTE_ASSET % remote_uri
            )
