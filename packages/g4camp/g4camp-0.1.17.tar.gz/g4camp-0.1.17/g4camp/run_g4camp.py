...
def parse_arguments():
    ...
    p.add_argument('--mode_custom_physlist', default='all_phys', help="choose available physics set", choices=['all_phys', 'em_cascade'])
...
def run(app, n_events=10, ofname="output.h5"):
    ...
    for ievt, data in enumerate(app.run(n_events)):
        ...
        log.info(f"   Number of tracks:       {len(np.unique(data.tracks['uid']))}")
