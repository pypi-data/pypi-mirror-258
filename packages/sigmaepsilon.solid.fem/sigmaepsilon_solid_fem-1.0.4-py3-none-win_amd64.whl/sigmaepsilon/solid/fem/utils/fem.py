...
from sigmaepsilon.math.linalg.sparse import csr_matrix as csr, JaggedArray
from sigmaepsilon.math import find1d, flatten2dC
...
@njit(nogil=True, parallel=True, cache=__cache)
def approximation_matrix(ndf: ndarray, NDOFN: int) -> ndarray:
    """
...
@njit(nogil=True, parallel=True, cache=__cache)
def nodal_approximation_matrix(ndf: ndarray) -> ndarray:
    """
...
@njit(nogil=True, parallel=True, cache=__cache)
def topo1d_to_gnum1d(topo1d: ndarray, NDOFN: int) -> ndarray:
    nN = topo1d.shape[0]
...
@njit(nogil=True, parallel=True, cache=__cache)
def ncf_to_cf(ncf: ndarray, NDOFN: int) -> ndarray:
    nN = ncf.shape[0]
...
@njit(nogil=True, parallel=True, cache=__cache)
def element_dofmap_bulk(dofmap: ndarray, nDOF: int, nNODE: int) -> ndarray:
...
@njit(nogil=True, parallel=True, cache=__cache)
def nodal_mass_matrix_data(nodal_masses: ndarray, ndof: int = 6) -> ndarray:
...
@njit(nogil=True, parallel=True, cache=__cache)
def calculate_fictitious_membrane_stiffness_bulk(
    K: ndarray, nNE: int, nDOF: int, alpha: float = 1e-8
) -> None:
    """
    Calculates fictive membrane drilling stiffnesses.
    """
    nE = K.shape[0]
    for iE in prange(nE):
        average_stiffness = np.mean(np.diag(K[iE]))
        fictive_stiffness = average_stiffness * alpha
        for iNE in prange(nNE):
            i = iNE * nDOF + 2
            K[iE, i, i] = fictive_stiffness
