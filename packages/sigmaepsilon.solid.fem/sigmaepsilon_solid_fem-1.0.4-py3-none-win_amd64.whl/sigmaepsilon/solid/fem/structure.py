...
from .nbc import FemLoad
from .ebc import FemEssentialBoundaryCondition as EBC
from .femsolver import StaticSolver, DynamicSolver
from .utils.preproc.preproc import assemble_load_vector
from .utils.tr import tr_element_matrices_bulk, tr_nodal_loads_bulk
from .utils.dyn import effective_modal_masses, Rayleigh_quotient
from .utils.utils import flatten_pd
...
class Structure:
    ...
    def __init__(
        self,
        mesh: FemMesh,
        essential_penalty: Optional[float] = DEFAULT_DIRICHLET_PENALTY,
        mass_penalty_ratio: Optional[float] = DEFAULT_MASS_PENALTY_RATIO,
    ):
    ...
    @mesh.setter
    def mesh(self, value: FemMesh) -> None:
        """
    ...
    @property
    def loads(self) -> FemLoad:
        """
        Returns the loads of the structure. The returned object is essentially
        a dictionary (descendent of DeepDict).
        """
    ...
    def linear_static_analysis(
        self, *args, return_solver: bool = False, **kwargs
    ) -> "Structure":
        """
        Performs a linear elastostatic calculation with pre- and
        post-processing.
        ...
        self._postproc_linstat_(*args, **kwargs)
        if return_solver:
            return self._static_solver_
    ...
    def _initialize_(self, *_, **__) -> "Structure":
        ...
        :func:`preprocess`
        """
        blocks = list(self.mesh.cellblocks(inclusive=True))

        standalone_blocks: Iterable[FemMesh] = filter(lambda b: b.cd.standalone, blocks)
        standalone_block_dofs: Iterable[FemMesh] = list(
            map(lambda b: set(b.cd.Material.displacement_variables), standalone_blocks)
        )

        if (num_standalone := len(standalone_block_dofs)) == blocks:
            raise ValueError(
                "If a mesh contains a standalone mesh, all other meshes must be standalone."
            )

        if num_standalone > 1:
            for dofs in standalone_block_dofs[1:]:
                if not dofs == standalone_block_dofs[0]:
                    raise ValueError("Standalone meshes must be of the same kind.")
    ...
    """def _assemble_linstat_nostore_(self):
        mesh = self.mesh
        jagged = mesh.is_jagged()

        blocks = mesh.cellblocks(inclusive=True)
        for block in blocks:
            K_dense = block.cd.elastic_stiffness_matrix(
                sparse=False, transform=False, recalculate=True
            )
            f_dense = block.cd.load_vector(transform=False, assemble=False, recalculate=True)
            """
    ...
    def _assemble_linstat_(self, *_, **__):
        ...
        # this is necessary because of condensation to work, the quantities
        # must be precalcualted on the virgin material
        ...
        if jagged:
            f_bulk = mesh.cell_load_vector(
                assemble=True, transform=True, recalculate=False
            )
            ...
            f_bulk = mesh.cell_load_vector(
                assemble=False, transform=False, recalculate=False
            )
            K_bulk = mesh.elastic_stiffness_matrix(
                sparse=False, transform=False, _jagged=jagged, recalculate=False
            )
        ...
        if not jagged:
            ...
            # in the solver
            nX = len(mesh.pd) * mesh.number_of_displacement_variables
    ...
    def _assemble_free_vib_(self, *_, **__):
        ...
        mesh.condensate_cell_fixity()
        # get condensated data
        M_sparse = mesh.mass_matrix(recalculate=False)
        K_bulk = mesh.elastic_stiffness_matrix(
            sparse=False, transform=False, recalculate=False
        )
        # transform to global
        dcm = mesh.direction_cosine_matrix(target="global")
        K_bulk = tr_element_matrices_bulk(K_bulk, dcm)
    ...
    def _postproc_linstat_(self, *args, cleanup: bool = False, **kwargs) -> "Structure":
        solver: StaticSolver = self._static_solver_
        mesh = self.mesh
