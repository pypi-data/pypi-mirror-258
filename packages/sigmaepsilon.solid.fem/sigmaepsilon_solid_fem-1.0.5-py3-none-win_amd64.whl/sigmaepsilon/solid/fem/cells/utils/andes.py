...
from sigmaepsilon.mesh.utils.tri import lcoords_tri, shp_tri_loc

__cache = True


def sig_ANDES(
    key: Optional[str] = "OPT", C: Optional[Union[ndarray, None]] = None
) -> ndarray:
    if key == "OPT":
...
def sig_ANDES_opt(C: ndarray) -> ndarray:
...
def sig_ANDES_opt_bulk(C: ndarray) -> ndarray:
    nE = len(C)
...
@njit(nogil=True, cache=__cache)
def _stiffness_data_(ecoords: ndarray, h: float, sig: ndarray) -> ndarray:
...
@njit(nogil=True, cache=__cache)
def _stiffness_matrix_LSTMR(
    C: ndarray, ecoords: ndarray, h: float, sig: ndarray
...
def stiffness_matrix_ANDES(
    C: ndarray, ecoords: ndarray, h: ndarray, sig: ndarray
) -> ndarray:
    nE = len(C)
...
def stiffness_matrix_LSQ_4_12(
    ...
    for i in range(subtopo.shape[0]):
        K_i_bulk = stiffness_matrix_ANDES(C, ecoords[:, subtopo[i], :], h, sig)
...
@njit(nogil=True, parallel=True, cache=__cache)
def strain_displacement_matrix_ANDES(pcoord, ecoords, h, sig):
    nE = ecoords.shape[0]
