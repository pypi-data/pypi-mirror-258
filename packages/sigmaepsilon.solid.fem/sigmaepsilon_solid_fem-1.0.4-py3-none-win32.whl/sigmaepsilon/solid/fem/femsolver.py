...
from .utils.linsolve import solve_standard_form, unbox_lhs
from .utils.imap import index_mappers, box_spmatrix, box_rhs, box_dof_numbering
from .utils.dyn import natural_circular_frequencies
...
class StaticSolver(Solver):
    ...
    def update_stiffness(self, K_bulk: Union[ndarray, JaggedArray]) -> None:
        """
        Updates the stiffness. Currently it is not checked, but the shapes of the new and
        the old array must match.
        """
        self.K_bulk = K_bulk
    ...
    def _preproc_(self, force: bool = False) -> None:
    ...
    def _proc_(self, preproc: bool = False, solver: str = None) -> None:
        if preproc:
            self._preproc_(force=True)
