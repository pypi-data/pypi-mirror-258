...
def namespace_package_name(start_dir: str = None, max_depth: int = 10) -> str:
    ...
    while depth <= max_depth:
        ...
        names.insert(0, current_dir_name)

        if os.path.isdir(current_dir) and parent_dir_name == "sigmaepsilon":
            init_file_path = os.path.join(start_dir, "__init__.py")
            assert os.path.isfile(init_file_path)
            names.insert(0, parent_dir_name)
    ...
    if not package_name:
        raise FileNotFoundError("The SigmaEpsilon namespace was not found")
