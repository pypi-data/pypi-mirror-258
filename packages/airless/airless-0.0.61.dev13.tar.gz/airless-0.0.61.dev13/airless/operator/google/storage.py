...
class BatchWriteDetectAggregateOperator(BaseEventOperator):
    ...
    def execute(self, data, topic):
        ...
        for b in self.gcs_hook.list(bucket, prefix):
            ...
            logging.info(f'Detecting file: {filename} from bucket {bucket} and table {key}')
    ...
    def save_last_timestamp_processed(self, directory, max_timestamp):
        ...
        try:
            ...
            logging.info(f"Deleting from {get_config('GCS_BUCKET_DOCUMENT_DB')} file {self.document_db_folder}/{directory}")
            ...
            logging.info(f"Uploading from memory {get_config('GCS_BUCKET_DOCUMENT_DB')} file {self.document_db_folder}/{directory} name {max_timestamp.strftime('%Y%m%d%H%M%S')}")
    ...
    def verify_table_last_timestamp_processed(self, directory):
        logging.info(f"Verify table last timestamp processed for {self.document_db_folder}/{directory}")
        if directory in self.tables_last_timestamp_processed.keys():
            logging.info('Get timestamp from memory')
            return self.tables_last_timestamp_processed[directory]
        else:
            logging.info(f"Get timestamp from bucket {get_config('GCS_BUCKET_DOCUMENT_DB')} dataset {self.document_db_folder}/{directory}")
            if self.gcs_hook.check_existance(get_config('GCS_BUCKET_DOCUMENT_DB'), f'{self.document_db_folder}/{directory}'):
