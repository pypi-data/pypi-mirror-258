...
def photon_pos_on_PSF(star_pos,photon_dic,PSF,PSF_size,max_func):
    dict_photon = {}
        ...
        for ph in range(len(photon_dic[st])):
            ...
            while stop == 0:
                ...
                if intensity <= PSF(test):
                    stop = 1
                    ph_list.append([(x-0.5)*PSF_size,(y-0.5)*PSF_size,lbd,time])
...
def photon_proj(Photon_dict, star_pos, psf_to_detect,rot_func, alt_ev, size, FOV,lam0):
    ...
    for st in range(len(star_pos)):
        for ph in range(len(Photon_dict[st])):
            # Adding the star position taking account of rotation + ration psf size to dectector size
