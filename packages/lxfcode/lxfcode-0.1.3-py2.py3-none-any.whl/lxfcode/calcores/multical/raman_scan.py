from calinstances.instances import ContiTBG, EffABt
import numpy as np
from typing import NewType, Union
from calcores.filesop.filesave import FilesSave, os
from calcores.raman.raman_plot import RamanPlot, RamanFiles, plt
from calcores.pubmeth.pubmeth import Line, PubMethod
...
class RamanScan:
    ...
    def raman_arr(self, xlim=[10, 20]):
        ...
        Line(self.twists, ri_list, self.scandir).plot(
            "rarr",
            r"$\theta$ ($\degree$)",
            "Raman Intensity (a.u.)",
            xlim=xlim,
        )
        Line([self.twists] * 2, [rireal, riimag], self.scandir).multiplot(
            "rarrrealimag",
        ...
        Line([self.twists] * 3, [ri_list, rireal, riimag], self.scandir).semilogy(
            "rarrlog",
        ...
        Line([self.twists] * 2, [rireal_neg, rireal_pos], self.scandir).multiplot(
            "real_negpos",
        ...
        print(np.max(ri_list) / ri_list[-1])

        return ri_list

    def raman_arr_over_eop(self):
        return
