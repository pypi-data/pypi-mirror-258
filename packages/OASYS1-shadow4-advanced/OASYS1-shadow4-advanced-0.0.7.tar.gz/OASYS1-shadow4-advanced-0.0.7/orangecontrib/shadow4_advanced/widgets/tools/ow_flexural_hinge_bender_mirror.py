...
from shadow4_advanced.benders.s4_flexural_hinge_bender_ellipsoid_mirror import S4FlexuralHingeBenderEllipsoidMirror, S4FlexuralHingeBenderEllipsoidMirrorElement, \
    BenderType, MirrorShape, FlexuralHingeBenderFitParameters, BenderMovement
...
class OWFlexuralHingeBenderMirror(_OWMirror):
    ...
    def populate_tab_bender(self, subtab_bender):
        ...
        self.ratio_box = oasysgui.widgetBox(self.fit_box, "", addSpace=False, orientation="vertical")
        gui.separator(self.fit_box, 10)
        self.e_box = oasysgui.widgetBox(self.fit_box, "", addSpace=False, orientation="vertical")
        gui.separator(self.fit_box, 10)

        add_parameter_box(m1_box, "M1", "M1 [Nm]")
        ...
        self.mov_box_1 = oasysgui.widgetBox(self.tab_mov, "Double Momentum", addSpace=False, orientation="vertical")

        oasysgui.lineEdit(self.mov_box_1, self, "q_upstream",  "Upstream Focus Position [m]", labelWidth=250, valueType=float, orientation="horizontal")
        oasysgui.lineEdit(self.mov_box_1, self, "q_downstream", "Downstream Focus Position [m]", labelWidth=250, valueType=float, orientation="horizontal")

        le = oasysgui.lineEdit(self.mov_box_1, self, "M1_movement", "M1 [Nm]", labelWidth=250, valueType=float, orientation="horizontal")
        le.setEnabled(False)
        le.setStyleSheet("color: blue; background-color: rgb(254, 244, 205); font:bold")
        le = oasysgui.lineEdit(self.mov_box_1, self, "M2_movement", "M2 [Nm]", labelWidth=250, valueType=float, orientation="horizontal")
        le.setEnabled(False)
        le.setStyleSheet("color: blue; background-color: rgb(254, 244, 205); font:bold")

        self.mov_box_2 = oasysgui.widgetBox(self.tab_mov, "Single Momentum", addSpace=False, orientation="vertical")

        oasysgui.lineEdit(self.mov_box_2, self, "q_upstream", "Focus Position [m]", labelWidth=250, valueType=float, orientation="horizontal")

        le = oasysgui.lineEdit(self.mov_box_2, self, "M1_movement", "M1 [Nm]", labelWidth=250, valueType=float, orientation="horizontal")
        le.setEnabled(False)
        le.setStyleSheet("color: blue; background-color: rgb(254, 244, 205); font:bold")
    ...
    def set_bender_type(self):
        self.ratio_box.setVisible(self.bender_type == BenderType.DOUBLE_MOMENTUM)
        self.mov_box_1.setVisible(self.bender_type == BenderType.DOUBLE_MOMENTUM)
        self.mov_box_2.setVisible(self.bender_type == BenderType.SINGLE_MOMENTUM)
    ...
    def _post_trace_operations(self, output_beam, footprint, element, beamline):
        ...
        if self.use_fitted_result_to_move_bender == 0:
            ...
            if self.bender_type==BenderType.DOUBLE_MOMENTUM: self.ratio_out = bender_data.ratio_out
        else:
