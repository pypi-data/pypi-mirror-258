...
class FreeGenius:
    ...
    def runPlugins(self):
        ...
        self.formulateFreeGeniusActionPrompt()

    def formulateFreeGeniusActionPrompt(self):
        actions = ""
        for key, value in config.freeGeniusActions.items():
            actions += f"""\n  * action: '{key}'\n    - description: {value}\n"""
        actions += """\n  * action: 'other_request'\n    - description: choose this action when all the actions listed above are not relevant to the original request.\n"""
        template = {
            "action": "",
        }
        return f"""Assistant, an expert JSON builder.

Use this template:

{template}

To generate a JSON string that contains "action" parameter, based on User's request.

Actions available to Assistant are:

{actions}
          
Respond with JSON string that contains "action" parameter ONLY, without extra comment or information.
...
{userInput}""")
                    ...
                    config.currentMessages.append({"role": "user", "content": f"{self.formulateFreeGeniusActionPrompt()}{fineTunedUserInput}"})
                    ...
                    action = json.loads(completion['message']['content'])["name"]
                    # take action
