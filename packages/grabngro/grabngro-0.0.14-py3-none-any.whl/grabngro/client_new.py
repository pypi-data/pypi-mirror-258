...
class GenericClient:
    ...
    """
    def __init__(self):
        self.API_ENDPOINT = 'http://localhost:8888/' # internal private ip
...
class FuturesClient(GenericClient):
    ...
    def get_by_ids(self, file: str, region_id: int, item_id: int, start_date: str, metric: int = 0) -> Dict[str, Dict]:
        ...
        """
        querystring: str = f"?file={file}&region_id={region_id}&item_id={item_id}&start_date={start_date}&metric={metric}"
    ...
    def get_by_code(self, code: str, metric: str = 'Close') -> Dict[str, Dict]:
        ...
        Example:
            ...
            ```
        Args:
            code (str): A futures contract code, e.g. `CZ24`, `CZ2024`, `CZ4`
            metric (str): The Gro `metric_id` to query as a simplified string i.e. `Close`
    ...
    def get_all_contracts_by_month(self, product_code: str, month_code: str, metric: str = 'Close') -> Dict[str, List]:
        ...
        Args:
            product_code (str): A product code from `get_product_codes`, e.g. `C` or `DCEC`
            month_code (str): A month code from `get_month_codes`, e.g. `Z`
            metric (str): The Gro `metric_id` to query as a simplified string i.e. `Close`
