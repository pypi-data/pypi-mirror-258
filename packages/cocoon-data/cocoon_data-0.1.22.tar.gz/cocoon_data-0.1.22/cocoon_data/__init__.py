...
class ProductSteps(Node):
    ...
    def postprocess(self, run_output, callback, viewer=False):
        ...
        next_button.on_click(lambda x: callback(document))
        display(next_button)
...
class DataSource(Node):
    ...
    def postprocess(self, run_output, callback, viewer=False):
        ...
        self.progress = show_progress(len(files))
        ...
        for file in files:
            if file.endswith(".csv"):
                ...
                html_content += f"<b>{idx}. {table_name}</b>"
                html_content += wrap_in_scrollable_div(truncate_html_td(df.head(2).to_html()))
                idx += 1

            self.progress.value += 1

        print(f"🤓 We found {idx-1} seed tables. Below are their samples ...")
        ...
        next_button.on_click(lambda x: callback(tables))
        display(next_button)
...
```"""
    ...
    def postprocess(self, run_output, callback, viewer=False):
        ...
        print("🤓 Below we group the data source ... ")
        ...
        for table in grouped_tables:
            ...
            if len(group) > 1:
                ...
                print(f"{BOLD}{table}{END}: The source is from {group[0]}")

        print("😎 Next we will write codes to load source ...")
                

        next_button = widgets.Button(description="Next Step", button_style='success')
        next_button.on_click(lambda x: callback(grouped_tables))
        display(next_button)
...
class SourceCodeWritingForAll(MultipleNode):
    ...
    def display_after_finish_workflow(self, callback, document):
        ...
        for key in document["Source Code Writing"]:
            ...
            tab_data.append((key, css_style + highlighted_sql))
...
class BasicStageForAll(MultipleNode):
    ...
    def display_after_finish_workflow(self, callback, document):
        ...
        for key in document["Basic Stage"]:
            ...
            highlighted_sql = wrap_in_scrollable_div(highlight(sql_query, SqlLexer(), formatter))
            tab_data.append((key.replace("src_", "stg_"), css_style + highlighted_sql))
...
```"""
        ...
        deleted_table_name = doc_df.table_name
...
"""
        ...
        display(HTML(wrap_in_iframe(graph_html.replace("{{data}}", json.dumps(data)), width=1000)))
...
def create_data_product_workflow(dbt_directory):
    ...
    main_workflow.add_to_leaf(ProductSteps())
    main_workflow.add_to_leaf(DataSource())
    main_workflow.add_to_leaf(BusinessWorkflow())
    main_workflow.add_to_leaf(GroupDataSource())
    main_workflow.add_to_leaf(SourceCodeWritingForAll())
    main_workflow.add_to_leaf(SourceDocumentForAll())
    main_workflow.add_to_leaf(BasicStageForAll())
    main_workflow.add_to_leaf(DecideCRUDForAll())
    main_workflow.add_to_leaf(PerformCRUDForAll())
    main_workflow.add_to_leaf(DecideForeignKeyForAll())
    main_workflow.add_to_leaf(PerformNormalizationForAll())
    main_workflow.add_to_leaf(ClassifyDimFactForAll())
    main_workflow.add_to_leaf(CreateFactTableForAll())
    main_workflow.add_to_leaf(CreateDimensionTableForAll())
    main_workflow.add_to_leaf(ProductDocumentForAll())
    main_workflow.add_to_leaf(CreateFactMatchDimForAll())
    main_workflow.add_to_leaf(MatchDimensions())
    main_workflow.add_to_leaf(BuildDataMartForAll())
