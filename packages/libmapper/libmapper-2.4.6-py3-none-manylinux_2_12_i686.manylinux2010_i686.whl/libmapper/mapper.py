...
class Object:
    ...
    class Status(Enum):
        ...
        def __repr__(self):
            return 'libmapper.Object.Status.' + self.name
...
class Signal(Object):
    ...
    def num_instances(self, status=Object.Status.ANY):
        ...
        Args:
            status (libmapper.Object.Status): The status of the instances to search for. Should be
                set to `ACTIVE`, `RESERVED`, or both (`ACTIVE | RESERVED`).
        ...
        if not isinstance(status, Object.Status):
            status = Object.Status(status)
        return mpr.mpr_sig_get_num_inst(self._obj, status.value)

    def instance_id(self, idx, status=Object.Status.ANY):
        """
