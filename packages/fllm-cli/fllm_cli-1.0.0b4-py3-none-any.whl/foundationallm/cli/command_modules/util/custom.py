...
def rest_call(cmd, url, method=None, headers=None, uri_parameters=None,
    ...
    from foundationallm.cli.core.commands.transform import unregister_global_transforms
    # No transform should be performed on `az rest`.
    unregister_global_transforms(cmd.cli_ctx)

    from foundationallm.cli.core.util import send_raw_request
...
def show_version(cmd):  # pylint: disable=unused-argument
    from foundationallm.cli.core.util import get_az_version_json
...
def upgrade_version(cmd, update_all=None, yes=None, allow_preview=None):  # pylint: disable=too-many-locals, too-many-statements, too-many-branches, no-member, unused-argument
    ...
    from foundationallm.cli.core import __version__ as local_version
    from foundationallm.cli.core._environment import _ENV_AZ_INSTALLER
    from foundationallm.cli.core.extension import get_extensions, WheelExtension
    from packaging.version import parse

    update_cli = True
    from foundationallm.cli.core.util import get_latest_from_github
    ...
    if update_cli:
        ...
        logger.warning("Your current FLLM CLI version is %s. %s", local_version, latest_version_msg)
        ...
        if not yes:
            ...
            try:
                ...
                from foundationallm.cli.core.azclierror import UnclassifiedUserFault
        ...
        if installer == 'DEB':
            ...
            if in_cloud_console():
                raise CLIError("fllm upgrade is not supported in Cloud Shell.")
            ...
            if exit_code == 0:
                logger.debug("Update fllm cli with '%s'", " ".join(az_update_cmd))
            ...
            if not distname:
                ...
                if any(x in distname for x in ['centos', 'rhel', 'red hat', 'fedora']):
                    ...
                    logger.debug("Update fllm cli with '%s'", " ".join(update_cmd))
            ...
            logger.debug("Update fllm cli with '%s'", " ".join(pip_args))
            ...
            logger.warning("Exit the container to pull latest image with 'docker pull mcr.microsoft.com/azure-cli' "
                           "or run 'pip install --upgrade fllm-cli' in this container")
        elif installer == 'MSI':
            _upgrade_on_windows()
        else:
            logger.warning(UPGRADE_MSG)
    ...
    version_result = subprocess.check_output(['fllm', 'version', '-o', 'json'], shell=platform.system() == 'Windows')
    version_json = json.loads(version_result)
    new_version = version_json['fllm-cli-core']
    ...
    for ext_name in exts:
        try:
            ...
            cmds = ['fllm', 'extension', 'update', '-n', ext_name]
            if allow_preview:
            ...
            msg = "Extension {} update failed during fllm upgrade. {}".format(ext_name, str(ex))
            raise CLIError(msg)
    auto_upgrade_msg = "You can enable auto-upgrade with 'fllm config set auto-upgrade.enable=yes'. " \
...
def _upgrade_on_windows():
    ...
    from foundationallm.cli.core.util import rmtree_with_retry

    if platform.architecture()[0] == '32bit':
        msi_url = 'https://aka.ms/installazurecliwindows'
    else:
        msi_url = 'https://aka.ms/installazurecliwindowsx64'
    logger.warning("Updating FLLM CLI with MSI from %s", msi_url)
    ...
    logger.warning("Installation started. Please complete the upgrade in the opened window.\nTo update extensions, "
                   "please run `fllm upgrade` again after completing the upgrade.")
    sys.exit(0)
...
def demo_style(cmd, theme=None):  # pylint: disable=unused-argument
    from foundationallm.cli.core.style import Style, print_styled_text, format_styled_text
    if theme:
...
def secret_store_save(cmd, key_value):
    ...
    from foundationallm.cli.core.util import get_secret_store
