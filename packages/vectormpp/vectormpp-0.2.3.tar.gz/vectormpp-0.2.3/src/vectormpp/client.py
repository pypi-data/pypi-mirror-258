...
class Kubectl(Command):
    ...
    def wait_for_desire(self, beacon):
        ...
        while time.time() - start_time < timeout:
            ...
            if code == 0:
                state = out.strip("'")
                if state in beacon["desire"]:
                    logger.info(f"{' '.join(cmd)}: achieve desired: {state}.")
                    archived = True
                    break
                logger.info(f"{' '.join(cmd)}: Current state: '{state}', continue waiting for desire state {'|'.join(beacon['desire'])}...")
            else:
            ...
            time.sleep(20)
        if not archived:
            raise Exception(f"Timeout: {' '.join(cmd)}: not achieve desired: {'|'.join(beacon['desire'])}.")
    ...
    def should_install_or_upgrade(self, yamlFiles):
        ...
        for f in yamls:
            ...
            if code == 0:
                ...
                logger.debug(f"{' '.join(cmd)}: Changed.")
                yamls[f] += 1
            else:
                raise KubectlError(cmd, out, err)
