...
from ...models.post_profile_merge_response import PostProfileMergeResponse
from ...models.profile_merge_query import ProfileMergeQuery
...
) -> Optional[Union[Any, PostProfileMergeResponse]]:
    if response.status_code == HTTPStatus.CREATED:
        response_201 = PostProfileMergeResponse.from_dict(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Any, PostProfileMergeResponse]]:
    return Response(
...
def sync_detailed(
    *,
    ...
    The profile provided under `relationships` (the \"source\" profile) will be merged into the profile
    provided by the ID in the base data object (the \"destination\" profile).
    This endpoint queues an asynchronous task which will merge data from the source profile into the
    destination profile, deleting the source profile in the process. This endpoint accepts only one
    source profile.

    To learn more about how profile data is preserved or overwritten during a merge, please [visit our
    Help Center](https://help.klaviyo.com/hc/en-
    us/articles/115005073847#merge-2-profiles3).<br><br>*Rate limits*:<br>Burst: `10/s`<br>Steady:
    `150/m`
...
def sync(
    ...
    **Scopes:**
    ...
    Returns:
        Union[Any, PostProfileMergeResponse]
    """
