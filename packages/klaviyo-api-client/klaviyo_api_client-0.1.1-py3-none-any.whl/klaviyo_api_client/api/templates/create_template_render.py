...
from ...models.post_template_response import PostTemplateResponse
from ...models.template_render_query import TemplateRenderQuery
...
) -> Optional[Union[Any, PostTemplateResponse]]:
    if response.status_code == HTTPStatus.CREATED:
        response_201 = PostTemplateResponse.from_dict(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Any, PostTemplateResponse]]:
    return Response(
...
def sync_detailed(
    *,
    ...
    r"""Create Template Render

     Render a template with the given template ID and context attribute. Returns the HTML and plain text
    versions of the email template.
    ...
    * `context`: This is the context your email template will be rendered with. You must pass in a
    `context` object as a JSON object.

    Email templates are rendered with contexts in a similar manner to Django templates. Nested template
    variables can be referenced via dot notation. Template variables without corresponding `context`
    values are treated as `FALSE` and output nothing.

    Ex. `{ \"name\" : \"George Washington\", \"state\" : \"VA\" }`<br><br>*Rate limits*:<br>Burst:
    `3/s`<br>Steady: `60/m`
...
def sync(
    ...
    **Scopes:**
    ...
    Returns:
        Union[Any, PostTemplateResponse]
    """
