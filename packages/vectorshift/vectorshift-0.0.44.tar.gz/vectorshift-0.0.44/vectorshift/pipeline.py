    ...
    def from_json_rep(json_data:dict[str, any]) -> 'Pipeline':
        ...
        for edge in json_data.get('edges', []):
            ...
            if target_input_name not in target_node._inputs.keys():
                # while we can just modify _inputs, this should not happen. The
                # only exception is if the node's input arg could have been 
                # initialized to be something other than a NodeOutput (e.g. 
                # system and prompt inputs for LLM nodes)
