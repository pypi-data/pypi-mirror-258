...
class IntegrationNode(NodeTemplate):
    ...
    def setup_integration_action(self, inputs, **kwargs):
        ...
        if self.integration_type == 'Airtable' and self.action == 'read_tables':
            ...
            if not ('airtable_tables' in kwargs):
                raise ValueError('IntegrationNode: missing airtable_tables input in Airtable integration. Expecting a list of dictionaries including base_id, base_name, and table_id keys.')
            for t in kwargs['airtable_tables']:
                if 'base_id' not in t or 'base_name' not in t or 'table_id' not in t:
                    raise ValueError('IntegrationNode: missing base_id, base_name, or table_id keys in airtable_tables input in Airtable integration.') 
                self.integration_specific_params['selectedTables'] = kwargs['airtable_tables']
            ...
            if not ('base_id' in kwargs) or not ('table_id' in kwargs) or not ('table_fields' in kwargs):
                raise ValueError('IntegrationNode: missing base_id, table_id, or table_fields inputs in Airtable integration.')
            self.integration_specific_params['base_id'] = kwargs['base_id']
            self.integration_specific_params['table_id'] = kwargs['table_id']
            self.integration_specific_params['input_names'] = kwargs['table_fields']
            self.input_names = handle_dynamic_inputs(kwargs['table_fields'])
        # Notion has a dynamic list of inputs
            ...
            if not 'database_id' in kwargs or not 'database_fields' in kwargs:
                raise ValueError('IntegrationNode: missing database_id or database_fields inputs in Notion integration.')
            ...
            self.input_names = handle_dynamic_inputs(kwargs['database_fields'])
    ...
    def _from_json_rep(json_data:dict) -> 'IntegrationNode':
        ...
        if not input_names: input_names = []
        integration_json = json_data.get('integration', {})
        return IntegrationNode(
            integration_type=json_data['integrationType'],
