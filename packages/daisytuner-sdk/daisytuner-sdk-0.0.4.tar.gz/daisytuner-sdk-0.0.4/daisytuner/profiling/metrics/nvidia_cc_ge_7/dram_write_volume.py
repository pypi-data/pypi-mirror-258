...
class DramWriteVolume(Metric):
    ...
    def compute(self) -> float:
        ...
        for state in self._sdfg.states():
            if state not in counters["DRAM_BYTES_WRITE_SUM"]:
                continue

            volume += sum(
                [
                    measurements[0]
                    for thread_id, measurements in counters["DRAM_BYTES_WRITE_SUM"][
                        state
                    ].items()
                ]
            )
