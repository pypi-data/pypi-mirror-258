    ...
    def __setitem__(self, key, val):
        ...
        if isinstance(self.obj, Series):
            if (
                ...
                or irutils._is_numeric_index_like(key)
            ) and irutils.irable_scalar(val):
                checked = True
                index_or_mask = key if isinstance(key, Series) else Series(key)
            ...
            if isinstance(key, tuple) and len(key) == 2:
                ...
                if (
                    (
                        isinstance(r, Series)
                        # or irutils._is_index_like(r) # TODO: support
                        or irutils._is_numeric_index_like(r)
                    )
                    and isinstance(c, str)
                    and irutils.irable_scalar(val)  # TODO: support other cases
                ):
                    checked = True
                    index_or_mask = r if isinstance(r, Series) else Series(r)
                    value = irutils.make_scalar(val)
        ...
        if checked:
            res_value = ir.loc_setter_with_scalar(
                self.obj._value, index_or_mask._value, value, cname, is_series
            )
