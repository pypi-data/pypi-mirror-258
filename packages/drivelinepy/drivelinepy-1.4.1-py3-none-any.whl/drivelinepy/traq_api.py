...
class TRAQAPI(BaseAPIWrapper):
    ...
    def get_users(self, traq_id=None, email=None, status_code=None):
        """
        ...
        if traq_id:
            ...
            if self.validate_traq_user_status_code(status_code):
                ...
                return None  # Invalid status_code - logging done in validate_status_code
        else:
    ...
    def validate_traq_user_status_code(self, status_code):
        """
        Validates that the TRAQ User's status (0 - Inactive; 1 - On-Site; 2 - Remote; 3 - Active; 4 - Archived).

        :param status_code: The TRAQ status_code number to be validated.
        :return: Boolean indicating whether the program is valid.
        """
        if isinstance(status_code, int) and 0 <= status_code <= 4:
            self.logger.info(f"TRAQ status_code int is valid: {status_code}. Exiting validate_traq_user_status_code()")
            return True
        else:
            self.logger.error(f"Invalid TRAQ status_code int: {status_code}. Possible options: 0, 1, 2, 3, 4.")
            return False
