...
def does_secrets_baseline_exist(repository_path):
    """
    ...
    return os.path.isfile(f'{repository_path}/.secrets.baseline')
...
def escape_special_characters(pattern):
    """
    Escapes special characters in a pattern that might be misinterpreted by the shell.
...
def update_secrets_scan(repository_path, exclude_files_pattern=None, exclude_lines_pattern=None):
    ...
    if exclude_files_pattern:
        escaped_exclude_files_pattern = escape_special_characters(exclude_files_pattern)
        command += f' --exclude-files "{escaped_exclude_files_pattern}"'

    if exclude_lines_pattern:
        escaped_exclude_lines_pattern = escape_special_characters(exclude_lines_pattern)
        command += f' --exclude-lines "{escaped_exclude_lines_pattern}"'
