...
def ask_git(prmpt="Setup git configuration to copy objects between? (Tabcompletion - Spaces for multiple - Empty: all): "):
    res = "all"
    repo = git.Repo("./")
    names = []
    for name, value in envs.items():
    ...
    res = prompt(prmpt, completer=WordCompleter(names))
    with repo.config_writer() as confw:
...
def main():
    ...
    if args.command == 'help':
        ...
        if envs == {}:
            ...
            if is_git_repo("./"):
                ...
                try:
                    ...
                    res = ask_git("No name given but in a git repository. Setup git configuration to copy objects between? (Tabcompletion - Spaces for multiple - Empty: all): ")
                    cpt_run(res.split())
            else:
        ...
        prompt = "Setup git configuration to copy objects between? (Tabcompletion - Spaces for multiple - Empty: all): "
        if not args.name:
            ask_git(prompt)
        else:
           repo = git.Repo("./")
           ...
           for name, value in envs.items():
                if not name == 'group':
                    names.append(str(name))
           res = prompt(prompt, completer=WordCompleter(names))
           with repo.config_writer() as confw:
                confw.set_value("copy-to", "run", res)
           print("Added " + str(res) + " to local git configuration")
