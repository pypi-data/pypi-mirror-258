...
from quartet_integrations.serialbox_integrations.parsing import UpdateResponseRuleParser
...
class ListToBase36ConversionStep(Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ret = [self.base36encode(int(datum)) for datum in data]
...
class ListToUrnConversionStep(Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        self.info(
            "Looking up the company prefix by using the pool "
        ...
        try:
            ...
            logger.debug(
                "Invalid barcode, this may be an indicator /"
        ...
        if len(pool) == 14:
            converter = self.handle_gtins(
                cp_length, data, return_vals, pool, rule_context
            )
            ...
            rule_context.context["saleable_unit_flag"] = (
                1 if converter.indicator_digit == "0" else 0
            )
        else:
    ...
    def handle_gtins(self, cp_length, numbers, return_vals, pool, rule_context):
        ...
        :param rule_context: The rule context to place the Trade Item django
            model instance onto.
        """
        rule_context.context["trade_item"] = TradeItem.objects.get(GTIN14=pool)
        ...
        converter = BarcodeConverter("01%s21%s" % (pool, "000000000001"), cp_length)
    ...
    def handle_ssccs(self, cp_length, numbers, return_vals, sb_response):
        ...
        try:
            ...
            logger.info(
                "Data was not sent as a barcode...trying "
        ...
        for number in numbers:
            return_vals.append(
                self.format_sscc_urn(
                    company_prefix, extension_digit, number, serial_length
                )
            )
        if not converter:
    ...
    def format_sscc_urn(self, company_prefix, extension_digit, number, serial_length):
        """
    ...
    def foramt_sscc_urn(self, company_prefix: str, extension: str, serial_number: str):
        pass
...
class RequestLoggingStep(Step):
    ...
    def __init__(self, db_task: models.Task, **kwargs):
        ...
        self.log_to_file = self.get_or_create_parameter(
            "Log To File", "True", "Whether or not to log task data to the log file."
        )
...
class ListToBarcodeConversionStep(Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        task_params = self.get_task_parameters(rule_context)
        pool = task_params["pool"]
        self.company_prefix = task_params.get("company_prefix", self.company_prefix)
        self.extension_digit = task_params.get("extension_digit", self.extension_digit)
        self.info("Working against Pool with machine name %s", pool)
    ...
    def handle_gtins(self, numbers, return_vals, pool, rule_context):
        """
        Override this function to look up trade item information, and to
        handle inbound GTIN requests.  This function adds the TradeItem matching
        the pool.machine_name to the context for downstream Steps (such as
        ...
        for number in numbers:
            return_vals.append(self.format_gtin_barcode(pool, number, format))

    def format_gtin_barcode(self, pool: str, serial_number: int, format: str) -> str:
        """
    ...
    def handle_ssccs(
        self, data: list, return_vals: list, pool: str, rule_context: RuleContext
    ):
        """
        ...
        """
        sequential = (
            Pool.objects.prefetch_related("sequentialregion_set")
            .filter(machine_name=pool)
            .count()
            == 1
        )
        ...
        for number in data:
            return_vals.append(self.format_sscc_barcode(number, padding))
    ...
    def format_sscc_barcode(self, number: int, padding: int) -> str:
        ...
        if self.use_parenthesis:
            ...
            sscc_val = "%s%s%s" % (self.extension_digit, self.company_prefix, number)
...
class ResponseRulesUpdateStep(Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        if parser.errors:
            self.error(
                "Updating Response Rules for the following "
