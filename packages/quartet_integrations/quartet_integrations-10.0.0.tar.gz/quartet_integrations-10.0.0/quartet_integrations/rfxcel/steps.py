...
class NumberResponseStep(Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        try:
            ...
            self.error(
                "The rfXcel system failed to return a list of numbers." " Response:",
...
class DBResponseStep(NumberResponseStep):
    ...
    def write_range(self, end, region, sb_start, start):
        raise NotImplementedError(
            "This number response step does not handle " "range based information."
        )
    ...
    def write_list(self, number_elements, region: ListBasedRegion):
        ...
        for id in number_elements:
            cursor.execute(
                "insert into %s (serial_number, used) values "
                "(?, ?)" % region.machine_name,
                (id, 0),
            )
...
class RFXCELNumberResponseParserStep(Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        try:
            ...
            self.info(
                "Error while processing response: %s",
                rule_context.context["NUMBER_RESPONSE"],
            )
            raise
...
class SerialboxSSCCConversionStep(Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        is_sequential = (
            Pool.objects.get(machine_name=machine_name).sequentialregion_set.count()
            != 0
        )
    ...
    def convert_numbers_to_urn(self, data: list, machine_name: str):
        ...
        return [
            "urn:epc:id:sscc:%s.%s%s" % (company_prefix, extension_digit, d)
            for d in data
        ]
    ...
    def declared_parameters(self):
        return {
            "Create URNs": "If set to true URNs will be created, if false "
            'the "packaging line" format will be used'
        }
