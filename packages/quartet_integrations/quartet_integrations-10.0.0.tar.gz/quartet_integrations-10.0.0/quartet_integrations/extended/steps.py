...
from quartet_output.steps import ContextKeys, DynamicTemplateMixin, EPCPyYesOutputStep
...
class TemplateOutputStep(DynamicTemplateMixin, EPCPyYesOutputStep):
    ...
    def declared_parameters(self):
        ...
        params["Template"] = "The name of the template to use to render the " "output."
    ...
    def get_epcis_document_class(
        ...
        template = self.get_or_create_parameter(
            "Template", "Please configure...", "The template to render."
        )
...
class AppendShippingStep(rules.Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        if isinstance(data, File):
            parser = ExtendedParser(data, reg_ex=self._regEx)

        elif isinstance(data, str):
            parser = ExtendedParser(io.BytesIO(str.encode(data)), reg_ex=self._regEx)

        else:
            parser = ExtendedParser(io.BytesIO(data), reg_ex=self._regEx)
        # parse
        parser.parse()
        ...
        epcis_document = template_events.EPCISEventListDocument(
            all_events,
            None,
            template=env.get_template("extended/ext_epcis_document.xml"),
