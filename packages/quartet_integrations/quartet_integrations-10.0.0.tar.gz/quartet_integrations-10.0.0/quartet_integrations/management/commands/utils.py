...
RULE_NAME = "SerialBox URN Response"
...
def create_serialbox_gtin_response_rule():
    ...
    conversion_step, created = Step.objects.get_or_create(
        ...
        step_class="quartet_integrations.serialbox_integrations.steps.ListToUrnConversionStep",
        order=1,
    )
    if not created:
        conversion_step.description = (
            "Convert the list of numbers to " "GTINs or SSCCs."
        )
    return rule
...
def create_gtin_response_rule():
    ...
    if not created:
        conversion_step.description = (
            "Convert the list of numbers to " "GTINs or SSCCs for use by OPSM.",
        )
    ...
    StepParameter.objects.get_or_create(
        step=format_step, name="Template Name", value="OPSM GTIN Response Template"
    )
...
def create_SSCC_response_rule():
    ...
    conversion_step, created = Step.objects.get_or_create(
        rule=rule,
        name="List Conversion",
        step_class="quartet_integrations.serialbox_integrations.steps.ListToBarcodeConversionStep",
        order=1,
    )
    if not created:
        conversion_step.description = (
            "Convert the list of numbers to " "SSCCs for use by OPSM.",
        )
    ...
    StepParameter.objects.get_or_create(
        step=format_step, name="Template Name", value="OPSM SSCC Response Template"
    )

    create_sscc_template()
    pool = Pool.objects.get(machine_name="031300000770000001-SSCC")
    return ResponseRule.objects.get_or_create(rule=rule, pool=pool, content_type="xml")[
        0
    ]
...
def create_template():
    ...
    file_path = os.path.join(curpath, "../../templates/opsm/sgtin_response.xml")
    with open(file_path, "r") as f:
        response_template = Template.objects.get_or_create(
            name="OPSM GTIN Response Template", content=f.read()
        )
...
def create_sscc_template():
    ...
    curpath = os.path.dirname(__file__)
    file_path = os.path.join(curpath, "../../templates/opsm/sscc_response.xml")
    with open(file_path, "r") as f:
        response_template = Template.objects.get_or_create(
            name="OPSM SSCC Response Template", content=f.read()
        )
...
def create_external_GTIN_response_rule():
    ...
    if not created:
        conversion_step.description = (
            "Convert the list of numbers to " "GTIN URNs or SSCCs for use by OPSM.",
        )
...
def create_output_filter_rule(rule_name="GS1USHC Output Filter", delay_rule=False):
    create_criteria()
