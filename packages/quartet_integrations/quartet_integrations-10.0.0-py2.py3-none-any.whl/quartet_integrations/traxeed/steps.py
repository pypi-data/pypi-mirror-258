...
class ProcessTraxeedStep(rules.Step):
    def __init__(self, db_task: models.Task, **kwargs):
        ...
        self._regEx = self.get_or_create_parameter(
            "Quantity RegEx",
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        if isinstance(data, File):
            parser = TraxeedParser(data, reg_ex=self._regEx)

        elif isinstance(data, str):
            parser = TraxeedParser(io.BytesIO(str.encode(data)), reg_ex=self._regEx)

        else:
            parser = TraxeedParser(io.BytesIO(data), reg_ex=self._regEx)
        # parse
        parser.parse()

        rule_context.context[
            ContextKeys.OBJECT_EVENTS_KEY.value
        ] = parser._object_events
        rule_context.context[
            ContextKeys.AGGREGATION_EVENTS_KEY.value
        ] = parser._aggregation_events
        ...
        epcis_document = template_events.EPCISEventListDocument(
            all_events,
            None,
            template=env.get_template("traxeed/tx_hk_epcis_document.xml"),
...
class TraxeedRfxcel(rules.Step):
    def __init__(self, db_task: models.Task, **kwargs):
        ...
        self._source_op = self.get_or_create_parameter(
            "Source Owning Party", "", "The SGLN URN of the Source Owning Party"
        )
        self._source_location = self.get_or_create_parameter(
            "Source Location", "", "The SGLN URN of the Source Location"
        )
        self._destination_op = self.get_or_create_parameter(
            "Destination Owning Party",
            "",
            "The SGLN URN of the Destination Owning Party",
        )
        self._destination_location = self.get_or_create_parameter(
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        # Parse EPCIS with the ExtendedParser
        if isinstance(data, File):
            parser = TraxeedRfxcelParser(data, reg_ex=self._regEx)

        elif isinstance(data, str):
            parser = TraxeedRfxcelParser(
                io.BytesIO(str.encode(data)), reg_ex=self._regEx
            )

        else:
            parser = TraxeedRfxcelParser(io.BytesIO(data), reg_ex=self._regEx)
        # parse
        parser.parse()
        ...
        biz_trans = BusinessTransaction(
            biz_transaction=parser.PO, type="urn:epcglobal:cbv:btt:po"
        )

        source_op = Source(
            source=self._source_op, source_type="urn:epcglobal:cbv:sdt:owning_party"
        )
        source_location = Source(
            source=self._source_location, source_type="urn:epcglobal:cbv:sdt:location"
        )
        destination_op = Destination(
            destination_type="urn:epcglobal:cbv:sdt:owning_party",
            destination=self._destination_op,
        )
        destination_location = Destination(
            destination_type="urn:epcglobal:cbv:sdt:location",
            destination=self._destination_location,
        )
        ...
        epcis_document = template_events.EPCISEventListDocument(
            all_events,
            None,
            template=env.get_template("traxeed/tx_rfxcel_epcis_document.xml"),
            additional_context=additional_context,
        )
        if self.get_boolean_parameter("JSON", False):
            data = epcis_document.render_json()
...
class TraxeedIRIS(rules.Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        # Parse EPCIS with the ExtendedParser
        if isinstance(data, File):
            parser = TraxeedIRISParser(data, reg_ex=self._regEx)

        elif isinstance(data, str):
            parser = TraxeedIRISParser(io.BytesIO(str.encode(data)), reg_ex=self._regEx)

        else:
            parser = TraxeedIRISParser(io.BytesIO(data), reg_ex=self._regEx)
        # parse
        parser.parse()
        # get the first aggregation event for the record/event times
        agg_event = parser._aggregation_events[0]
        ...
        epcis_document = template_events.EPCISEventListDocument(
            all_events,
            None,
            template=env.get_template("traxeed/tx_seton_epcis_document.xml"),
            additional_context=additional_context,
        )
        if self.get_boolean_parameter("JSON", False):
            data = epcis_document.render_json()
...
class TraxeedCIVICA(rules.Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        # Parse EPCIS with the ExtendedParser
        if isinstance(data, File):
            parser = TraxeedCIVICAParser(data, reg_ex=self._regEx)

        elif isinstance(data, str):
            parser = TraxeedCIVICAParser(
                io.BytesIO(str.encode(data)), reg_ex=self._regEx
            )

        else:
            parser = TraxeedCIVICAParser(io.BytesIO(data), reg_ex=self._regEx)
        # parse
        parser.parse()
        # get the first aggregation event for the record/event times
        agg_event = parser._aggregation_events[0]
        ...
        epcis_document = template_events.EPCISEventListDocument(
            all_events,
            None,
            template=env.get_template("traxeed/tx_civica_epcis_document.xml"),
            additional_context=additional_context,
        )
        if self.get_boolean_parameter("JSON", False):
            data = epcis_document.render_json()
