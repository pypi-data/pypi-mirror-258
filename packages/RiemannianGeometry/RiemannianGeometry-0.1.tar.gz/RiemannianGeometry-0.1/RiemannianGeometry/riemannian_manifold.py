...
class RiemannianManifold:
    ...
    def get_geodesic_numerically(self, start, end, step=0.01, params={}):
        """
    ...
    def parallel_transport(self, curve, tau, interval, V0, step=0.01, params={}):
        substitutions = {self.coords[i]: curve[i] for i in range(self.dimensions)}
        if not self.Christoffels:
            self.get_christoffels()
        Christoffels_subs = {self.coords[i]: self.Christoffels[self.coords[i]].subs(substitutions) for i in range(self.dimensions)}
        V_subs = sp.Matrix([sp.Function(f'V_{i}')(tau) for i in range(self.dimensions)])
        d_curve = curve.diff(tau)
        # Get rhs of parallel transport
        rhs = []
        for mu in range(self.dimensions):
            tmp = 0
            for alpha in range(self.dimensions):
                for beta in range(self.dimensions):
                    tmp += -d_curve[alpha]*Christoffels_subs[self.coords[mu]][alpha, beta]*V_subs[beta]
            rhs.append(tmp)
        # Solve the system of ODEs numerically
        V = sp.Matrix([sp.Symbol(f'V_{i}') for i in range(self.dimensions)])
        substitutions = {sp.Function(f'V_{i}')(tau): V[i] for i in range(self.dimensions)}
        rhs = [rhs[i].subs(substitutions) for i in range(self.dimensions)]
        func = sp.lambdify([tau, V], [rhs_.rewrite(sp.tan) for rhs_ in rhs], 'numpy')
        def dy_dt(y, t):
            out = func(t, y)
            return out
        interval = np.arange(interval[0], interval[1], step)
        solution = odeint(dy_dt, V0, interval)
        return solution[-1, :]
...
if __name__ == '__main__':
    ...
    interval = [0.01, np.pi/2] 
    solution = S2.parallel_transport(curve, tau, interval, [1, 0], params={r: 1})
    
    # Part 2
    curve = sp.Matrix([sp.pi/2, tau])
    interval = [0, np.pi/2]
    solution = S2.parallel_transport(curve, tau, interval, solution, params={r: 1})

    # Part 3
    curve = sp.Matrix([sp.pi/2-tau, np.pi/2])
    interval = [0, np.pi/2-0.01]
    solution = S2.parallel_transport(curve, tau, interval, solution, params={r: 1})
