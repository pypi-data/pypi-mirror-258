...
from peelee.color import ColorName, bg, hex2hls, hex2rgb, hls2hex, rgb2hex
...
def get_scheme_colors(hex_color, n_colors=7):
    ...
    base_colors = [hls2hex(hls_color) for hls_color in triadic_colors][
        0:n_colors
    ]
...
def generate_random_colors(
    ...
    for _base_color in base_colors:
        slice_colors = get_slice_colors(
            _base_color, color_gradations, color_slice_type
        )
...
class Palette:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            self.colors_gradations > 0
        ), "colors_gradations must be greater than 0."
        ...
        self.colors_hue = kwargs.get(
            "colors_hue", random.randint(0, 360) / 360
        )
        ...
        assert (
            self.dark_colors_total > 0
        ), "dark_colors_total must be greater than 0."
        assert (
        ...
        if not any(
            member.name == self.dark_base_color_name for member in ColorName
        ):
