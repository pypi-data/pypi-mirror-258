        ...
        if action["init_side"]:
            ...
            if xo!=x or yo!=y:
                ...
                if (xo,yo) in graveyard:
                    new_uid = f"{graveyard[(xo,yo)].side:01d}X{unit_ids[graveyard[(xo,yo)].side]:02d}"
                    unit_list[(xo,yo)] = Unit(uid=new_uid, unit_def=unit_db["Walking Corpse"], side=graveyard[(xo,yo)].side)
                    unit_ids[graveyard[(xo,yo)].side] += 1 

                    for i in range(len(data)-1,-1,-1):
                        cur_action = data[i]
                        if cur_action["action"]=="attack" and graveyard[(xo,yo)].uid in cur_action["combat_ids"]:

                            resurrection = {
                                "turn":cur_action["turn"]
                                ,"side":graveyard[(xo,yo)].side
                                ,"action":"resurrect"
                                ,"uid":new_uid
                                ,"unit":"Walking Corpse"
                                ,"x":xo
...
#             combats += 1
            ...
            if attacker_coord in graveyard:
                new_uid = f"{graveyard[attacker_coord].side:01d}X{unit_ids[graveyard[attacker_coord].side]:02d}"
                unit_list[attacker_coord] = Unit(uid=new_uid, unit_def=unit_db["Walking Corpse"], side=graveyard[attacker_coord].side)
                unit_ids[graveyard[attacker_coord].side] += 1 
                
                for i in range(len(data)-1,-1,-1):
                    cur_action = data[i]
                    if cur_action["action"]=="attack" and graveyard[attacker_coord].uid in cur_action["combat_ids"]:
                        
                        resurrection = {
                            "turn":cur_action["turn"]
                            ,"side":graveyard[attacker_coord].side
                            ,"action":"resurrect"
                            ,"uid":new_uid
                            ,"unit":"Walking Corpse"
                            ,"x":attacker_x
            ...
            if defender_coord in graveyard:
                new_uid = f"{graveyard[defender_coord].side:01d}X{unit_ids[graveyard[defender_coord].side]:02d}"
                unit_list[defender_coord] = Unit(uid=new_uid, unit_def=unit_db["Walking Corpse"], side=graveyard[defender_coord].side)
                unit_ids[graveyard[defender_coord].side] += 1 
                
                for i in range(len(data)-1,-1,-1):
                    cur_action = data[i]
                    if cur_action["action"]=="attack" and graveyard[defender_coord].uid in cur_action["combat_ids"]:
                        
                        resurrection = {
                            "turn":cur_action["turn"]
                            ,"side":graveyard[defender_coord].side
                            ,"action":"resurrect"
                            ,"uid":new_uid
                            ,"unit":"Walking Corpse"
                            ,"x":defender_x
...
#                                     print(turn,attacker,defender,position)
                                ...
                                if position=="attacker":
                                    ...
                                    if unit_list[defender_coord].race!="Undead" and attacker_attack.plague:
                                        graveyard[defender_coord] = unit_list[defender_coord]
                                        graveyard[defender_coord].side = unit_list[attacker_coord].side
                                    ...
                                    if unit_list[attacker_coord].race!="Undead" and defender_attack.plague:
                                        graveyard[attacker_coord] = unit_list[attacker_coord]
                                        graveyard[attacker_coord].side = unit_list[defender_coord].side
                    ...
                    data.append({"turn":turn,"side":side,"action":"attack","combat_string":combat_string,"combat_ids":[a.uid,d.uid],"xo":attacker_x,"yo":attacker_y,"x":defender_x,"y":defender_y})
