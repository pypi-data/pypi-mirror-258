...
class TorTree:
    ...
    #             return 0
    #     def __sortTorsionMap(node):
    #         return (node.atoms_to_move, node.number)
    #
    #     allNodes.sort(key=__sortTorsionMap)
    ...
    def __orderTorsionMap(self, allNodes):
        ...
        def sortTorsionMap(node):
            return (node.atoms_to_move, node.number)

        allNodes.sort(key=sortTorsionMap)
    ...
    def __makeTorsionMap(self):
        ...
        # return torsionMap
        def __orderTorsionMap(self, torsionMap):
            def key_function(node):
                return (node.atoms_to_move, node.number)

            torsionMap.sort(key=key_function)

            # Additional check for indistinguishable nodes
            for i in range(len(torsionMap) - 1):
                if (torsionMap[i].atoms_to_move == torsionMap[i+1].atoms_to_move and
                        torsionMap[i].number == torsionMap[i+1].number):
                    raise RuntimeError("indistinguishable torsion TreeNodes")
            return torsionMap
