...
import numpy as np
from suncal.common import ttable


def test_t():
    # Test calculation of k given conf and degf
    assert np.isclose(ttable.k_factor(conf=.6827, degf=1), 1.84, atol=.005)
    assert np.isclose(ttable.k_factor(conf=.95, degf=1), 12.71, atol=.005)
    assert np.isclose(ttable.k_factor(conf=.9973, degf=5), 5.51, atol=.005)
    assert np.isclose(ttable.k_factor(conf=.90, degf=20), 1.72, atol=.005)
    assert np.isclose(ttable.k_factor(conf=.9545, degf=1E9), 2.00, atol=.005)
...
def test_degf():
    ...
    # This 2.09 shows up in table on two rows! Fractional part is between 19 and 20
    assert np.isclose(ttable.degf(tp=2.09, conf=.95), 19.5, atol=.5)
