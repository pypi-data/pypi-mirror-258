...
def test_av_ffmpeg_link():
    ...
    version_pattern = r"version\s+(?P<major>\d+)\.(?P<minor>\d+)"
    if (version := re.search(version_pattern, version_ffmpeg)) is None:
        raise RuntimeError("failed to decode the version of ffmpeg")
    assert 4 <= int(version["major"]) <= 6, (
        "only the version 4, 5, and 6 of ffmeg are supported, "
...
def test_gpu_torch():
    ...
    if torch.cuda.is_available():
        return  # case alway ok
    # TODO: tests if gpu is present
    # nvidia-smi
