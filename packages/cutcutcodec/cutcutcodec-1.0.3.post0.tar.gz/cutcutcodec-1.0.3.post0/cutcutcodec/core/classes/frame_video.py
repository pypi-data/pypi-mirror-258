...
from cutcutcodec.core.filters.mix.video_cast import to_gray, to_gray_alpha, to_bgr, to_bgr_alpha
...
class FrameVideo(Frame):
    ...
    def to_numpy_bgr(self, contiguous=False) -> np.ndarray[np.uint8]:
        """Return the 3 channels numpy frame representation.
        ...
        >>> import torch
        >>> from cutcutcodec.core.classes.frame_video import FrameVideo
        >>> frame = FrameVideo(0, torch.empty(480, 720, 3)).to_numpy_bgr()  # classical bgr
        >>> type(frame), frame.shape, frame.dtype
        (<class 'numpy.ndarray'>, (480, 720, 3), dtype('uint8'))
        >>> frame = FrameVideo(0, torch.empty(480, 720, 3)).to_numpy_bgr()  # grayscale
        >>> type(frame), frame.shape, frame.dtype
        (<class 'numpy.ndarray'>, (480, 720, 3), dtype('uint8'))
        >>> frame = FrameVideo(0, torch.empty(480, 720, 3)).to_numpy_bgr()  # alpha channel
        >>> type(frame), frame.shape, frame.dtype
        (<class 'numpy.ndarray'>, (480, 720, 3), dtype('uint8'))
        >>>
        """
