...
def dq_generated_dataframe_conf(namespace=None,
    ...
    if table_name not in table_dict.keys():
        ...
        table_dict[table_name]["hammurabi"]["dataFrameInfo"]["targetPathName"] = [f"/in/staging/datax/{namespace}/{target_path_name}"]
        ...
        if hamu_type == "staging":
            ...
            table_dict[table_name]["hammurabi"]["input"]["paths"] = [f"/in/staging/datax/{namespace}/{target_path_name}"]
        ...
        if hamu_type == "raw":
            dir_hocons_mvp_filename = os.path.join(directory_mvp_filename_conf)
            table_dict[table_name]["hammurabi"]["dataFrameInfo"]["targetPathName"] = [f"/data/raw/{namespace}/data/{table_name}/"]
            table_dict[table_name]["hammurabi"]["dataFrameInfo"]["subset"] = "cutoff_date='${?SUBSET_ODATE}'"
            table_dict[table_name]["hammurabi"]["input"] = dict()
            table_dict[table_name]["hammurabi"]["input"]["applyConversions"] = False
            table_dict[table_name]["hammurabi"]["input"]["paths"] = [f"/data/raw/{namespace}/data/{table_name}/"]
            table_dict[table_name]["hammurabi"]["input"]["schema"] = dict(path="${ARTIFACTORY_UNIQUE_CACHE}/artifactory/${SCHEMAS_REPOSITORY}"
                                                                               f"/schemas/pe/{namespace}"
                                                                               f"/raw/{table_name}/latest/{table_name}.output.schema")
...
def dq_generated_mvp(table_master_name=None,
    ...
    for index, rule_id in enumerate(rule_ids_raw):
        ...
        hamu_dict["config"]["dataValues"]["paths"] = [f"/in/staging/datax/{uuaa_name_raw}/" + "#SDA_MYFILENAME_${?PERIOD}.csv"]
...
def generate_kirby_conf(table_raw_name=None, table_master_name=None, kirby_type=None):
    ...
    if kirby_type == "raw":
        ...
        dir_hocons_kirby_raw_output_schema = f"{table_master_name}.output.schema"

        pj_dq_dir_name = os.path.join(pj_dir_workspace, pj_dq_dir_name)
        pj_dq_dir_table_raw_json = os.path.join(pj_dq_dir_name, "data_kirby", "kirby", f"{uuaa_master}", f"{table_master_name}", "raw", f"{dir_hocons_kirby_raw_filename_json}")
        pj_dq_dir_table_raw_conf = os.path.join(pj_dq_dir_name, "data_kirby", "kirby", f"{uuaa_master}", f"{table_master_name}", "raw", f"{dir_hocons_kirby_raw_filename_conf}")
        pj_dq_dir_table_output_schema = os.path.join(pj_dq_dir_name, "data_kirby", "kirby", f"{uuaa_master}", f"{table_master_name}", "raw", f"{dir_hocons_kirby_raw_output_schema}")
        ...
        table_dict_conf[table_raw_name]["kirby"]["input"]["paths"] = [f"/in/staging/datax/{uuaa_raw}/" + "#SDA_MYFILENAME_${?PERIOD}.csv"]
