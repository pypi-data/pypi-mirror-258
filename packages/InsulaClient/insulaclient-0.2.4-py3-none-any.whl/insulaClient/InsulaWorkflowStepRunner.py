...
class InsulaWorkflowStepRunner(object):
    ...
    def __get_parameters_for_value(self, param) -> list[StepResult]:
    ...
    def __run_platform_processor_step(self, step):
        ...
        while attempt < 3:
            ...
            for param in step['params']:
                ...
                params_arr = []
                for _ in self.__get_parameters_for_value(param):
    ...
    def __run_download_job_result(self, step):
        ...
        for param in step['params']:
            ...
            all_in = self.__get_parameters_for_value(param)
            for _ in all_in:
    ...
    def __run_s3_step(self, step):
        ...
        if step['action'] == 'pull':
            for param in step['params']:
                ...
                for file in [param['values']] if isinstance(param['values'], str) else param['values']:
            ...
            for param in step['params']:
                if 'save_in' in param:
                    for file in [param['values']] if isinstance(param['values'], str) else param['values']:
                        try:
                            s3_connections.upload(file, '/'.join([param['save_in'], basename(file)]))
                        except Exception as e:
                            if not continue_on_error:
                                status.set_job_error('FAILED', e)
                                run['status'] = status.get_status()
    ...
    def __run_python_script(self, step):
        ...
        for param in step['params']:
            ...
            return_values = self.__get_parameters_for_value(param)
            if len(return_values) > 1:
                my_namespace.__setattr__(param['name'], [])
                for _ in return_values:
