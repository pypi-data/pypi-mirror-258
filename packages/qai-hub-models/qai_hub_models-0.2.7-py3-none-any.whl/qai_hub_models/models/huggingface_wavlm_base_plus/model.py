...
class HuggingFaceWavLMBasePlus(BaseModel):
    ...
    def __init__(
        self, wavlm_model: torch.nn.Module, apply_apple_npu_opt: bool = False
    ) -> None:
        super().__init__()

        if apply_apple_npu_opt:
    ...
    @classmethod
    def from_pretrained(
        cls, weights_path: str | None = None, apply_apple_npu_opt: bool = False
        ...
        return cls(model, apply_apple_npu_opt)
...
class SliceConv1d(torch.nn.Module):
    def __init__(self, orig_module: torch.nn.Conv1d, slice_size: int = 16000):
        ...
        out_channels, in_channels, kernel_size_1d = orig_module.weight.shape
