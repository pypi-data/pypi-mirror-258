...
class FuncOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_595')) else
      _ods_ir.AttrBuilder.get('anonymous_595')(function_type, context=_ods_context))
...
@_ods_cext.register_operation(_Dialect)
class ExperimentalJoinOp(_ods_ir.OpView):
  OPERATION_NAME = "tt.experimental_join"
...
def experimental_join(lhs, rhs, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(ExperimentalJoinOp(lhs=lhs, rhs=rhs, loc=loc, ip=ip))

@_ods_cext.register_operation(_Dialect)
class ExperimentalSplitOp(_ods_ir.OpView):
  OPERATION_NAME = "tt.experimental_split"
...
def experimental_split(src, *, loc=None, ip=None) -> _Sequence[_ods_ir.Value]:
  return _get_op_result_or_op_results(ExperimentalSplitOp(src=src, loc=loc, ip=ip))
...
def scan(result, srcs, axis, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(ScanOp(result=result, srcs=srcs, axis=axis, loc=loc, ip=ip))
