...
class Ask:
    ...
    def _run_prompt(self, prompt):
        ...
        cache = _ask_cache
        completion = cache.get(prompt) or ollama.chat(
            model=model,
            messages=[
                dict(
                    role="system",
                    content="Write the function in a Python code block must include all necessary imports and no example usage, catach any erros.",
                ),
