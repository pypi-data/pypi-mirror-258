...
from dql.storage import STALE_HOURS_LIMIT, Status, Storage, StorageURI
...
def test_adding_storage(metastore: "AbstractMetastore"):
    uri = StorageURI("s3://whatever")
    with pytest.raises(StorageNotFoundError):
        metastore.get_storage(uri)

    storage, _, _, _ = metastore.register_storage_for_indexing(uri)
    cnt = next(metastore.db.execute(metastore.storages_select(func.count())), (0,))
    assert cnt[0] == 1

    bkt = list(
        metastore.db.execute(
            metastore.storages_select().where(metastore.storages.c.uri == uri)
        )
    )
    assert len(bkt) == 1
...
def test_storage_status(metastore: "AbstractMetastore"):
...
async def test_storage_update_last_inserted_at(metastore: "AbstractMetastore"):
    uri = StorageURI("s3://bucket_last_inserted")
    metastore.create_storage_if_not_registered(uri)
    await metastore.update_last_inserted_at(uri)
    storage = metastore.get_storage(uri)
    assert storage.last_inserted_at


def test_stale_storage(metastore: "AbstractMetastore"):
...
def test_failed_storage(metastore: "AbstractMetastore"):
