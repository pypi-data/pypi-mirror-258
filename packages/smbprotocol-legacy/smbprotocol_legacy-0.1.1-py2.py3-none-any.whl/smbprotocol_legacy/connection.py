...
import smbprotocol_legacy.exceptions
...
from smbprotocol_legacy.transport import Tcp
...
class Connection(object):
        ...
        if tid and session:
            if tid not in session.tree_connect_table.keys():
                error_msg = "Cannot find Tree with the ID %d in the session " \
                            "tree table" % tid
                raise smbprotocol_legacy.exceptions.SMBException(error_msg)
    ...
    def receive(self, request, wait=True, timeout=None):
        ...
        if status not in [NtStatus.STATUS_SUCCESS, NtStatus.STATUS_PENDING]:
            raise smbprotocol_legacy.exceptions.SMBResponseException(response, status)
