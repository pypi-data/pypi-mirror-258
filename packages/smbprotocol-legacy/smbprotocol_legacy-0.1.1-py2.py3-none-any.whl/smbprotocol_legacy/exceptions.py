...
import smbprotocol_legacy.connection
from smbprotocol_legacy.structure import BytesField, EnumField, IntField, ListField, \
...
class SMBUnsupportedFeature(SMBException):
    ...
    def _get_dialect_name(self, dialect):
        dialect_field = EnumField(
            enum_type=smbprotocol_legacy.connection.Dialects,
            enum_strict=False,
            size=2)
...
class SMBResponseException(SMBException):
    ...
    def error_details(self):
        ...
        for raw_error_data in error['error_data'].get_value():
            nt_status = smbprotocol_legacy.connection.NtStatus
