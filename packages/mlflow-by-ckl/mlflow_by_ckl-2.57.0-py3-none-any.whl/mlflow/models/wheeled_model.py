...
class WheeledModel:
    ...
    def save_model(self, path, mlflow_model=None):
        ...
        ocr_reqs = [line for line in open(pip_requirements_path, 'r') if 'spark-ocr' in line]
        ocr_req_path = pip_requirements_path+'.ocr'
        open(ocr_req_path, 'w').write('\n'.join(ocr_reqs))

        no_ocr_reqs = [line for line in open(pip_requirements_path, 'r') if 'spark-ocr' not in line]
        ...
        WheeledModel._download_wheels(
            pip_requirements_path=ocr_req_path, dst_path=wheels_dir
        )
        WheeledModel._download_wheels(
