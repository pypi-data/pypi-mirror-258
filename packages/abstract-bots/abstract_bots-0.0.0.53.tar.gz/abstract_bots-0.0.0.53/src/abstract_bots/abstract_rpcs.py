...
class directoryStart(metaclass=SingletonMeta):
    def __init__(self):
        if not hasattr(self, 'initialized'):  # Prevent reinitialization
            ...
            #data folder
             = make_directory(os.path.join(self.src_dir, 'data'))
            #accounts
            ...
            self.all_pool_info_file = os.path.join(self.all_pool_info_dir,'all_pool_info.json')
            #blockchain_dta
            self.blockchain_data_dir = make_directory(self.data_path,'blockchain_data')
...
class rpcCallManager(metaclass=SingletonMeta):
    ...
    def get_transaction(self,slot,signature):
        ...
        sig_dir = make_directory(slot_dir,str(signature.lower())
