...
from adapter.adapter_server import GPTRole
from command_line_tool.utils import (
    JsonBlob,
    get_folder_contents,
    write_to_file,
)
...
class Interpreter:
    def __init__(
        ...
        self.mark_scheme = self.server_adapter.get_mark_scheme(mark_scheme)
    ...
    def generate_query(
        ...
        match response_type:
            ...
            case "feedback":
                query = (
                    f"{query} Don't include the question in your response. Respond with at "
                    "most a couple of sentences."
                )
            case None:
                pass
            case _:
    ...
    def get_gpt_response(self, query: str) -> str:
        ...
        response = self.server_adapter.get_response(
            adapter.GPTRequest(
                ...
                temperature=0,
                model=adapter.GPTModel.GPT3_5TURBO_UNLIMITED,
            )
        )
        if self.debug_mode:
    ...
    def get_feedback(self) -> list[str]:
        ...
        for part_name, query in self.feedback_queries_full:
            feedback.append(part_name + ": " + self.get_gpt_response(query))
        if self.debug_mode:
            print(feedback)
        return feedback
