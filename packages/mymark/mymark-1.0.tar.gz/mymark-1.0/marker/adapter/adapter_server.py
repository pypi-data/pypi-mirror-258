...
import requests
from adapter.adapter import Adapter, Request
...
class ServerAdapter(Adapter):
    def __init__(self) -> None:
        self.host_address = f"https://{os.getenv('SERVER_HOST')}:{os.getenv('SERVER_PORT_HTTPS')}"

    def get_mark_scheme(self, mark_scheme_name: str) -> str:
        response = requests.get(
            f"{self.host_address}/modules/SED/exercises/{mark_scheme_name}/mark_scheme",
