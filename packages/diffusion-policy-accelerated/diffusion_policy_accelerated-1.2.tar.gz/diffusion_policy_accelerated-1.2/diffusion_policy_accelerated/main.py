...
def diffusion_policy_eval_eager(n, env_handler, model_manager):
    with config.inference_mode_context(config.InferenceMode.NORMAL):
        start_normal_time = time.perf_counter()
        rewards = []
        for _ in tqdm(range(n), desc='Diffusion Policy Eval - Pytorch Eager Mode'):
            config.reset_seed()
        ...
        print(f"Completed {n} normal mode evaluations in {total_time:.4f} seconds (avg {total_time / n:.4f} seconds per episode).")
        print(f"Average reward per episode: {avg_reward:.4f}")


def diffusion_policy_eval_accelerated(n, env_handler, model_manager): 
    with config.inference_mode_context(config.InferenceMode.ACCELERATED):
        start_normal_time = time.perf_counter()
        rewards = []
        for _ in tqdm(range(n), desc='Diffusion Policy Eval - Accelerated Mode'):
            config.reset_seed()
            ep_reward = run_evaluation(env_handler, model_manager)
            rewards.append(ep_reward)
        end_normal_time = time.perf_counter()
        total_time = end_normal_time - start_normal_time
        avg_reward = sum(rewards) / n
        print(f"Completed {n} accelerated mode evaluations in {total_time:.4f} seconds (avg {total_time / n:.4f} seconds per episode).")
        print(f"Average reward per episode: {avg_reward:.4f}")
...
def main():
    ...
    if args.mode == "inference-eval":
        ...
        diffusion_policy_eval_eager(args.evals, env_handler, model_manager)
        diffusion_policy_eval_accelerated(args.evals, env_handler, model_manager)
