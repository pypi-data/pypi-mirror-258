...
def start_listen():
    ...
    def _remove_commented_graphs():
        ...
        for graph in graphs:
            if literal_eval(graph) in GRAPHS:
                GRAPHS.remove(literal_eval(graph))

    def _get_saved_graphs() -> list[str]:
        graphs = _get_graphs(False)

        for graph in graphs:
            if literal_eval(graph) not in GRAPHS:
                GRAPHS.append(literal_eval(graph))
...
class Autolog:
    ...
    def _get_graphs(self, is_notebook: bool) -> list[Any]:
        ...
        for graph in GRAPHS:
            ...
            is_cell_condition = not is_notebook and graph in str(self._cell_content)
