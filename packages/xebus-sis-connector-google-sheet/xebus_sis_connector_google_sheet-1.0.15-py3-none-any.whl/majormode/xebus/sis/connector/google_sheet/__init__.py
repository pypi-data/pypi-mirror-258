...
LIBRARY_DATA_PATH = os.path.join(
    get_project_root_path(__file__, __name__, 1),
    'data'
)
...
class GoogleSheetConnector(SisConnector):
    ...
    DEFAULT_EDUCATION_GRADES_SHEET_NAME = '(Education Grades)'
    ...
    @classmethod
    def __fetch_education_grades_name_mapping_from_google_sheet(
            cls,
    ...
    def __init__(
            ...
            grades_names_mapping: dict[str, int] = None
    ):
        """
        ...
        :param grades_names_mapping: A dictionary representing a mapping
            between the names of grades (the keys) and their corresponding
            grade levels (the values).
        """
    ...
    def fetch_families_list(self) -> FamilyList:
        ...
        if self.__grades_names_mapping is None:
            logging.debug(
                ...
                f"\"{self.DEFAULT_EDUCATION_GRADES_SHEET_NAME}\"..."
            )
            self.__grades_names_mapping = self.__fetch_education_grades_name_mapping_from_google_sheet(
                self.__google_client,
                self.__spreadsheet_url,
                self.DEFAULT_EDUCATION_GRADES_SHEET_NAME
            )
