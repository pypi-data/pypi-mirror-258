...
from .decorators import _MetadataDecorator, DependsOn, DependsOnIfExists, DependsOnTags, EventListener, TaggedWith
...
class Container:
	...
	def __resolve_shard_metadata__(self):
		...
		for shard in self._shards.values():
			# Class metadata
			for metadata in getattr(shard.__class__, _MetadataDecorator.ATTRIBUTE_NAME, []):
				match metadata:
					case TaggedWith():
		...
		for shard in self._shards.values():
			...
			# Attributes
			sentinel = object()
			for attribute_name in dir(shard):
				attribute_value = getattr(shard, attribute_name)
				is_instance_attribute = getattr(shard.__class__, attribute_name, sentinel) is sentinel
				match attribute_value:
					case Injected():
						if is_instance_attribute:
							setattr(shard, attribute_name, self.instance_of(attribute_value.shard_name))
					...
					case InjectedByTags():
						if is_instance_attribute:
							setattr(shard, attribute_name, self.instances_by_tags_of(*attribute_value.tags))
					...
					case class_method if callable(class_method):
						for function_metadata in getattr(class_method, _MetadataDecorator.ATTRIBUTE_NAME, []):
	...
	def send_event(self, event_name: str, *args, **kwargs) -> Future:		
		...
		return self._event_pool.submit(listener, *args, **kwargs)
