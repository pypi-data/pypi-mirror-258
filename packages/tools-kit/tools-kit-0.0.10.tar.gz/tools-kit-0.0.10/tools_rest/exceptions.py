...
from tools_rest.response_view import ResultViewModel, bad_request
...
def exception_handler(exc, context):
    ...
    if isinstance(exc, ValidationError):
        for field, messages in exc.detail.items():
            ...
            result.add_errors(f'{field}{messages[0] if messages else None}')
    ...
    return Response(result.__dict__,status=status_code)
