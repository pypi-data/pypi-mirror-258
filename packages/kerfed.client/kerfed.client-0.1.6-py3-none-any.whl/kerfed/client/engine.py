...
class EngineClient:
    ...
    _api_url: str = "https://api-test.gcp.kerfed.dev/api/v2"
    ...
    def geometry_start(self, request: GeometryRequest, upload: bool = True) -> str:
        """
        ...
        if upload and len(request.source.data) > 0:
            ...
            dest = self.upload_url(filename=request.source.name)
    ...
    def upload_url(self, filename: str) -> dict:
        """
        ...
        response = self.authorized.post(
            f"{self._api_url}/upload", json={"filename": filename}
        )
        response.raise_for_status()
