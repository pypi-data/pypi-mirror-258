    ...
    def __init__(
      ...
      if shape:
        ...
        if ragged_rank is not None and ragged_rank > 1:
          raise ValueError(
            f'Field {name} with shape {shape} should be a fixed-length list '
            'not a nested list ({ragged_rank})')
      else:
          ...
          if isinstance(f.default_value, sparse_tensor.SparseTensor):
            populated[f.name] = sparse_tensor.SparseTensor(
              indices=array_ops.concat(
                [
                  array_ops.zeros(
                    [array_ops.shape(f.default_value.indices)[0], 1],
                    dtype=dtypes.int64),
                  f.default_value.indices],
                axis=1),
              values=f.default_value.values,
              dense_shape=array_ops.concat(
                [[1], f.default_value.dense_shape], axis=0))
            ...
            raise ValueError(
              ...
              f'should be a SparseTensor or a Tensor')
      return populated
      ...
      if f.name not in features and f.default_value is not None:
        if isinstance(f.default_value, sparse_tensor.SparseTensor):
          ...
          populated[f.name] = array_ops.tile(
            array_ops.expand_dims(f.default_value, axis=0),
            tensor_shape.TensorShape([batch_size]).concatenate(
              f.default_value.shape))
        else:
          raise ValueError(
            f'Field {f.name} default_value {f.default_value} '
            f'should be a SparseTensor or a Tensor')
...
def build_fields(filename, fn, fields=None, lower=False):
  ...
  if not all_field_tuples:
    raise ValueError(f'No field found in file {filename}')
  all_fields = {
  ...
  for f in fields:
    if isinstance(f, DataFrame.Field):
      ...
      if f.name not in all_fields and f.default_value is None:
        raise ValueError(
          f'Field {f.name} not found in file {filename}')
      dtype = f.dtype
      if f.name in all_fields:
        actual_shape = f.shape
        ...
        if f.default_value is None:
          raise ValueError(
            f'Field {f.name} not found in file {filename}')
        actual_dtype = f.default_value.dtype
        if isinstance(f.default_value, sparse_tensor.SparseTensor):
          if f.shape is not None:
            raise ValueError(
              f'Field {f.name} shape should be None if default_value is '
              'a SparseTensor')
          actual_shape = None
          actual_ragged_rank = len(f.default_value.dense_shape)
        elif isinstance(f.default_value, ops.Tensor):
          actual_shape = f.default_value.shape
      ...
      if shape is None:
        ...
        raise ValueError(
          f'Field {f.name} shape should be {actual_shape} not {shape}')
      f = DataFrame.Field(
        f.name,
        dtype=dtype,
