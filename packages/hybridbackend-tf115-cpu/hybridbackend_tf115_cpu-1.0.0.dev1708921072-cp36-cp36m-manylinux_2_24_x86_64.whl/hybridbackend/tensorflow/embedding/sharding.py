...
from hybridbackend.tensorflow.distribute.partition.ops import \
  partition_by_modulo
from hybridbackend.tensorflow.embedding.offloading import EmbeddingOffloading
...
class ShardedEmbeddingWeightsRewriting(GraphRewriting):  # pylint: disable=useless-object-inheritance
  ...
  def build_unsharded_weights(self, fn, *args, **kwargs):
    ...
    '''
    if Context.get().options.use_embedding_offloading:
      return EmbeddingOffloading.build_params(
        cache_capacity=Context.get().options.cache_capacity,
        get_var_offloading=self._get_variable_for_offloading)(fn)(
          *args, **kwargs)
    return fn(*args, **kwargs)
...
class ShardedEmbeddingLookupRewriting(GraphRewriting):
  ...
  def wraps_embedding_lookup(self, fn):
    ...
    def _wrapped_embedding_lookup(params, ids, **kwargs):
      ...
      if Context.get().options.use_embedding_offloading:
        with Context.scope(sharding=False):
          return EmbeddingOffloading.lookup(fn)(params, ids, **kwargs)
  ...
  def begin(self):
    r'''Rewrites API.
    '''
    self._prev_lookup = embedding_ops._embedding_lookup_and_transform  # pylint: disable=protected-access

    if (Context.get().options.use_hierarchical_embedding_lookup
        and self._local_world_size > 1
        and self._num_nodes > 1):
      # pylint: disable=protected-access
      embedding_ops._embedding_lookup_and_transform = (
        self.wraps_hierarchical_embedding_lookup(
          embedding_ops._embedding_lookup_and_transform))
    else:
      embedding_ops._embedding_lookup_and_transform = (  # pylint: disable=protected-access
        self.wraps_embedding_lookup(
          embedding_ops._embedding_lookup_and_transform))  # pylint: disable=protected-access
  ...
  def end(self):
    ...
    '''
    # pylint: disable=protected-access
    embedding_ops._embedding_lookup_and_transform = self._prev_lookup
    _shared_emb._get_dense_tensor_internal = (
      self._prev_shared_emb_get_dense_tensor_internal)
    _contrib_feature_column._embeddings_from_arguments = (
      self._prev_embeddings_from_arguments)
