...
class ShardedEmbeddingWeightsRewritingForVariables(
  ...
  def __init__(self):
    ...
    self._get_variable_for_offloading = vs.VariableScope.get_variable
  ...
  def build_sharded_weights(
    ...
    if shape_dims is None or len(shape) != 2:
      if Context.get().options.use_embedding_offloading:
        not_embedding_weights = EmbeddingOffloading.build_params(
    ...
    if bucket_size <= num_shards or bucket_size <= batch_size:
      ...
      if Context.get().options.use_embedding_offloading:
        embedding_weights = EmbeddingOffloading.build_params(
          cache_capacity=Context.get().options.cache_capacity,
          get_var_offloading=self._get_variable_for_offloading)(fn)(
            *args, **kwargs)
      else:
