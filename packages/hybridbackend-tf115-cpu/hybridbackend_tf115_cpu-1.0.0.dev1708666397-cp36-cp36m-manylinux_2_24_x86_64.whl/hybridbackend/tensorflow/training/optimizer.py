...
def wraps_optimizer(
  ...
  class HybridBackendOptimizer(cls, HybridBackendOptimizerBase):
    ...
    def _create_slots(self, var_list):
      ...
      cached_var_list = []
      for item in items:
        if item.params not in var_list:
          var_list.append(item.params)
        if item.params in sharded_vars:
          cached_var_list.append(item.cache_values)
      super()._create_slots(var_list)
      sharded_and_cached_vars = sharded_vars + cached_var_list
      slot_names = self.get_slot_names()
      for v in sharded_and_cached_vars:
    ...
    def _finish(self, update_ops, name_scope):
      r'''Do what is needed to finish the udpate.
      '''
      items = ops.get_collection_ref(EmbeddingOffloading.__name__)
      for item in items:
        update_ops.append(item.before_apply_gradients(self))
        item.before_save_checkpoints()
