...
class Secweb:
    ...
    def get(self, domain):
        ...
        if self.response.status_code == 200:
            return self.parse_response(self.response.json())
        elif self.response.status_code == 403:
            return f"Invalid username, password or API key: {self.response.status_code}"
        elif self.response.status_code == 429:
            return f"Rate limit exceeded: {self.response.status_code}"
        else:
            return f"Request failed with status code {self.response.status_code}"
