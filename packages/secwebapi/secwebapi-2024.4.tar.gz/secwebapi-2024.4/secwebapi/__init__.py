...
class Secweb:
    ...
    async def async_get(self, domain):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{self.base_url}{domain}", headers=self.headers, ssl=sslcontext) as response:
                if response.status == 200:
                    ...
                    return None, f"Invalid username, password or API key: {response.status}"
                elif response.status == 429:
                    return None, f"Rate limit exceeded: {response.status}"
                else:
                    return None, f"Request failed with status code {response.status}"
    ...
    async def async_read_domains_from_file(self, file_path):
        self.results = []
        with open(file_path, 'r') as file:
            for line in file:
                ...
                if not extracted.subdomain and extracted.suffix:
                    self.domain, self.prediction = await self.get(domain)
    ...
    def read_domains_from_file(self, file_path):
        asyncio.run(self.async_read_domains_from_file(file_path))

    def get(self, domain):
        return asyncio.run(self.async_get(domain))