...
class Secweb:
    ...
    async def async_read_domains_from_file(self, file_path, verbose=True):
        self.results = []
        with open(file_path, 'r') as file:
            for line in file:
                ...
                if not extracted.subdomain and extracted.suffix:
                    self.domain, self.prediction = await self.async_get(domain)
                    if verbose:
    ...
    def read_domains_from_file(self, file_path, verbose=True):
        asyncio.run(self.async_read_domains_from_file(file_path, verbose))
