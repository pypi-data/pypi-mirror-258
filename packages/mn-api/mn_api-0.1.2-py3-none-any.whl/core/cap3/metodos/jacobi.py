"""
Capítulo 3. Métodos. Algoritmos de Jacobi.
Módulo que provee de los principales métodos para el algoritmo
de Jacobi
"""
from numpy import copy, zeros
import pandas as pd


class ResultadoJacobi:
    """
    Clase auxiliar que permite modelar el estado del algoritmo de Jacobi en cada iteración

    Contenido:
        lista_x: valores de x en dicha iteración
        delta: valor de delta(δ) en dicha iteración
        error: valor del error cometido en dicha iteración
        iteracion_0: si es la primera iteración de algoritmo; se usa para obviar el error y el delta en la primera iteración
    """

    def __init__(self):
        self.lista_x = []
        self.delta = 0
        self.error = 0
        self.iteracion_0 = False


def jacobi(a, b, x0, f_convergencia, tol, max_iter):
    """
    Descripción:
    ---------------
        Implementación del algoritmo de Jacobi para dar solución
        a sistemas de ecuaciones lineales de orden n en la forma
        AX=B con error menor que ε

    Parámetros:
    ----------------
        a: matriz de los coeficientes de A (AX=B)

        b: matriz de los términos independientes B (AX=B)

        x0: matriz columna que representa los valores estimados de solución (se puede utilizar la matriz trivial)

        f_convergencia: define el factor de convergencia α de la matriz A (AX=B)

        tol: cota para el error absoluto

        max_iter: cantidad máxima de iteraciones

    Salida:
    ------------
        list[ResultadoJacobi]: resultados del algoritmo de Jacobi

    Ejemplo:
    -------------
        >> from numpy import array

        >> a = array([[5, -1, 1],
           [2, 5, -1],
           [-1, 1, 5]])

        >> b = array([10, 12, 10])

        >> x0 = array([0, 0, 0])

        >> tol = 0.005

        >> max_iter = 100

        >> f_convergencia = 0.6

        >> r = jacobi(a, b, x0, f_convergencia, tol, max_iter)

        >> # Ver métodos para convertir los resultados

    :param a: Matriz de los coeficientes de A (AX=B)
    :param b: Matriz de los términos independientes B (AX=B)
    :param x0: Matriz columna que representa los valores estimados de solución (se puede utilizar la matriz trivial)
    :param f_convergencia: Factor de convergencia α de la matriz A (AX=B)
    :param tol: Cota para el error absoluto
    :param max_iter: Cantidad máxima de iteraciones
    """
    xv = copy(x0)
    xa = zeros(b.shape[0])
    condicion = True
    paso = 1
    retorno = []
    r = ResultadoJacobi()

    r.iteracion_0 = True
    r.lista_x = xv.tolist()
    retorno.append(r)

    while condicion:
        r = ResultadoJacobi()
        error = 0

        for i in range(a.shape[0]):
            xa[i] = b[i]

            for j in range(a.shape[1]):
                if j != i:
                    xa[i] -= a[i][j] * xv[j]

            xa[i] /= a[i][i]

            if abs(xa[i] - xv[i]) > error:
                error = abs(xa[i] - xv[i])

        r.delta = error
        error *= abs(f_convergencia / (1 - f_convergencia))
        r.error = error
        r.lista_x = xa.tolist()
        retorno.append(r)

        xv = copy(xa)
        paso += 1
        condicion = error > tol and paso <= max_iter

    return retorno


def convertir_headers_resultados_jacobi(lista_resultados_jacobi):
    """
    Método que permite crear los headers de la tabla de los resultados de Jacobi

    :param lista_resultados_jacobi: Lista de resultados de Jacobi proveniente del método <jacobi>
    """
    lista = []

    r = lista_resultados_jacobi[0]
    for i in range(len(r.lista_x)):
        lista.append(f'X{i + 1}')
    lista.append('Delta (δ)')
    lista.append('Error')

    return lista


def convertir_resultados_jacobi(lista_resultados_jacobi):
    """
    Descripción:
    ----------------
        Permite procesar el resultado del algoritmo de Jacobi en una tabla (DataFrame de pandas)

    Parámetros:
    ----------------
        lista_resultados_jacobi: lista de iteraciones que modela la clase ResultadoJacobi

    Salida:
    ----------------
        DataFrame: tabla con el resultado del algoritmo de Jacobi de forma ordenada

    Ejemplo:
    -----------------
        >> from tabulate import tabulate

        >> from numpy import array

        >> a = array([[5, -1, 1],
           [2, 5, -1],
           [-1, 1, 5]])

        >> b = array([10, 12, 10])

        >> x0 = array([0, 0, 0])

        >> tol = 0.005

        >> max_iter = 100

        >> f_convergencia = 0.6

        >> r = jacobi(a, b, x0, f_convergencia, tol, max_iter)

        >> dataframe = convertir_resultados_jacobi(r)

        >> print(tabulate(dataframe, headers="keys", tablefmt="fancy_grid"))

    :param lista_resultados_jacobi: Lista de iteraciones que modela la clase ResultadoJacobi
    """
    lista = []
    for r in lista_resultados_jacobi:
        l = []
        if r.iteracion_0:
            for x in r.lista_x:
                l.append(x)
            l.append('-------')
            l.append('-------')
        else:
            for x in r.lista_x:
                l.append(x)
            l.append('{:.7f}'.format(r.delta))
            l.append('{:.7f}'.format(r.error))

        lista.append(l)

    df = pd.DataFrame(data=lista, columns=convertir_headers_resultados_jacobi(lista_resultados_jacobi))
    df.index.name = 'Iteración'
    return df
