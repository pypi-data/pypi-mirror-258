...
class Meteo_initializer:
    ...
    def init_ERA5(self):
            ...
            try:
                meteo=Meteo(date=date,times=times,step=step,latmin=latmin,latmax=latmax,lonmin=lonmin,lonmax=lonmax,res=res, output=output,verbose=verbose,path=os.path.join(self.file_manager.project_path,self.file_manager.meteo_folder),server=server,log_file=self.file_manager.log_file_path,api_key=api_key)
