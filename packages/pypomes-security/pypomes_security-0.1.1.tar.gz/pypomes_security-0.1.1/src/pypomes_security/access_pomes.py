...
def access_get_token(errors: list[str],
    ...
    # obtain the current access token expiration
    token_expiration: datetime = __get_token_expiration(service_url)
    ...
    if just_now < token_expiration:
        # yes, return it
        result = __access_tokens.get(service_url).get("token")
    else:
...
def __get_token_expiration(service_url: str) -> datetime:
    ...
    return url_token_data.get("expiration")
