...
class ContextFiles(Static):
    ...
    def compose(self) -> ComposeResult:
        yield ScrollableContainer(Container(Button('Add Files', id='navigateFileSelector'), Button('Search Phrase', id="navigateSearchPhrase"), id="filesActionButtons"), id="selectedFilesScroll")
...
class ChatInput(Static):
    ...
    def compose(self) -> ComposeResult:
        yield Container(TextArea(language="markdown", id="userInput"), Button('Submit', variant='success', id="submit"), id="chatInputContainer")
...
class Main(Screen):
    ...
    def ask_open_ended(self, action):
        prompt = ChatInput()
    ...
    async def solve_task(self):
        ...
        self.answer_actions = [self.start_philly]
    ...
    async def start_philly(self):
        ...
        res = phil.generate_step(future_thought=self.future)

        if res:
            self.action, self.future, self.file_paths, self.current_snippets = res
    ...
    async def apply_plan(self):
        ...
        for snippet in env.SNIPPETS:
                if not snippet.get('file_path') in self.file_paths:
    ...
    async def apply_feedback(self):
        ...
        self.action, self.future = phil.apply_feedback(action=self.action, feedback=feedback, file_path=self.file_paths, current_snippets=self.current_snippets)
