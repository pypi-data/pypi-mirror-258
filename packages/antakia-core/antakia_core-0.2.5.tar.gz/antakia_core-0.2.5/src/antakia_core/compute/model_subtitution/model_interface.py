...
class InterpretableModels:
    ...
    def _train_models(self, X_train, y_train, X_test, y_test):
        Parallel(n_jobs=1)(
            delayed(model.fit_and_compute_fi)(X_train, y_train, X_test, y_test, score=self.custom_score) for
            model_name, model in
            self.models.items() if not model.fitted)

    def compute_score_type(self, customer_model, X: pd.DataFrame, y: pd.Series):
    ...
    def get_models_performance(
        ...
        if self.score_type == 'compute':
            self.compute_score_type(customer_model, X_test, y_test)
        ...
        if customer_model is not None:
            ...
            self.models[self.customer_model_name].compute_feature_importances(X_test, y_test, self.custom_score, self.score_type)
