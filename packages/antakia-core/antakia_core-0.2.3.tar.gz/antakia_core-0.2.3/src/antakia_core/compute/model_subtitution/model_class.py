...
class MLModel:
    ...
    def compute_feature_importances(self, X_test, y_test, score):
        if self.feature_importances_ is None:
            if hasattr(self.model, 'feature_importances_'):
                ...
                def scorer(model, X, y):
                    ...
                    self.model, X_test, y_test, n_repeats=10, random_state=42, n_jobs=2, scoring=scorer
                )
                self.feature_importances_ = pd.Series(
                    fi.importances_mean,
                    index=X_test.columns
    ...
    def fit_and_compute_fi(self, X_train, y_train, X_test, y_test, score):
        ...
        self.compute_feature_importances(X_test, y_test, score)
