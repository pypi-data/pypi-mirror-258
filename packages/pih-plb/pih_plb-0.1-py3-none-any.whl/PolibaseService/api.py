...
from pih.tools import j, js, esc, e, escs, n, nn, ne, nl, BitMask as BM, while_not_do, one
    ...
    def get_connection(
        ...
        if poolled:
            try:
                ...
                A.ER.global_except_hook(exception, A.D.map(lambda item: item.message, exception.args))
                if catch_exception:
