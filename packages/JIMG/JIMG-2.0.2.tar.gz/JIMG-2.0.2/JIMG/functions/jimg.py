...
def resize_tiff(image, metadata, height = None, width = None, resize_factor = None):
    ...
    Returns:
        if metadata == None:
            Image: Resized image in *.tiff format
...
def save_tiff(tiff_image:np.ndarray, path_to_save:str = '', metadata = None):
    ...
    Args:
       ...
       metadata (dict | None) - metadata to the file
...
def xml_load(path_to_xml:str):
    ...
    Args:
       ...
       path_to_xml (str) - path to a images
       path_to_save (str) - path to save directories with raw images divided by channels 
...
def manual_outlires(image_info:pd.DataFrame, list_of_out:list = [], dispaly_plot = False):
    ...
    Returns:
        Graph: location and numeration of raw images in the main core of the full image
    ...
    """
     ...
     The full image core is based on image metadata and raw images occurrence modified by manual_outlires() and repair_image() functions.
      ...
      Args:
         ...
         image_queue (pd.DataFrame) - data frame with calculated raw images queue from image sequences() function
...
def load_tiff(path_to_tiff:str):
    ...
    """
      ...
      Returns:
          Image: loaded image returned to a variable
...
def z_projection(tiff_object, projection_type = 'avg'):
    ...
    """
      ...
      Returns:
          Image: image projection returned to a variable
...
def clahe_16bit(img, kernal = (100, 100)):
    ...
    """
      ...
      Returns:
          Image: image after the CLAHE adjustment
...
def equalizeHist_16bit(image_eq):
    ...
    """
      ...
      Returns:
          Image: image after the global histograme equalization adjustment
...
def adjust_img_16bit(img, color = 'gray', max_intensity = 65535, min_intenisty = 0, brightness = 100, contrast = 1, gamma = 1):
    ...
    """
      ...
      Returns:
          Image: image after the parameters adjustment
...
def load_image(path):
    ...
    Returns:
        Image: 16-bit image loaded to a variable
