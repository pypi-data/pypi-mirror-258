   ...
   # Fonction qui permet d'executer toutes les commmandes nécessaires via une connection SSH et SFTP
    def ssh_upload(self, hostname, username, password, local_files, remote_directory, command_to_execute, id, python, epochs, server, dependances):
        ...
        try:
            ...
            folders = remote_directory.split('/')

            # Parcourir les composants du chemin et créer les dossiers manquants
            current_path = ''
            for folder in folders:
                current_path = current_path + '/' + folder if current_path else folder

                try:
                    # Essayez de changer de répertoire. Si cela échoue, le dossier n'existe pas encore.
                    ssh.sftp.chdir(current_path)
                except IOError:
                    # Le dossier n'existe pas, alors le créons
                    ssh.sftp.mkdir(current_path)
                    ssh.sftp.chdir(current_path)
