...
class InferenceGroup(Group):
    """
    This class contains functions for inference api
    """
    def create(self, data : dict)->dict:
        """
            This function create the infernece data
        """
        result = self.client.post(LENDSMART_INFERENCES, data=data)
        if not "id" in result:
            return self.raise_unexpected_response(
                message="Error on creating infernece data",
                status=500,
                json=result
            )
        return result
...
class LendsmartClient:
    def __init__(self, service_account, base_url, user_agent=None):
        ...
        self.inference = InferenceGroup(self)
