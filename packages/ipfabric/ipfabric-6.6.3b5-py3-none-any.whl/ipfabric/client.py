...
from typing import Optional, Union, Dict, List, Any, overload, Literal, Tuple
from urllib.parse import urlparse, urljoin
from warnings import warn

from httpx import InvalidURL, URL, HTTPStatusError
...
class IPFClient(IPFabricAPI):
    ...
    @staticmethod
    def _shared_url_id(url: Union[int, str]) -> Tuple[str, str]:
        try:
            return str(int(url)), None
        except ValueError:
            parsed_url = URL(url)
            if parsed_url.params.get("copyId"):
                return parsed_url.params.get("copyId"), "table"
            else:
                return parsed_url.path.split("/")[-1], "graph"

    def _get_shared_view(self, url_id: str, view: str = None) -> dict:
        if view == "table":
    ...
    def _table_url(self, url_id: str, data=True, reports=True):
        query = self._get_shared_view(url_id, "table")
        columns = list(set(query.get("columnVisibility", {}).keys()).union(set(query.get("columnWidth", {}).keys())))
    ...
    def shared_view(self, url: Union[int, str], data=True, reports: bool = True) -> Union[str, list]:
        ...
        """
        url_id, _ = self._shared_url_id(url)
        return self._table_url(url_id, data, reports)

    def shared_url(self, url: Union[int, str], data=True) -> Union[str, list, dict]:
        """Takes a shared link and returns the data or the code to implement in python.

        Args:
            url: Id of the shared view (1453653298) or
                 full/partial URL (`/inventory/devices?copyId=1453653298` or `/diagrams/share/1453626097`)
            data: Defaults to return the data instead of printing the code

        Returns: Data (list of dicts for table or dict for graph) or a string representing the code.
        """
        url_id, view = self._shared_url_id(url)
        if view == "table":
            return self.shared_view(url_id, data)
        elif view == "graph":
            return self.diagram.shared_view(url_id, data="json" if data else "code")
        if "webEndpoint" in self._get_shared_view(url_id):
            return self.shared_view(url_id, data)
        return self.diagram.shared_view(url_id, data="json" if data else "code")
