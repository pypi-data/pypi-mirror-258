...
class IPFClient(IPFabricAPI):
    ...
    @staticmethod
    def _shared_url_id(url: Union[int, str]):
        try:
    ...
    def _parse_shared_url(self, url: Union[int, str]) -> dict:
    ...
    def _table_url(self, query: dict, data=True):
        ...
        if data:
            return self.fetch_all(url=url, columns=columns, filters=filters, sort=sort)
    ...
    def shared_view(self, url: Union[int, str], data=True) -> Union[str, list]:
        ...
        """
        query, _ = self._shared_url_id(url)
        return self._table_url(query, data)
    ...
    def shared_url(self, url: Union[int, str], data=True) -> Union[str, list, dict]:
        ...
        """
        query = self._parse_shared_url(url)
        if "webEndpoint" in query:
            return self.shared_view(query, data)
        return self.diagram.shared_view(query, data="json" if data else "code")
