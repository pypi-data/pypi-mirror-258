        ...
        if step % print_freq == 0 or step == (len(train_loader) - 1):
            print(
                ...
                .format(
                    ...
                    lr=batch_scheduler.get_lr()[0],
                    # acc=np.mean(acc_list),
                )
            )

        if wandb and step % print_freq == 0:
            print({f"[loss": losses.val, f"[lr": optimizer.param_groups[0]["lr"]})
        ...
        **kwargs,
        ...
        for epoch in range(epochs):
            ...
            self.train_step(
                ...
                data_collator=data_collator,  # kwargs["collate_fn"] if kwargs["collate_fn"] else None
                device=self.device,
            )
...
class CustomPairwiseTrainer(CustomTrainer):
    def __init__(self, vec_model: Union[str, nn.Module], device=None):
        super().__init__(vec_model, device)


class CustomRerankerTrainer(object):
