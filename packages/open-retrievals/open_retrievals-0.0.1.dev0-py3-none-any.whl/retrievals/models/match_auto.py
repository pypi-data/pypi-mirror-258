...
class AutoModelForMatch(nn.Module):
    ...
    def similarity_search(self, query_embed, content_embed=None, topk=1, **kwargs):
        """ """
        if self.method == "knn":
            neighbors_model = NearestNeighbors(n_neighbors=topk, metric="cosine", n_jobs=-1)
            neighbors_model.fit(content_embed)
            dists, indices = neighbors_model.kneighbors(query_embed)
            return dists, indices

        elif self.method == "cosine":
            dists, indices = cosine_similarity_search(query_embed, content_embed, topk=topk)
            return dists, indices
...
def cosine_similarity_search(
    ...
    for idx in range(len(embeddings_chunks)):
        cos_sim_chunk = torch.mm(embeddings_chunks[idx], content_embed.transpose(0, 1))
        ...
        #     cos_sim_chunk = cos_sim_chunk.clamp(0,1) ** torch.Tensor((1-(contents["old_count"].values==0)*0.1).reshape(1,-1)).to(DEVICE)
        if temperature:
