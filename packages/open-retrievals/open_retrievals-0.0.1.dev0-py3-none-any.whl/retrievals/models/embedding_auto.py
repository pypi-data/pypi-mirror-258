...
from transformers import AdamW, AutoConfig, AutoModel, AutoTokenizer, GenerationConfig
...
class AutoModelForEmbedding(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.tokenizer = AutoTokenizer.from_pretrained(model_name_or_path, return_tensors=False)
        if pretrained:
    ...
    def encode_from_loader(
        ...
        with torch.no_grad():
            for idx, inputs in enumerate(loader):
                ...
                all_embeddings.append(embed.detach().cpu().numpy())
    ...
    def encode_from_text(
        ...
        """
        Computes sentence embeddings from sentence-transformers.
