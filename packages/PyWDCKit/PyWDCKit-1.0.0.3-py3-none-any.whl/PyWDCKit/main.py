...
class WDCKit:
    ...
    def get_dui(self, device, file_save_path:str = ".\\"):
    ...
    def get_idd(self,device):
        logging.info("Collectiong IDD")
        output = self.__execute(f'{self.exec_path} idd {device}  --output json')
        return self.__parse_results(output)
