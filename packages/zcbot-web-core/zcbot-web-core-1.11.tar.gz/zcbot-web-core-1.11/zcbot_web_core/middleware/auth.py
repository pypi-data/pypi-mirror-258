...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, next_func) -> Response:
        ...
        try:
            ...
            if rs:
                # 挂载
                user_or_app = share_auth.get_auth_user_or_app(request)
                auth_user_context.set(user_or_app)
                if user_or_app and user_or_app.tenantCode:
                    tenant_context.set(user_or_app.tenantCode)
                # 执行
