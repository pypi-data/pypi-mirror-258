...
class ShareAuth(object):
    ...
    def get_api_token(self, request: Request) -> str:
        """
        api接口解析token
        """
        if request and (request.headers or request.query_params):
    ...
    def access_check(self, request: Request):
        ...
        if not auth_cache_pool.has_user(access_token):
    ...
    def get_auth_user(self, request: Request) -> Optional[AuthUser]:
        ...
        """
        access_token = self.get_access_token(request)
        if access_token and auth_cache_pool.has_user(access_token):
    ...
    def get_auth_api(self, request: Request) -> Optional[AuthUser]:
        """
        api接口通过headers.token 查找对应的用户
        """
        token_str = self.get_access_token(request)
        if token_str and auth_cache_pool.has_user(token_str):
            auth_user = auth_cache_pool.get_user_by_token(token_str=token_str)
            if auth_user:
                return auth_user

        return None

    def get_auth_tenant(self, request: Request):
        """
        获取授权用户所属租户
        :param request:
        :return:
...
# 用于handler来Depends，获取授权用户，使用api推送数据
async def authed_api(request: Request) -> Optional[AuthUser]:
    return ShareAuth().get_auth_api(request)
