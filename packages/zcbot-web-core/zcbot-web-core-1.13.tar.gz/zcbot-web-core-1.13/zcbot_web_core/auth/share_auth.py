...
class ShareAuth(object):
    ...
    def get_auth_user_or_app(self, request: Request) -> Optional[Union[AuthApp, AuthUser]]:
        access_token = self.get_access_token(request)
        # TODO 这里暂时通过token前缀区分
        if access_token and access_token.startswith('app-'):
            return auth_cache_pool.get_cached_app(access_token)
        else:
            return auth_cache_pool.get_cached_user(access_token)
