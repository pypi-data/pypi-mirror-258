...
class ShareAuth(object):
    ...
    def get_auth_app(self, request: Request) -> Optional[AuthUser]:
        """
        api接口通过headers.token 查找对应的用户
        """
        token_str = self.get_access_token(request)
        if token_str and auth_cache_pool.has_access(token_str):
            auth_app = auth_cache_pool.get_cached_app(access_token=token_str)
            if auth_app:
                return auth_app
...
async def authed_app(request: Request) -> Optional[AuthUser]:
