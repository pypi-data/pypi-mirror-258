...
class ShapeService(object):
    ...
    def save_dataset_shape(self, dataset, model: CruiseShape):
        ...
        # shape_id = None
        # mb_shape = model.shape
        # cruise_shape = None  # self.cruise.get_type_obj(oracledb.DB_TYPE_BLOB)
        # geom_type = model.geom_type
        # shape_type = model.shape_type
        #
        # insert_values = {'SHAPE': cruise_shape, 'GEOM_TYPE': geom_type, 'SHAPE_TYPE': shape_type, 'SHAPE_ID': shape_id}
        # command = "INSERT INTO CRUISE.SHAPES(SHAPE, SHAPE_TYPE, GEOM_TYPE) " \
        #           "VALUES (SDO_GEOMETRY(empty_blob(), 8307), :SHAPE_TYPE, :GEOM_TYPE) " \
        #           "returning SHAPE_ID, SHAPE into :SHAPE_ID, :SHAPE;"
        #
        # with self.cruise.get_connection() as connection:
        #     cursor = connection.cursor()
        #
        #     cruise_shape = cursor.var(oracledb.DB_TYPE_CLOB)
        #     cursor.execute(command, insert_values)
        #
        #     blob, = cruise_shape.getvalue()
        #     offset = 1
        #
        #     num_bytes_in_chunk = 65536
        #
        #     for i in self.chunkstring(mb_shape, num_bytes_in_chunk):
        #         blob.write(i, offset)
        #         offset += len(i)
        #
        #     connection.commit()

        ### https://python-oracledb.readthedocs.io/en/latest/user_guide/lob_data.html#lobdata
        # id_val = 9
