...
class DatalakeInterface:
    ...
    def get_item_list_from_datalake_llm(self, payload, request_id):
        ...
        try:
            ...
            status_code = response.status_code
            res = response.json()
            logger.info(f"get_item_list_from_datalake_llm | request_id: {request_id}, response: {res}")
            if status_code == 200:
                return response.json()
            else:
            ...
            logger.error(f"get_item_list_from_datalake_llm | An error occurred for request_id: {request_id} Error : {e}\nTraceback:\n{traceback_str}")
    ...
    def execute_mongo_query(self, payload, request_id):
        ...
        try:
            logger.info(f"execute_mongo_query | sending request to metalake, request_id: {request_id}, payload: {payload}")
            response = requests.post(url=url, json=payload, headers=hed)
            status_code = response.status_code
            res = response.json()
            logger.info(f"execute_mongo_query | request_id: {request_id}, response: {res}")
            if status_code == 200:
            ...
            traceback_str = "".join(tb_str)
            # Log the detailed error with traceback
            logger.error(f"execute_mongo_query | An error occurred for request_id: {request_id} Error : {e}\nTraceback:\n{traceback_str}")
            # Re-raise the caught exception
            raise
    ...
    def run_sql_query(self, payload, request_id):
        ...
        try:
            logger.info(f"run_sql_query | sending request to metalake, request_id: {request_id}, payload: {payload}")
            response = requests.post(url=url, json=payload, headers=hed)
            status_code = response.status_code
            res = response.json()
            ...
            traceback_str = "".join(tb_str)
            # Log the detailed error with traceback
            logger.error(f"run_sql_query | An error occurred for request_id: {request_id} Error : {e}\nTraceback:\n{traceback_str}")
            # Re-raise the caught exception
            raise
