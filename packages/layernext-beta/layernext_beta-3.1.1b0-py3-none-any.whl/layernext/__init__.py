...
class LayerNextClient:
    ...
    def retrieve_documents(
        ...
        """
        request_id = uuid.uuid4()
        logger.info(f" sdk | retrieve_documents | request_id >>>> {request_id}, document_categories:{document_categories}, document_search_keys:{document_search_keys}, chunk_search_keys:{chunk_search_keys}, page_index:{page_index}, page_size:{page_size}")
        ...
        logger.info(f" sdk | retrieve_documents | request_id <<<< {request_id}, response: {chunk_list}")
        return chunk_list
    ...
    def execute_mongo_query(self, source_name: str, function, input_data, collection, request_id=uuid.uuid4()):
        """ 
        ...
        return _datalake_client.execute_mongo_query(source_name, function, input_data, collection, request_id)
    ...
    def run_mongodb_aggregation(self, source_name: str, pipeline_stages, collection):
        ...
        """
        request_id = uuid.uuid4()
        logger.info(f" sdk | run_mongodb_aggregation | request_id >>>> {request_id}, source_name:{source_name}, pipeline_stages:{pipeline_stages}, collection:{collection}")
        res = self.execute_mongo_query(source_name, 'aggregate', pipeline_stages, collection, request_id)
        logger.info(f" sdk | run_mongodb_aggregation | request_id <<<< {request_id}, response: {res}")
        return res
    ...
    def run_sql_query(self, source_name: str, query: str):
        ...
        """
        request_id = uuid.uuid4()
        logger.info(f" sdk | run_sql_query | request_id >>>> {request_id}, source_name:{source_name}, query:{query}")
        _datalake_client = datalake.DatalakeClient(
            self.encoded_key_secret, self.layernext_url)
        res = _datalake_client.run_sql_query(source_name, query, request_id)
        logger.info(f" sdk | run_sql_query | request_id <<<< {request_id}, response: {res}")
        return res
