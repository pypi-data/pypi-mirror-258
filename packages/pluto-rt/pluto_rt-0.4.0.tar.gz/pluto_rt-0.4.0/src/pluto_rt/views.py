...
from django.http import HttpRequest, HttpResponseBase, HttpResponse, StreamingHttpResponse
...
async def rt_sse_content(
    ...
    """Deliver event stream of formatted messages from the pluto_rt/item.html template."""
    ...
    while True:
        ...
        if item == mqueue.QUEUE_EXHAUSTED:
            yield "event: close\ndata: \n\n"
            return
        formatted_item = render_to_string(item_template, {"item": item})
        yield f"data: {formatted_item}\n\n"


def rt_sse(
    request: HttpRequest, queue_name: str, item_template: str = "pluto_rt/item.html"
) -> HttpResponseBase:
...
def rt_polling(
    ...
    body = "".join(render_to_string(item_template, {"item": item}) for item in items)
