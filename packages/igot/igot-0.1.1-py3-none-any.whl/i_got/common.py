...
def script_main(download, download_playlist, **kwargs):
    ...
    try:
        ...
        if not args.debug:
            ...
            log.e('  (2) Make sure igot is up-to-date.')
            log.e('  (3) Check if the issue is already known, on')
            log.e('        https://github.com/iniself/igot/wiki/Known-Bugs')
            log.e('        https://github.com/iniself/igot/issues')
...
def url_to_module(url):
    ...
    if k in SITES:
        return (
            import_module('.'.join(['i_got', 'extractors', SITES[k]])),
            url
        )
    else:
        try:
        ...
        if location and location != url and not location.startswith('/'):
            ...
            return import_module('i_got.extractors.universal'), url
