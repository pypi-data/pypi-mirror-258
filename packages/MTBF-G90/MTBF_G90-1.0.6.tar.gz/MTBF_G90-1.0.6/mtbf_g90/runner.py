        ...
        def gather_debug(test, status):
            ...
            if marionette.session is not None:
                try:
                    ...
                    screen_shot_folder = os.environ.get("FAILURE_SCREENSHOT_PATH")
                    if bool(self.marionette._test_name) and ".py" in str(self.marionette._test_name):
                        test_name_and_time_str = str(self.marionette._test_name).split('.py')[0]
                        time_now = str(datetime.now().strftime("%d_%m_%Y_%H_%M_%S"))
                        device_path = "/data/"+str(self.device_serial)+"_"+test_name_and_time_str+"_"+str(time_now)+".png"
                        
                        cmd = "adb -s "+str(self.device_serial)+" shell gfxdebugger -c screencap -p "+device_path
                        os.popen(cmd)
                        time.sleep(2)
                        cmd_pull = "adb -s "+str(self.device_serial)+" pull "+str(device_path)+" "+screen_shot_folder
                        os.popen(cmd_pull)
                        time.sleep(2)
                        os.popen("adb -s "+str(self.device_serial)+" shell rm -rf "+device_path)
                        time.sleep(1)
                    
                    else:
                        print("Can't take screenshot, Testcase name is not valid ..")
                    # os.popen("adb -s "+str(self.device_serial)+" shell gfxdebugger -c screencap -p /data/test_"+str(self.device_serial)+"_"+str(test_tags)+".png")
