...
from puffling.metadata.utils import (
    get_normalized_dependency,
    is_valid_project_name,
    normalize_project_name,
)
from puffling.plugin.manager import PluginManagerBound
from puffling.utils.constants import DEFAULT_CONFIG_FILE
...
if TYPE_CHECKING:
    ...
    from puffling.utils.context import Context
...
class ProjectMetadata(Generic[PluginManagerBound]):
    def __init__(
        ...
        self._puffling: PufflingMetadata | None = None
    ...
    def version(self) -> str:
        ...
        if self._version is None:
            ...
            if (
                "version" in self.dynamic
                and "version" in self.core_raw_metadata["dynamic"]
            ):
    ...
    def core(self) -> CoreMetadata:
        if self._core is None:
            metadata = CoreMetadata(
                self.root, self.core_raw_metadata, self.puffling.metadata, self.context
            )

            # Save the fields
            _ = self.dynamic

            metadata_hooks = self.puffling.metadata.hooks
            if metadata_hooks:
                static_fields = set(self.core_raw_metadata)
                if "version" in self.puffling.config:
    ...
    @property
    def puffling(self) -> PufflingMetadata:
        if self._puffling is None:
            tool_config = self.config.get("tool", {})
            if not isinstance(tool_config, dict):
                message = "The `tool` configuration must be a table"
                raise TypeError(message)

            puffling_config = tool_config.get("puffling", {})
            if not isinstance(puffling_config, dict):
                message = "The `tool.puffling` configuration must be a table"
                raise TypeError(message)
            ...
            if puffling_file and os.path.isfile(puffling_file):
                config = load_toml(puffling_file)
                puffling_config = puffling_config.copy()
                puffling_config.update(config)

            self._puffling = PufflingMetadata(
                self.root, puffling_config, self.plugin_manager
            )

        return self._puffling
    ...
    def _get_version(self, core_metadata: CoreMetadata | None = None) -> str:
        ...
        if version is None:
            version = self.puffling.version.cached
            source = f"source `{self.puffling.version.source_name}`"
...
class CoreMetadata:
    ...
    def __init__(
        ...
        root: str,
        config: dict[str, Any],
        puffling_metadata: PufflingMetadataSettings,
        context: Context,
    ) -> None:
        self.root = root
        self.config = config
        self.puffling_metadata = puffling_metadata
        ...
        self._optional_dependencies_complex: dict[
            str, dict[str, Requirement]
        ] | None = None
    ...
    def license(self) -> str:
        ...
        if self._license is None:
            ...
            if data is None:
                ...
                from puffling.licenses.parse import normalize_license_expression
        ...
        if self._dependencies_complex is None:
            ...
            for i, entry in enumerate(dependencies, 1):
                ...
                try:
                    ...
                    if (
                        requirement.url
                        and not self.puffling_metadata.allow_direct_references
                    ):
                        message = (
                            f"Dependency #{i} of field `project.dependencies` cannot be a direct reference unless "
                            f"field `tool.puffling.metadata.allow-direct-references` is set to `true`"
                        )
                        raise ValueError(message)

                    dependencies_complex[
                        get_normalized_dependency(requirement)
                    ] = requirement
    ...
    def optional_dependencies_complex(self) -> dict[str, dict[str, Requirement]]:
        ...
        if self._optional_dependencies_complex is None:
            ...
            for option, dependencies in optional_dependencies.items():
                    ...
                    try:
                        ...
                        if (
                            ...
                            message = (
                                ...
                                f"`tool.puffling.metadata.allow-direct-references` is set to `true`"
                            )
                            raise ValueError(message)
                ...
                normalized_option = (
                    option
                    if self.puffling_metadata.allow_ambiguous_features
                ...
                optional_dependency_entries[normalized_option] = dict(
...
class PufflingMetadata(Generic[PluginManagerBound]):
    def __init__(
        self,
        root: str,
        ...
        self._metadata: PufflingMetadataSettings | None = None
        self._build_config: dict[str, Any] | None = None
        self._build_targets: dict[str, Any] | None = None
        self._version: PufflingVersionConfig | None = None

    @property
    def metadata(self) -> PufflingMetadataSettings:
        if self._metadata is None:
            metadata_config = self.config.get("metadata", {})
            if not isinstance(metadata_config, dict):
                message = "Field `tool.puffling.metadata` must be a table"
                raise TypeError(message)

            self._metadata = PufflingMetadataSettings(
    ...
    def build_config(self) -> dict[str, Any]:
        if self._build_config is None:
            ...
            if not isinstance(build_config, dict):
                message = "Field `tool.puffling.build` must be a table"
                raise TypeError(message)
    ...
    def build_targets(self) -> dict[str, Any]:
        if self._build_targets is None:
            ...
            if not isinstance(build_targets, dict):
                message = "Field `tool.puffling.build.targets` must be a table"
                raise TypeError(message)
    ...
    @property
    def version(self) -> PufflingVersionConfig:
        if self._version is None:
            if "version" not in self.config:
                message = "Missing `tool.puffling.version` configuration"
                raise ValueError(message)

            options = self.config["version"]
            if not isinstance(options, dict):
                message = "Field `tool.puffling.version` must be a table"
                raise TypeError(message)

            self._version = PufflingVersionConfig(
...
class PufflingVersionConfig(Generic[PluginManagerBound]):
    def __init__(
    ...
    def source_name(self) -> str:
        if self._source_name is None:
            ...
            if not source:
                message = "The `source` option under the `tool.puffling.version` table must not be empty if defined"
                raise ValueError(message)

            if not isinstance(source, str):
                message = "Field `tool.puffling.version.source` must be a string"
                raise TypeError(message)
    ...
    def scheme_name(self) -> str:
        if self._scheme_name is None:
            ...
            if not scheme:
                message = "The `scheme` option under the `tool.puffling.version` table must not be empty if defined"
                raise ValueError(message)

            if not isinstance(scheme, str):
                message = "Field `tool.puffling.version.scheme` must be a string"
                raise TypeError(message)
    ...
    def source(self) -> VersionSourceInterface:
        if self._source is None:
            ...
            if version_source is None:
                from puffling.plugin.exceptions import UnknownPluginError
...
class PufflingMetadataSettings(Generic[PluginManagerBound]):
    def __init__(
        self, root: str, config: dict[str, Any], plugin_manager: PluginManagerBound
    ) -> None:
        self.root = root
    ...
    def allow_direct_references(self) -> bool:
        if self._allow_direct_references is None:
            ...
            if not isinstance(allow_direct_references, bool):
                message = "Field `tool.puffling.metadata.allow-direct-references` must be a boolean"
                raise TypeError(message)
    ...
    def allow_ambiguous_features(self) -> bool:
        ...
        if self._allow_ambiguous_features is None:
            ...
            if not isinstance(allow_ambiguous_features, bool):
                message = "Field `tool.puffling.metadata.allow-ambiguous-features` must be a boolean"
                raise TypeError(message)
    ...
    def hook_config(self) -> dict[str, Any]:
        if self._hook_config is None:
            ...
            if not isinstance(hook_config, dict):
                message = "Field `tool.puffling.metadata.hooks` must be a table"
                raise TypeError(message)
