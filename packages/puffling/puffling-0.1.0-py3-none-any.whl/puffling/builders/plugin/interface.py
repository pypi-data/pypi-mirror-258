...
from puffling.builders.config import BuilderConfig, BuilderConfigBound, env_var_enabled
from puffling.builders.constants import EXCLUDED_DIRECTORIES, BuildEnvVars
from puffling.builders.utils import get_relative_path, safe_walk
from puffling.plugin.manager import PluginManagerBound

if TYPE_CHECKING:
    from puffling.bridge.app import Application
    from puffling.builders.hooks.plugin.interface import BuildHookInterface
    from puffling.metadata.core import ProjectMetadata
...
class BuilderInterface(ABC, Generic[BuilderConfigBound, PluginManagerBound]):
    ...
    def __init__(
        ...
        self.__puffling_config: dict[str, Any] | None = None
    ...
    def build(
        ...
        *,
        ...
        if versions:
            ...
            if unknown_versions:
                message = f'Unknown versions for target `{self.PLUGIN_NAME}`: {", ".join(map(str, sorted(unknown_versions)))}'
        ...
        for version in versions:
            ...
            if hooks_only:
                self.app.display_debug(
                    f"Only ran build hooks for `{self.PLUGIN_NAME}` version `{version}`"
                )
                continue
    ...
    def recurse_project_files(self) -> Iterable[IncludedFile]:
        for root, dirs, files in safe_walk(self.root):
            ...
            dirs[:] = sorted(
                d
                for d in dirs
                if not self.config.directory_is_excluded(d, relative_path)
            )
            ...
            for f in files:
                ...
                distribution_path = self.config.get_distribution_path(
                    relative_file_path
                )
    ...
    def recurse_forced_files(
        self, inclusion_map: dict[str, str]
                ...
                for root, dirs, files in safe_walk(source):
                    ...
                    for f in files:
                        relative_file_path = os.path.join(
                            target_path, relative_directory, f
                        )
                        distribution_path = self.config.get_distribution_path(
                            relative_file_path
                        )
                ...
                for root, dirs, files in safe_walk(source):
                    ...
                    for f in files:
                        ...
                        if self.config.include_path(
                            relative_file_path, explicit=True, is_package=is_package
                        ):
                            yield IncludedFile(
                                os.path.join(root, f),
                                "" if external else relative_file_path,
    ...
    def plugin_manager(self) -> PluginManagerBound:
        if self.__plugin_manager is None:
            from puffling.plugin.manager import PluginManager
    ...
    def metadata(self) -> ProjectMetadata:
        if self.__metadata is None:
            ...
            self.__metadata = ProjectMetadata(
                self.root, self.plugin_manager, self.__raw_config
            )
    ...
    def app(self) -> Application:
        """
        An instance of [Application](../utilities.md#puffling.bridge.app.Application).
        """
    ...
    @property
    def puffling_config(self) -> dict[str, Any]:
        if self.__puffling_config is None:
            self.__puffling_config = self.metadata.puffling.config

        return self.__puffling_config

    @property
    def config(self) -> BuilderConfigBound:
        """
        An instance of [BuilderConfig](../utilities.md#puffling.builders.config.BuilderConfig).
        """
    ...
    def build_config(self) -> dict[str, Any]:
        ...
        if self.__build_config is None:
            self.__build_config = self.metadata.puffling.build_config
    ...
    def target_config(self) -> dict[str, Any]:
        """
        ```toml config-example
        [tool.puffling.build.targets.<PLUGIN_NAME>]
        ```
        """
        if self.__target_config is None:
            target_config: dict[str, Any] = self.metadata.puffling.build_targets.get(
                self.PLUGIN_NAME, {}
            )
            if not isinstance(target_config, dict):
                message = f"Field `tool.puffling.build.targets.{self.PLUGIN_NAME}` must be a table"
    ...
    def get_config_class(cls) -> type[BuilderConfig]:
        """
        Must return a subclass of [BuilderConfig](../utilities.md#puffling.builders.config.BuilderConfig).
        """
