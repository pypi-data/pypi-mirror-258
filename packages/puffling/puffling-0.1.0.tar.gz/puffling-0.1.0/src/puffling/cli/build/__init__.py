...
def build_impl(
    *,
        ...
        for item in config_settings:
            ...
            if key in config_settings_parsed:
                if not isinstance(config_settings_parsed[key], list):
                    config_settings_parsed[key] = [config_settings_parsed[key], item]
                else:
                    config_settings_parsed[key].append(item)
            else:
                config_settings_parsed[key] = item
    else:
    ...
    for i, (target_name, versions) in enumerate(target_data.items()):
        ...
        builder = builder_class(root, plugin_manager=plugin_manager, metadata=metadata, app=app.get_safe_application(), config_settings=config_settings)
