...
class PluginManager:
    ...
    def initialize(self) -> None:
        from puffling.plugin import specs
    ...
    def __getattr__(self, name: str) -> ClassRegister:
        ...
        hook_name = f"puffling_register_{name}"
        hook = getattr(self, hook_name, None)
        if hook:
            hook()

        register = ClassRegister(
            getattr(self.manager.hook, hook_name),
            "PLUGIN_NAME",
    ...
    def puffling_register_version_source(self) -> None:
        from puffling.version.source.plugin import hooks

        self.manager.register(hooks)

    def puffling_register_version_scheme(self) -> None:
        from puffling.version.scheme.plugin import hooks

        self.manager.register(hooks)

    def puffling_register_builder(self) -> None:
        from puffling.builders.plugin import hooks

        self.manager.register(hooks)

    def puffling_register_build_hook(self) -> None:
        from puffling.builders.hooks.plugin import hooks

        self.manager.register(hooks)

    def puffling_register_metadata_hook(self) -> None:
        from puffling.metadata.plugin import hooks
...
class ClassRegister:
    def __init__(
        ...
        identifier: str,
        third_party_plugins: ThirdPartyPlugins,
    ) -> None:
    ...
    def collect(self, *, include_third_party: bool = True) -> dict:
        ...
        for raw_registered_classes in self.registration_method():
            registered_classes = (
                raw_registered_classes
                if isinstance(raw_registered_classes, list)
                else [raw_registered_classes]
            )
