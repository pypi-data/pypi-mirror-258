...
def build_impl(
    *,
    ...
    from puffling.builders.constants import BuildEnvVars
    ...
    if config_settings:
        config_settings_parsed = {}
        for item in config_settings:
            if not "=" in item:
                app.abort(f"Invalid config setting `{item}` does not have a key.")
            key, value = item.split("=", 1)
            if key in config_settings_parsed:
                if not isinstance(config_settings_parsed[key], list):
                    config_settings_parsed[key] = [config_settings_parsed[key], value]
                else:
                    config_settings_parsed[key].append(value)
            else:
                config_settings_parsed[key] = value
    else:
        config_settings_parsed = None
    ...
    for i, (target_name, versions) in enumerate(target_data.items()):
        ...
        builder = builder_class(root, plugin_manager=plugin_manager, metadata=metadata, app=app.get_safe_application(), config_settings=config_settings_parsed)
...
def build_command(subparsers: argparse._SubParsersAction, defaults: Any) -> None:
    ...
    parser.add_argument(
        ...
        action='append',
        help='Additional config settings.',
    )
