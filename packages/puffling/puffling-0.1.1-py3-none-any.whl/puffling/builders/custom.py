...
from puffling.plugin.utils import load_plugin_from_script
from puffling.utils.constants import DEFAULT_BUILD_SCRIPT
...
class CustomBuilder(Generic[PluginManagerBound]):
    ...
    def __new__(  # type: ignore
        ...
        target_config = project_metadata.puffling.build_targets.get(cls.PLUGIN_NAME, {})
        if not isinstance(target_config, dict):
            message = (
                f"Field `tool.puffling.build.targets.{cls.PLUGIN_NAME}` must be a table"
            )
        ...
        hook_class = load_plugin_from_script(
            path, build_script, BuilderInterface, "builder"
        )  # type: ignore
        hook = hook_class(
            root,
