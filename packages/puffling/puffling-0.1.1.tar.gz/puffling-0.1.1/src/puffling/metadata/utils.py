...
# NOTE: this module should rarely be changed because it is likely to be used by other packages like Puffling
...
def is_valid_project_name(project_name: str) -> bool:
    ...
    return (
        re.search(
            "^([A-Z0-9]|[A-Z0-9][A-Z0-9._-]*[A-Z0-9])$", project_name, re.IGNORECASE
        )
        is not None
    )
...
def get_normalized_dependency(requirement: Requirement) -> str:
    ...
    if requirement.extras:
        requirement.extras = {
            normalize_project_name(extra) for extra in requirement.extras
        }
...
def resolve_metadata_fields(metadata: ProjectMetadata) -> dict[str, Any]:
    ...
    return {
        ...
        "readme": {
            "content-type": metadata.core.readme_content_type,
            "text": metadata.core.readme,
        },
