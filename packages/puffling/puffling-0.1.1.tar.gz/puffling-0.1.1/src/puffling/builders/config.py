...
from puffling.builders.constants import (
    DEFAULT_BUILD_DIRECTORY,
    EXCLUDED_DIRECTORIES,
    BuildEnvVars,
)
from puffling.builders.utils import (
    normalize_inclusion_map,
    normalize_relative_directory,
    normalize_relative_path,
)
from puffling.metadata.utils import normalize_project_name
from puffling.utils.fs import locate_file
...
class BuilderConfig:
    ...
    def include_path(
        self, relative_path: str, *, explicit: bool = False, is_package: bool = True
    ) -> bool:
        return (
    ...
    def directory_is_excluded(self, name: str, relative_path: str) -> bool:
        ...
        return (
            ...
            or (
                self.skip_excluded_dirs
                and self.path_is_excluded(f"{relative_directory}/")
            )
        )
    ...
    def include_spec(self) -> pathspec.GitIgnoreSpec | None:
        if self.__include_patterns is None:
            if "include" in self.target_config:
                ...
                include_location = (
                    f"tool.puffling.build.targets.{self.plugin_name}.include"
                )
            else:
                include_config = self.build_config
                include_location = "tool.puffling.build.include"
            ...
            for i, include_pattern in enumerate(include_patterns, 1):
                if not isinstance(include_pattern, str):
                    message = (
                        f"Pattern #{i} in field `{include_location}` must be a string"
                    )
                    raise TypeError(message)
            ...
            if all_include_patterns:
                self.__include_spec = pathspec.GitIgnoreSpec.from_lines(
                    all_include_patterns
                )
    ...
    def exclude_spec(self) -> pathspec.GitIgnoreSpec | None:
        if self.__exclude_patterns is None:
            if "exclude" in self.target_config:
                ...
                exclude_location = (
                    f"tool.puffling.build.targets.{self.plugin_name}.exclude"
                )
            else:
                exclude_config = self.build_config
                exclude_location = "tool.puffling.build.exclude"
            ...
            for i, exclude_pattern in enumerate(exclude_patterns, 1):
                if not isinstance(exclude_pattern, str):
                    message = (
                        f"Pattern #{i} in field `{exclude_location}` must be a string"
                    )
                    raise TypeError(message)
            ...
            if all_exclude_patterns:
                self.__exclude_spec = pathspec.GitIgnoreSpec.from_lines(
    ...
    def artifact_spec(self) -> pathspec.GitIgnoreSpec | None:
        if self.__artifact_patterns is None:
            if "artifacts" in self.target_config:
                ...
                artifact_location = (
                    f"tool.puffling.build.targets.{self.plugin_name}.artifacts"
                )
            else:
                artifact_config = self.build_config
                artifact_location = "tool.puffling.build.artifacts"
            ...
            for i, artifact_pattern in enumerate(artifact_patterns, 1):
                if not isinstance(artifact_pattern, str):
                    message = (
                        f"Pattern #{i} in field `{artifact_location}` must be a string"
                    )
                    raise TypeError(message)
            ...
            if all_artifact_patterns:
                self.__artifact_spec = pathspec.GitIgnoreSpec.from_lines(
                    all_artifact_patterns
                )
    ...
    def hook_config(self) -> dict[str, Any]:
        if self.__hook_config is None:
            ...
            if not isinstance(global_hook_config, dict):
                message = "Field `tool.puffling.build.hooks` must be a table"
                raise TypeError(message)

            for hook_name, config in global_hook_config.items():
                if not isinstance(config, dict):
                    message = (
                        f"Field `tool.puffling.build.hooks.{hook_name}` must be a table"
                    )
                    raise TypeError(message)
            ...
            if not isinstance(target_hook_config, dict):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.hooks` must be a table"
                raise TypeError(message)

            for hook_name, config in target_hook_config.items():
                if not isinstance(config, dict):
                    message = f"Field `tool.puffling.build.targets.{self.plugin_name}.hooks.{hook_name}` must be a table"
                    raise TypeError(message)
            ...
            if not env_var_enabled(BuildEnvVars.NO_HOOKS):
                ...
                final_hook_config = {
                    ...
                    if (
                        ...
                        or env_var_enabled(
                            f"{BuildEnvVars.HOOK_ENABLE_PREFIX}{hook_name.upper()}"
                        )
                    )
                }
            else:
        ...
        if self.__directory is None:
            if "directory" in self.target_config:
                ...
                if not isinstance(directory, str):
                    message = f"Field `tool.puffling.build.targets.{self.plugin_name}.directory` must be a string"
                    raise TypeError(message)
            else:
                directory = self.build_config.get("directory", DEFAULT_BUILD_DIRECTORY)
                if not isinstance(directory, str):
                    message = "Field `tool.puffling.build.directory` must be a string"
                    raise TypeError(message)
        ...
        if self.__skip_excluded_dirs is None:
            if "skip-excluded-dirs" in self.target_config:
                ...
                if not isinstance(skip_excluded_dirs, bool):
                    message = f"Field `tool.puffling.build.targets.{self.plugin_name}.skip-excluded-dirs` must be a boolean"
                    raise TypeError(message)
            else:
                skip_excluded_dirs = self.build_config.get("skip-excluded-dirs", False)
                if not isinstance(skip_excluded_dirs, bool):
                    message = "Field `tool.puffling.build.skip-excluded-dirs` must be a boolean"
                    raise TypeError(message)
        ...
        if self.__ignore_vcs is None:
            if "ignore-vcs" in self.target_config:
                ...
                if not isinstance(ignore_vcs, bool):
                    message = f"Field `tool.puffling.build.targets.{self.plugin_name}.ignore-vcs` must be a boolean"
                    raise TypeError(message)
            else:
                ignore_vcs = self.build_config.get("ignore-vcs", False)
                if not isinstance(ignore_vcs, bool):
                    message = "Field `tool.puffling.build.ignore-vcs` must be a boolean"
                    raise TypeError(message)
    ...
    def require_runtime_dependencies(self) -> bool:
        if self.__require_runtime_dependencies is None:
            if "require-runtime-dependencies" in self.target_config:
                require_runtime_dependencies = self.target_config[
                    "require-runtime-dependencies"
                ]
                if not isinstance(require_runtime_dependencies, bool):
                    message = (
                        f"Field `tool.puffling.build.targets.{self.plugin_name}.require-runtime-dependencies` "
                        f"must be a boolean"
                    )
                    raise TypeError(message)
            else:
                require_runtime_dependencies = self.build_config.get(
                    "require-runtime-dependencies", False
                )
                if not isinstance(require_runtime_dependencies, bool):
                    message = "Field `tool.puffling.build.require-runtime-dependencies` must be a boolean"
                    raise TypeError(message)
    ...
    def require_runtime_features(self) -> list[str]:
        if self.__require_runtime_features is None:
            if "require-runtime-features" in self.target_config:
                ...
                features_location = f"tool.puffling.build.targets.{self.plugin_name}.require-runtime-features"
            else:
                features_config = self.build_config
                features_location = "tool.puffling.build.require-runtime-features"

            require_runtime_features = features_config.get(
            ...
            for i, raw_feature in enumerate(require_runtime_features, 1):
                if not isinstance(raw_feature, str):
                    message = (
                        f"Feature #{i} of field `{features_location}` must be a string"
                    )
                    raise TypeError(message)
        ...
        if self.__only_packages is None:
            if "only-packages" in self.target_config:
                ...
                if not isinstance(only_packages, bool):
                    message = f"Field `tool.puffling.build.targets.{self.plugin_name}.only-packages` must be a boolean"
                    raise TypeError(message)
            else:
                only_packages = self.build_config.get("only-packages", False)
                if not isinstance(only_packages, bool):
                    message = (
                        "Field `tool.puffling.build.only-packages` must be a boolean"
                    )
                    raise TypeError(message)
        ...
        if self.__reproducible is None:
            if "reproducible" in self.target_config:
                ...
                if not isinstance(reproducible, bool):
                    message = f"Field `tool.puffling.build.targets.{self.plugin_name}.reproducible` must be a boolean"
                    raise TypeError(message)
            else:
                reproducible = self.build_config.get("reproducible", True)
                if not isinstance(reproducible, bool):
                    message = (
                        "Field `tool.puffling.build.reproducible` must be a boolean"
                    )
                    raise TypeError(message)
        ...
        if self.__dev_mode_dirs is None:
            if "dev-mode-dirs" in self.target_config:
                ...
                dev_mode_dirs_location = (
                    f"tool.puffling.build.targets.{self.plugin_name}.dev-mode-dirs"
                )
            else:
                dev_mode_dirs_config = self.build_config
                dev_mode_dirs_location = "tool.puffling.build.dev-mode-dirs"
            ...
            if not isinstance(dev_mode_dirs, list):
                message = (
                    f"Field `{dev_mode_dirs_location}` must be an array of strings"
                )
                raise TypeError(message)
        ...
        if self.__dev_mode_exact is None:
            if "dev-mode-exact" in self.target_config:
                ...
                if not isinstance(dev_mode_exact, bool):
                    message = f"Field `tool.puffling.build.targets.{self.plugin_name}.dev-mode-exact` must be a boolean"
                    raise TypeError(message)
            else:
                dev_mode_exact = self.build_config.get("dev-mode-exact", False)
                if not isinstance(dev_mode_exact, bool):
                    message = (
                        "Field `tool.puffling.build.dev-mode-exact` must be a boolean"
                    )
                    raise TypeError(message)
    ...
    def versions(self) -> list[str]:
        if self.__versions is None:
            ...
            if not isinstance(versions, list):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.versions` must be an array of strings"
                raise TypeError(message)

            for i, version in enumerate(versions, 1):
                if not isinstance(version, str):
                    message = f"Version #{i} in field `tool.puffling.build.targets.{self.plugin_name}.versions` must be a string"
                    raise TypeError(message)

                if not version:
                    message = (
                        f"Version #{i} in field `tool.puffling.build.targets.{self.plugin_name}.versions` "
            ...
            if not all_versions:
                ...
                unknown_versions = set(all_versions) - set(
                    self.__builder.get_version_api()
                )
                if unknown_versions:
                    message = (
                        f'Unknown versions in field `tool.puffling.build.targets.{self.plugin_name}.versions`: '
    ...
    def dependencies(self) -> list[str]:
        if self.__dependencies is None:
            ...
            if not isinstance(target_dependencies, list):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.dependencies` must be an array"
                raise TypeError(message)

            for i, dependency in enumerate(target_dependencies, 1):
                if not isinstance(dependency, str):
                    message = (
                        f"Dependency #{i} of field `tool.puffling.build.targets.{self.plugin_name}.dependencies` "
                        f"must be a string"
                    )
                    raise TypeError(message)
            ...
            if not isinstance(global_dependencies, list):
                message = "Field `tool.puffling.build.dependencies` must be an array"
                raise TypeError(message)

            for i, dependency in enumerate(global_dependencies, 1):
                if not isinstance(dependency, str):
                    message = f"Dependency #{i} of field `tool.puffling.build.dependencies` must be a string"
                    raise TypeError(message)
            ...
            for hook_name, config in self.hook_config.items():
                hook_require_runtime_dependencies = config.get(
                    "require-runtime-dependencies", False
                )
                ...
                hook_require_runtime_features = config.get(
                    "require-runtime-features", []
                )
            ...
            if require_runtime_features:
                for feature in require_runtime_features:
                    for dependency in self.builder.metadata.core.optional_dependencies[
                        feature
                    ]:
                        dependencies[dependency] = None
    ...
    def sources(self) -> dict[str, str]:
        if self.__sources is None:
            if "sources" in self.target_config:
                ...
                sources_location = (
                    f"tool.puffling.build.targets.{self.plugin_name}.sources"
                )
            else:
                sources_config = self.build_config
                sources_location = "tool.puffling.build.sources"
            ...
            if isinstance(raw_sources, list):
                ...
                for source, path in raw_sources.items():
                    ...
                    sources[
                        normalize_relative_directory(source) if source else source
                    ] = normalized_path
            else:
                message = (
                    f"Field `{sources_location}` must be a mapping or array of strings"
                )
                raise TypeError(message)
            ...
            for relative_path in self.packages:
                ...
                if (
                    source
                    and normalize_relative_directory(relative_path) not in sources
                ):
                    sources[normalize_relative_directory(source)] = ""
    ...
    def packages(self) -> list[str]:
        if self.__packages is None:
            if "packages" in self.target_config:
                ...
                package_location = (
                    f"tool.puffling.build.targets.{self.plugin_name}.packages"
                )
            else:
                package_config = self.build_config
                package_location = "tool.puffling.build.packages"
            ...
            for i, package in enumerate(packages, 1):
                if not isinstance(package, str):
                    message = (
                        f"Package #{i} in field `{package_location}` must be a string"
                    )
                    raise TypeError(message)
            ...
            self.__packages = sorted(
                normalize_relative_path(package) for package in packages
            )
    ...
    def force_include(self) -> dict[str, str]:
        if self.__force_include is None:
            if "force-include" in self.target_config:
                ...
                force_include_location = (
                    f"tool.puffling.build.targets.{self.plugin_name}.force-include"
                )
            else:
                force_include_config = self.build_config
                force_include_location = "tool.puffling.build.force-include"
    ...
    def only_include(self) -> dict[str, str]:
        if self.__only_include is None:
            if "only-include" in self.target_config:
                ...
                only_include_location = (
                    f"tool.puffling.build.targets.{self.plugin_name}.only-include"
                )
            else:
                only_include_config = self.build_config
                only_include_location = "tool.puffling.build.only-include"

            only_include = (
                only_include_config.get("only-include", self.default_only_include())
                or self.packages
            )
            ...
            for i, relative_path in enumerate(only_include, 1):
                if not isinstance(relative_path, str):
                    message = (
                        f"Path #{i} in field `{only_include_location}` must be a string"
                    )
                    raise TypeError(message)
    ...
    def set_build_data(self, build_data: dict[str, Any]) -> Generator:
        try:
            ...
            if build_artifacts:
                self.build_artifact_spec = pathspec.GitIgnoreSpec.from_lines(
                    build_artifacts
                )

            self.build_force_include.update(
                normalize_inclusion_map(build_data["force_include"], self.root)
            )
            ...
            for inclusion_map in (self.force_include, self.build_force_include):
                for source, target in inclusion_map.items():
                    ...
                    if source.startswith(f"{self.root}{os.sep}"):
                        self.build_reserved_paths.add(
                            self.get_distribution_path(
                                os.path.relpath(source, self.root)
                            )
                        )
                    ...
                    # ../out.ext -> ../in.ext
                        ...
                        self.build_reserved_paths.add(
                            self.get_distribution_path(target)
                        )
