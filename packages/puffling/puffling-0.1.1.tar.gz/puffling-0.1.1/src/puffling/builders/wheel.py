...
from puffling.__about__ import __version__
from puffling.builders.config import BuilderConfig
from puffling.builders.constants import EDITABLES_REQUIREMENT
...
from puffling.metadata.spec import (
...
if TYPE_CHECKING:
    ...
    from puffling.builders.plugin.interface import IncludedFile
...
class RecordFile:
    def __init__(self) -> None:
        ...
        self.__writer = csv.writer(
            self.__file_obj, delimiter=",", quotechar='"', lineterminator="\n"
        )
...
class WheelArchive:
    ...
    def add_file(self, included_file: IncludedFile) -> tuple[str, str, str]:
        ...
        with open(included_file.path, "rb") as in_file, self.zf.open(
            zip_info, "w"
        ) as out_file:
            while True:
    ...
    def write_metadata(
        self, relative_path: str, contents: str | bytes
    ) -> tuple[str, str, str]:
        relative_path = (
    ...
    def add_extra_metadata_file(
        self, extra_metadata_file: IncludedFile
    ) -> tuple[str, str, str]:
        extra_metadata_file.distribution_path = f"{self.metadata_directory}/extra_metadata/{extra_metadata_file.distribution_path}"
...
class WheelBuilderConfig(BuilderConfig):
    ...
    def default_file_selection_options(self) -> FileSelectionOptions:
        include = self.target_config.get(
            "include", self.build_config.get("include", [])
        )
        exclude = self.target_config.get(
            "exclude", self.build_config.get("exclude", [])
        )
        packages = self.target_config.get(
            "packages", self.build_config.get("packages", [])
        )
        only_include = self.target_config.get(
            "only-include", self.build_config.get("only-include", [])
        )
        ...
        for project_name in (
            self.builder.normalize_file_name_component(
            ...
            if os.path.isfile(os.path.join(self.root, project_name, "__init__.py")):
                normalized_project_name = self.get_raw_fs_path_name(
                    self.root, project_name
                )
                return FileSelectionOptions([], exclude, [normalized_project_name], [])

            if os.path.isfile(
                os.path.join(self.root, "src", project_name, "__init__.py")
            ):
                normalized_project_name = self.get_raw_fs_path_name(
                    os.path.join(self.root, "src"), project_name
                )
                return FileSelectionOptions(
                    [], exclude, [f"src/{normalized_project_name}"], []
                )
        ...
        message = (
            ...
            f"At least one file selection option must be defined in the `tool.puffling.build.targets.wheel` "
            ...
            f"[tool.puffling.build.targets.wheel]\n"
            f'packages = ["src/foo"]'
        )
    ...
    def core_metadata_constructor(self) -> Callable[..., str]:
        if self.__core_metadata_constructor is None:
            ...
            if not isinstance(core_metadata_version, str):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.core-metadata-version` must be a string"
            ...
            if core_metadata_version not in constructors:
                message = (
                    ...
                    f'`tool.puffling.build.targets.{self.plugin_name}.core-metadata-version`. '
    ...
    def shared_data(self) -> dict[str, str]:
        if self.__shared_data is None:
            ...
            if not isinstance(shared_data, dict):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.shared-data` must be a mapping"
                raise TypeError(message)

            for i, (source, relative_path) in enumerate(shared_data.items(), 1):
                if not source:
                    message = (
                        f"Source #{i} in field `tool.puffling.build.targets.{self.plugin_name}.shared-data` "
                ...
                if not isinstance(relative_path, str):
                    message = (
                        ...
                        f"`tool.puffling.build.targets.{self.plugin_name}.shared-data` must be a string"
                    )
                    raise TypeError(message)
                ...
                if not relative_path:
                    message = (
                        f"Path for source `{source}` in field "
                        f"`tool.puffling.build.targets.{self.plugin_name}.shared-data` cannot be an empty string"
                    )
                    raise ValueError(message)
    ...
    def extra_metadata(self) -> dict[str, str]:
        if self.__extra_metadata is None:
            ...
            if not isinstance(extra_metadata, dict):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.extra-metadata` must be a mapping"
                raise TypeError(message)

            for i, (source, relative_path) in enumerate(extra_metadata.items(), 1):
                if not source:
                    message = (
                        f"Source #{i} in field `tool.puffling.build.targets.{self.plugin_name}.extra-metadata` "
                        f"cannot be an empty string"
                    )
                    raise ValueError(message)
                ...
                if not isinstance(relative_path, str):
                    message = (
                        f"Path for source `{source}` in field "
                        f"`tool.puffling.build.targets.{self.plugin_name}.extra-metadata` must be a string"
                    )
                    raise TypeError(message)
                ...
                if not relative_path:
                    message = (
                        f"Path for source `{source}` in field "
                        f"`tool.puffling.build.targets.{self.plugin_name}.extra-metadata` cannot be an empty string"
                    )
                    raise ValueError(message)
        ...
        if self.__strict_naming is None:
            if "strict-naming" in self.target_config:
                ...
                if not isinstance(strict_naming, bool):
                    message = f"Field `tool.puffling.build.targets.{self.plugin_name}.strict-naming` must be a boolean"
                    raise TypeError(message)
            else:
                strict_naming = self.build_config.get("strict-naming", True)
                if not isinstance(strict_naming, bool):
                    message = (
                        "Field `tool.puffling.build.strict-naming` must be a boolean"
                    )
                    raise TypeError(message)
    ...
    def macos_max_compat(self) -> bool:
        if self.__macos_max_compat is None:
            ...
            if not isinstance(macos_max_compat, bool):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.macos-max-compat` must be a boolean"
                raise TypeError(message)
    ...
    def bypass_selection(self) -> bool:
        ...
        if not isinstance(bypass_selection, bool):
            message = f"Field `tool.puffling.build.targets.{self.plugin_name}.bypass-selection` must be a boolean"
            raise TypeError(message)
...
class WheelBuilder(BuilderInterface):
    ...
    def build_standard(self, directory: str, **build_data: Any) -> str:
        ...
        target = os.path.join(
            directory, f"{self.artifact_project_id}-{build_data['tag']}.whl"
        )
        ...
        with WheelArchive(
            ...
            for included_file in self.recurse_selected_project_files():
                ...
                if len(path_parts) == 1:  # no cov
                    exposed_packages[os.path.splitext(relative_path)[0]] = os.path.join(
            ...
            for included_file in self.recurse_forced_files(
                self.get_forced_inclusion_map(build_data)
            ):
                record = archive.add_file(included_file)
                records.write(record)
    ...
    def build_editable_explicit(self, directory: str, **build_data: Any) -> str:
        ...
        with WheelArchive(
            ...
            record = archive.write_file(
                f"_{self.metadata.core.name.replace('-', '_')}.pth",
                "\n".join(directories),
            )
            records.write(record)
    ...
    def write_data(
        ...
        self.write_metadata(
            archive, records, build_data, extra_dependencies=extra_dependencies
        )
    ...
    def write_metadata(
        ...
        self.write_project_metadata(
            archive, records, extra_dependencies=extra_dependencies
        )
        ...
        self.write_config_setting_metadata(archive, records)
    ...
    def write_archive_metadata(
        archive: WheelArchive, records: RecordFile, build_data: dict[str, Any]
    ) -> None:
        from packaging.tags import parse_tag

        metadata = f"""\
Wheel-Version: 1.0
Generator: puffling {__version__}
...
"""
    ...
    def write_entry_points_file(
        self, archive: WheelArchive, records: RecordFile
    ) -> None:
    ...
    def write_project_metadata(
        ...
        record = archive.write_metadata(
            "METADATA",
            self.config.core_metadata_constructor(
                self.metadata, extra_dependencies=extra_dependencies
            ),
        )
        records.write(record)

    def write_config_setting_metadata(
        self,
        archive: WheelArchive,
        records: RecordFile,
    ) -> None:
        if self.config_settings:
            record = archive.write_metadata(
                "CONFIG-SETTINGS",
                json.dumps(self.config_settings, indent=2),
            )
            records.write(record)
    ...
    def add_extra_metadata(
        ...
        extra_metadata.update(
            normalize_inclusion_map(build_data["extra_metadata"], self.root)
        )
        ...
        if sys.platform == "darwin":
            if archflags and sys.version_info[:2] >= (3, 8):
                ...
                if archs:
                    ...
                    new_arch = (
                        "universal2" if set(archs) == {"x86_64", "arm64"} else archs[0]
                    )
