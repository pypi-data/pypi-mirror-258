    ...
    def scripts(self) -> list[str]:
        if self.__scripts is None:
            ...
            if not isinstance(scripts, list):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.scripts` must be an array"
                raise TypeError(message)

            for i, script in enumerate(scripts, 1):
                if not isinstance(script, str):
                    message = f"Script #{i} of field `tool.puffling.build.targets.{self.plugin_name}.scripts` must be a string"
                    raise TypeError(message)

                if script not in known_scripts:
                    message = f"Unknown script in field `tool.puffling.build.targets.{self.plugin_name}.scripts`: {script}"
    ...
    def python_version(self) -> str:
        if self.__python_version is None:
            ...
            if not isinstance(python_version, str):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.python-version` must be a string"
                raise TypeError(message)

            if not python_version and "PYAPP_DISTRIBUTION_SOURCE" not in os.environ:
                for supported_version in self.SUPPORTED_VERSIONS:
                    if self.builder.metadata.core.python_constraint.contains(
                        supported_version
                    ):
    ...
    def pyapp_version(self) -> str:
        if self.__pyapp_version is None:
            ...
            if not isinstance(pyapp_version, str):
                message = f"Field `tool.puffling.build.targets.{self.plugin_name}.pyapp-version` must be a string"
                raise TypeError(message)
