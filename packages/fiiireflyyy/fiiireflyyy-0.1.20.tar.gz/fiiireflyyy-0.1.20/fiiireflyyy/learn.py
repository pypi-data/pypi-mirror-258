...
def test_clf_by_confusion(clf, test_dataset: pd.DataFrame, training_targets: tuple, return_data=False, **kwargs):
    ...
    if return_data:
        return overall_matrix, mixed_labels_matrix, CORRESPONDENCE
    ...
    ax.set_xticks([CORRESPONDENCE[x] + 0.5 for x in options['testing_targets']], options['testing_targets'])
    ax.set_yticks([CORRESPONDENCE[x] + 0.5 for x in training_targets], training_targets)
    plt.tight_layout()
