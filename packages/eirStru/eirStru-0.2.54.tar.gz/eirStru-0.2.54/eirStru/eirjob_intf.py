...
class JobIntf:
    ...
    async def quote_spot(self, session: SessionData, params: SpotParams):
        url = f'{self.host}/quote_spot'
        data = {'session': session, 'params': params}
        headers = {
            'accept': 'application/json',
            'Content-Type': 'application/json',
        }
        try:
            async with ClientSession() as cs:
                async with cs.post(url, headers=headers, data=json.dumps(data), verify_ssl=False) as resp:
                    r_json = await resp.json()
                    return ResponseData(**r_json)
        except Exception as e:
