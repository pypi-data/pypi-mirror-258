...
class LeftNavigationHandler(ClientBaseHandler):
    ...
    def get_async(self):
        """
        :description: 左侧导航栏
        :param is_multiple: 是否同时实例化多个模板,显示过个模板列表
        :return:
        ...
        invoke_result_data = app_base_model.get_left_navigation(self.get_user_nick(), access_token, app_key, app_secret, self.get_app_id(), is_multiple)
        if invoke_result_data.success == False:
