...
Ext.app.App = Ext.extend(Ext.util.Observable, {
    ...
    checkHeights: function () {
        // Изменяет размер иконок в зависимости от количества текста

        function addNewHover(cellHeight) {
            // Изменяет размер картинки заднего плана (block:hover)
            let head = document.getElementsByTagName('head')[0];
            let style = document.createElement('style');
            let declarations = document.createTextNode(
                `#x-shortcuts td a .block:hover{
                background:url( ../${params.desktopItemBackgroundUrl} ) no-repeat 0 -${cellHeight}px;
        ...
        let blocks = Ext.select('.block', true),
            blockElements = blocks.elements,
            textHeight = 0;
        if (blockElements.length === 0) return;

        let cellHeight = blocks.last().getHeight();

        const baseTextHeight = 134,
              step = 15;

        for (let i = 0; i < blockElements.length; i++) {
            textHeight = Math.max(textHeight, blockElements[i].query('.text')[0].clientHeight);
            if (textHeight >= cellHeight) {
                while (cellHeight <= textHeight) {
                    cellHeight += step;
                }
            } else if (baseTextHeight >= textHeight) {
                while (cellHeight > baseTextHeight && cellHeight > 149) {
                    cellHeight -= step;
                }
            }
        }

        Ext.each(blockElements, function (block) {
            block.setStyle({
                'background-size': '100% 200%',
            })
            block.select('.base-desktop-image').item(0).setStyle({
                'height': `${cellHeight}px`
            })
        });
        addNewHover(cellHeight);
    },
