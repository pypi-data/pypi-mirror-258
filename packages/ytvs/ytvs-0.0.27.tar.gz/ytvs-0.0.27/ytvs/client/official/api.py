...
class OfficialApi(object):
    ...
    def __init__(
        ...
        quota_reset_time='00:00',  # 쿼터 리셋 시간 (태평양 표준시 기준 자정)
        cool_down_period=3600,  # API 키 만료 후 대기 시간 (초 단위)
        ...
        self.api_key_usage = {key: {"expired":False, "reset_time": None} for key in api_keys}  # API 키 사용량 기록
        self.quota_reset_time = quota_reset_time
        self.cool_down_period = cool_down_period
        self.last_quota_reset = None
        self.next_key_switch_time = None  # 다음 API 키로 전환할 시간
    ...
    def _update_key_usage(self):
        """태평양 표준시 자정에 모든 키의 만료 상태를 초기화합니다."""
        pst_now = datetime.datetime.now(pytz.timezone("Asia/Seoul"))
        for key, usage in self.api_key_usage.items():
            if usage["reset_time"] is None or pst_now >= usage["reset_time"]:
                usage["expired"] = False
                next_day = pst_now + datetime.timedelta(days=1)
                reset_time = datetime.datetime(year=next_day.year, month=next_day.month, day=next_day.day, hour=0, minute=0, second=0, tzinfo=pytz.timezone("America/Los_Angeles"))
