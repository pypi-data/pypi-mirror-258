...
def cat_plot(data: pd.DataFrame, target: str, cat_cols: list, figsize: tuple=(8, 4), stacked: bool=True):
    """
    data: dataframe
    ...
    for i in cat_cols:
        if i!=target:
            (pd.crosstab(data[i],data[target],normalize='index')*100).plot(kind='bar',figsize=figsize,stacked=stacked)
            plt.ylabel(target)
            plt.show()
