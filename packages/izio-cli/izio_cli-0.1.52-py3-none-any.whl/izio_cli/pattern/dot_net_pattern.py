...
from izio_cli.values.dot_net_payload import envPayload, responseArrayInterfacePayload, responseArrayPayload, responseInterfacePayload, responsePayload
...
def createNetCoreSolution(path, projectName, solution, console):
    with console.status(
        ...
        if projects:
            ...
            create_file(f"{path}{os.sep}{projectName}.Application{os.sep}Dtos{os.sep}Responses", "Response.cs", payload=responsePayload())
            create_file(f"{path}{os.sep}{projectName}.Application{os.sep}Dtos{os.sep}Responses", "ResponseArray.cs", payload=responseArrayPayload())
            
            create_file(f"{path}{os.sep}{projectName}.Application{os.sep}Interfaces", "IResponseArray.cs", payload=responseInterfacePayload())
            create_file(f"{path}{os.sep}{projectName}.Application{os.sep}Interfaces", "IResponse.cs", payload=responseArrayInterfacePayload())
...
def create_entity(path, name, console: Console):
    with console.status("Creating a new entity", spinner="arc", spinner_style="bold green") as status:

        create_file(
            f"{path}{os.sep}Domain{os.sep}Entities", f"{name.capitalize}.cs", payload=f"// Todo: implement {name.capitalize}.cs"
        )
        status.update(status="Creating Domain Interfaces")
        create_file(
            f"{path}{os.sep}Domain{os.sep}Interfaces",
            f"I{name.capitalize}AppDataAccess.cs",
            payload=f"// Todo: implement {name.capitalize}AppDataAccess.cs",
        )
        create_file(
        f"{path}{os.sep}Domain{os.sep}Interfaces",
        f"I{name.capitalize}OfficeDataAccess.cs",
        payload=f"// Todo: implement {name.capitalize}OfficeDataAccess.cs",
    )
        status.update(status="Creating Domain Validations")
        create_file(
        f"{path}{os.sep}Domain{os.sep}Validations",
        "DomainExeptionValidation.cs",
        payload="// Todo: implement DomainExeptionValidation",
    )
        status.update(status="Creating Application Dtos")
        create_file(
            f"{path}{os.sep}Application{os.sep}Dtos", f"{name.capitalize}App.cs", payload=f"// Todo: implement {name.capitalize}App.cs"
        )
        create_file(
            f"{path}{os.sep}Application{os.sep}Dtos",
            f"{name.capitalize}Office.cs",
            payload=f"// Todo: implement {name.capitalize}Office.cs",
        )
        status.update(status="Creating Application Interfaces")
        create_file(
            f"{path}{os.sep}Application{os.sep}Interfaces",
            f"I{name.capitalize}AppServices.cs",
            payload=f"// Todo: implement {name.capitalize}AppServices.cs",
        )
        create_file(
            f"{path}{os.sep}Application{os.sep}Interfaces",
            f"I{name.capitalize}OfficeServices.cs",
            payload=f"// Todo: implement {name.capitalize}OfficeServices.cs",
        )
        ...
        create_file(
            ...
            f"{name.capitalize}AppServices.cs",
            payload="// Todo: implement {name.capitalize}AppServices.cs",
        )
        create_file(
            f"{path}{os.sep}Application{os.sep}Services",
            f"{name.capitalize}OfficeServices.cs",
            payload=f"// Todo: implement {name.capitalize}OfficeServices.cs",
        )
        console.print(
    ...
    create_file(
        ...
        f"{name.capitalize}AppDataAccess.cs",
        payload="// Todo: implement",
    )
    create_file(
        f"{path}{os.sep}Application{os.sep}Infra.DataAccess{os.sep}Repository",
        f"{name.capitalize}OfficeDataAccess.cs",
        payload="// Todo: implement",
    )