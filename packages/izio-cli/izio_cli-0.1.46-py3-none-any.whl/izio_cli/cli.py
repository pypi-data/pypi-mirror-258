...
def new_project(
    ...
    if (
        ...
        if type == "flutter":
            ...
            # create first entity?
            new_entity = inquirer.confirm(
                "Do you want to create a new entity?", default=True
            )
            if new_entity:
                new_entity(path, projectName)
...
def new_entity(
    ...
    if ".sln" not in os.listdir(path):
        raise FileNotFoundError("This command must be run from a .NET project")

    create_entity(path, name)
