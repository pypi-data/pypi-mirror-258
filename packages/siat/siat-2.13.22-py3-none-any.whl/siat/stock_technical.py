...
def security_bollinger(ticker,fromdate,todate,boll_days=20, \
    ...
    draw_lines2(df1,y_label,x_label,axhline_value,axhline_label,title_txt, \
               ...
               colorlist=colorlist,lslist=lslist,lwlist=lwlist,band_area=['上(压力)线','下(支撑)线'])
...
def security_technical(ticker,start='default',end='default', \
    ...
    # 检查日期：截至日期
    import datetime as dt; todaystr=dt.date.today().strftime("%Y-%m-%d")
    end=end.lower()
    if end in ['default','today']:
        todate=todaystr
    else:
        ...
        if not validdate:
            print("  #Warning(security_technical): invalid date for",end)
            todate=todaystr
    ...
    if start in ['default','mrm','l1m']:  # 默认近一个月
        ...
        if not validdate:
            print("  #Warning(security_technical): invalid date for",start,"/b, set to MRM")
