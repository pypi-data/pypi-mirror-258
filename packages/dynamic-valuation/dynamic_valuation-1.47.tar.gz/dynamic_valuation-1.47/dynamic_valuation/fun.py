...
class Solow:
    ...
    def __init__(self,
                ...
                n=int(1e2),policy_bound=int(1e8),
                 freq=1,
                 δ=1e-1,
                 A=1,
                 mps=.5, alpha=.3,ϕ=0, s_min=1e-1, s_max = 9):
        self.A=A
        self.mps=mps
        self.alpha=alpha
        self.ϕ=ϕ
    ...
    def x(self,s):
        return (1-self.mps)*s
    def sdot(self, s,x):
        return self.A *(s-x)**self.alpha-self.ϕ*s-x
...
def T(cl=so_low,v=[],s_grid=[],policy=None):
  if not list(s_grid):
    s_grid=cl.s_grid
  if not list(v):
     v=0*s_grid
