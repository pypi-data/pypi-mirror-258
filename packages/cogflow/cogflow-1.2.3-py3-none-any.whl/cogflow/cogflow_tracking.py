...
from cogflow.cogclient import Cog_Client
...
def end_run(status: str = RunStatus.to_string(RunStatus.FINISHED)) -> None:
    ...
    if len(_active_run_stack) > 0:
        ...
        Cog_Client().set_terminated(_last_active_run_id, status)
...
def active_run() -> Optional[ActiveRun]:
    ...
    **Note**: You cannot access currently-active run attributes
    ...
    to access such attributes, use the :py:class:`mlflow.client.Cog_Client` as follows:
...
def get_run(run_id: str) -> Run:
    ...
    """
    return Cog_Client().get_run(run_id)
...
def get_parent_run(run_id: str) -> Optional[Run]:
    ...
    """
    return Cog_Client().get_parent_run(run_id)
...
def log_param(key: str, value: Any, synchronous: bool = True) -> Any:
    ...
    return Cog_Client().log_param(run_id, key, value, synchronous=synchronous)
...
def set_experiment_tag(key: str, value: Any) -> None:
    ...
    Cog_Client().set_experiment_tag(experiment_id, key, value)
...
def set_tag(key: str, value: Any, synchronous: bool = True) -> Optional[RunOperations]:
    ...
    """
    run_id = _get_or_start_run().info.run_id
    return Cog_Client().set_tag(run_id, key, value, synchronous=synchronous)
...
def delete_tag(key: str) -> None:
    ...
    """
    run_id = _get_or_start_run().info.run_id
    Cog_Client().delete_tag(run_id, key)
...
def log_input(
    ...
    Cog_Client().log_inputs(run_id=run_id, datasets=[dataset_input])
...
def log_artifact(
    ...
    """
    run_id = run_id or _get_or_start_run().info.run_id
    Cog_Client().log_artifact(run_id, local_path, artifact_path)
...
def log_artifacts(
    ...
    """
    run_id = run_id or _get_or_start_run().info.run_id
    Cog_Client().log_artifacts(run_id, local_dir, artifact_path)
...
def log_text(text: str, artifact_file: str, run_id: Optional[str] = None) -> None:
    ...
    """
    run_id = run_id or _get_or_start_run().info.run_id
    Cog_Client().log_text(run_id, text, artifact_file)
...
def log_dict(dictionary: Dict[str, Any], artifact_file: str, run_id: Optional[str] = None) -> None:
    ...
    """
    run_id = run_id or _get_or_start_run().info.run_id
    Cog_Client().log_dict(run_id, dictionary, artifact_file)
...
def log_figure(
    ...
    *,
    ...
    """
    run_id = _get_or_start_run().info.run_id
    Cog_Client().log_figure(run_id, figure, artifact_file, save_kwargs=save_kwargs)
...
def log_image(image: Union["numpy.ndarray", "PIL.Image.Image"], artifact_file: str) -> None:
    ...
    """
    run_id = _get_or_start_run().info.run_id
    Cog_Client().log_image(run_id, image, artifact_file)
...
def log_table(
    ...
    """
    run_id = run_id or _get_or_start_run().info.run_id
    Cog_Client().log_table(run_id, data, artifact_file)
...
def load_table(
    ...
    """
    experiment_id = _get_experiment_id()
    return Cog_Client().load_table(experiment_id, artifact_file, run_ids, extra_columns)


def _record_logged_model(mlflow_model, run_id=None):
    run_id = run_id or _get_or_start_run().info.run_id
    Cog_Client()._record_logged_model(run_id, mlflow_model)
...
def get_experiment(experiment_id: str) -> Experiment:
    ...
    """
    return Cog_Client().get_experiment(experiment_id)
...
def get_experiment_by_name(name: str) -> Optional[Experiment]:
    ...
    .. code-block:: text
        ...
        Creation timestamp: 1662004217511
    """
    return Cog_Client().get_experiment_by_name(name)
...
def search_experiments(
    ...
    def pagination_wrapper_func(number_to_get, next_page_token):
        return Cog_Client().search_experiments(
            view_type=view_type,
...
def create_experiment(
    ...
    .. code-block:: text
        ...
        Creation timestamp: 1662004217511
    """
    return Cog_Client().create_experiment(name, artifact_location, tags)
...
def delete_experiment(experiment_id: str) -> None:
    ...
    """
    Cog_Client().delete_experiment(experiment_id)
...
def _get_experiment_id_from_env():
    ...
    if experiment_name is not None:
        exp = Cog_Client().get_experiment_by_name(experiment_name)
        if exp:
            ...
            return Cog_Client().create_experiment(name=experiment_name)
    if experiment_id is not None:
        try:
            exp = Cog_Client().get_experiment(experiment_id)
            return exp.experiment_id
...
def autolog(
    ...
    .. testcode:: python
        ...
        def print_auto_logged_info(r):
            ...
            artifacts = [f.path for f in Cog_Client().list_artifacts(r.info.run_id, "model")]
