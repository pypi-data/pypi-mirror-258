...
class WebullOptions:
            ...
            async with sema:
                try:
                    ...
                    async with session.post(url, data=json.dumps(params)) as resp:
                        ...
                        asyncio.create_task(self.db_manager.batch_insert_dataframe(option_data.as_dataframe, table_name='wb_opts', unique_columns='option_symbol', batch_size=5000))
    ...
    async def option_flow(self, option_id):   
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            ...
            df = pd.DataFrame(data_dict, index=['option_id'])

            await self.db_manager.batch_insert_dataframe(df, table_name='wb_flow', unique_columns='option_id')
