...
def main(thisModel=""):
    ...
    if thisModel:
        ...
        if args.model and args.model.strip():
            ...
            bottom_toolbar = f""" {str(config.hotkey_exit).replace("'", "")} {config.exit_entry}"""
            option = HealthCheck.simplePrompt(style=promptStyle, promptSession=model_session, bottom_toolbar=bottom_toolbar, default=prompt, accept_default=True, completer=completer)
