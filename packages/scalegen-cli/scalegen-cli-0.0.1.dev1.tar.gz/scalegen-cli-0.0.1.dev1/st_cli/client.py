...
def get_new_access_token(client_id, client_secret) -> str:
    ...
    resp = requests.post(f"{AUTH0_TENANT}/oauth/token", json=data, headers={'content-type' : 'application/json'})
    # print(resp.json())
...
def store_creds(client_id, client_secret):
...
def read_creds():
    if not os.path.exists(CACHE_PATH):
        click.echo("Could not perform operation. Please try logging in again.", err=True)
        exit(1)
...
def send_request(method, end_point, data=None):
    ...
    resp = requests.request(method, url, headers=header, json=data)
