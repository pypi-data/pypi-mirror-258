...
def start(job_id):
    ...
    if resp.status_code == 200:
        click.echo(click.style(f"Visualization server is starting. Please run `{'scaletorch' if PRODUCT_TYPE=='SCALETORCH' else 'scalegen finetune'} view {job_id}` to view the visualisation server's URL.", fg="green"))
    else:
        err = resp.json() if resp.status_code != 500 else resp.content.decode('utf-8')
        click.echo(click.style(f"Could not start visualisation server: {resp.json()}", fg="red"), err=True)
...
def stop(job_id):
    ...
    if resp.status_code == 200:
        ...
        err = resp.json() if resp.status_code != 500 else resp.content.decode('utf-8')
        click.echo(click.style(f"Could not stop visualisation server: {resp.json()}", fg="red"), err=True)
