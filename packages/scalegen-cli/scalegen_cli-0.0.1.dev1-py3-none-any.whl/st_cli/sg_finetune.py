...
def find_next_word_in_string(imput: str, target: str) -> Optional[str]:
    list_of_words = imput.split()
    try:
...
def finetune():
    """
    Commands for managing fine-tuning deployment for different ScaleGen tasks
    """
    pass
...
@click.option('--task', type=click.Choice(['llm', 'seq2seq', 'text_classification']), required=False, default='llm')
...
@click.option("--data_path", required=True, type=click.STRING, help="Data path to use. Can be a hugging face dataset or name of a virtual mount (filename is set with user_dataset)")
@click.option('--user_dataset', required=False, type=click.STRING, help="Filename of a dataset on virtual mount to use")
@click.option("--train_subset", required=False, type=click.STRING, help="Training data subset to use")
@click.option("--text_column", required=False, type=click.STRING, default="text", help="Text column to use")
@click.option("--lr", required=False, type=click.FLOAT, default=3e-5, help="Learning rate to use")
@click.option("--epochs", required=False, type=click.INT, default=1, help="Number of training epochs to use")
@click.option("--use_peft", required=False, type=click.Choice(["lora", "adalora", "ia3", "llama_adapter"]), help="PEFT method to use")
@click.option("--wandb_key", required=False, type=click.STRING, help="Use experiment tracking")
@click.option("--mixed_precision", required=False, type=click.Choice(["fp16", "bf16"], case_sensitive=True), help="Mixed precision type to use")
@click.option("--quantization", required=False, type=click.Choice(['nf4', 'fp4', 'int8'], case_sensitive=True), help="Quantization type to use")
@click.option("--disable_gradient_checkpointing", required=False, is_flag=True, help="Disable gradient checkpointing")
@click.option("--use_flash_attention_2", required=False, is_flag=True, help="Use flash attention 2")
@click.option("--lora_r", required=False, type=click.INT, default=16, help="Lora r to use")
@click.option("--lora_alpha", required=False, type=click.INT, default=32, help="Lora alpha to use")
@click.option("--lora_dropout", required=False, type=click.FLOAT, default=0.05, help="Lora dropout to use")
@click.option("--gradient_accumulation_steps", required=False, type=click.INT, default=1, help="Gradient accumulation steps to use")
@click.option("--batch_size", required=False, type=click.INT, default=2, help="Training batch size to use")
@click.option("--block_size", required=False, type=click.STRING, default="-1", help="Block size to use")
@click.option("--project_name", required=False, type=click.STRING, help="Project name in HF hub")
@click.option("--model_max_length", required=False, type=click.INT, default=1024, help="Model max length to use")
@click.option("--repo_id", required=False, type=click.STRING, help="Repo id for hugging face hub. Format is username/repo_name")
@click.option("--hf_token", required=False, type=click.STRING, help="Hugingface token to use")
@click.option("--torch_dtype", required=False, type=click.Choice(["auto", "bfloat16", "float16", "float32"]), help="Load the model under this dtype")
@click.option("--username", required=False, type=click.STRING, help="Huggingface username to use")
...
def create(**finetune_kwargs):
    ...
    data = {
        ...
        "autotrain_params": {
            ...
            "disable_gradient_checkpointing": finetune_kwargs["disable_gradient_checkpointing"],
    ...
    console = Console()
    with console.status("[bold green]Launching fine-tuning job...") as status:

        resp = send_request("POST", "/finetune/create", data=data)

    if resp.status_code != 500:
        resp_data = resp.json()
        if resp_data["warning"] is not None:
    ...
    if resp.status_code == 200:
        ...
        click.echo(click.style(f"Bad request: {resp_data['message']}", fg="red"), err=True)
        return
    elif resp.status_code == 500:
        click.echo(click.style(f"\nSomething went wrong: {resp_data}. Please try launch job later", fg="red"), err=True)
        return
    else:
...
def list(all):
    ...
    with console.status("[bold green]Fetching fine-tuning jobs...") as status:
        ...
        for job in resp_data:
            if all or (not all and (job['status'] == 'RUNNING' or job['status'] == 'QUEUED')):
