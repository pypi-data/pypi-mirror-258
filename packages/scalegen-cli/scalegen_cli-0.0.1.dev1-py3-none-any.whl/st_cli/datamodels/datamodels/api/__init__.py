...
class InferenceDeploymentInitialWorkersConfig(BaseModel):
    min_workers: int = 0
    initial_workers_gpu: Optional[Union[str, None]] = None
    initial_workers_gpu_num: Optional[Union[int, None]] = None
...
class InferenceDeploymentAutoscalingConfig(BaseModel):
    ...
    upper_allowed_latency_sec: Optional[Union[float, int]] = 4
    lower_allowed_latency_sec: Optional[Union[float, int]] = 1
    scaling_timeout_min: Optional[int] = 20
    scale_to_zero_timeout_min: Optional[int] = 30


class InferenceDeploymentIn(BaseModel):
    name: str
    model: str
    inf_type: Optional[Union[None, str]] = None
    hf_token: Optional[Union[None, str]] = None
    ...
    autoscaling_config: Optional[InferenceDeploymentAutoscalingConfig] = InferenceDeploymentAutoscalingConfig()
...
class InferenceDeploymentUpdateIn(BaseModel):
    ...
    initial_worker_config: InferenceDeploymentInitialWorkersConfig

    # Auto scaling parameters
    autoscaling_config: InferenceDeploymentAutoscalingConfig
...
class AutotrainParams(BaseModel):
    ...
    model_max_length: Optional[int] = Field(2048, title="Model input max context length")
    block_size: str = Field("-1", title="Block size")
    torch_dtype: Optional[Literal["auto", "bfloat16", "float16", "float32"]] = Field(None, title="Load the model under this dtype")
...
class OpenaiFinetuningIn(BaseModel):
    ...
    hyperparameters: Optional[OpenaiFinetuningHyperparameters] = OpenaiFinetuningHyperparameters()
    suffix: Optional[str] = ''  # A string of up to 18 characters that will be added to your fine-tuned model name.
    validation_file: Optional[Union[str, None]] = None  # The ID of an uploaded file that contains validation data.


class OpenaiFinetuningJobError(BaseModel):
    code: str  # A machine-readable error code.
    message: str  # A human-readable error message.
    param: Union[str, None] = None  # The parameter that was invalid, usually training_file or validation_file. This
...
class OpenaiFinetuningJob(BaseModel):
    ...
    error: Optional[Union[OpenaiFinetuningJobError, None]] = None  # For fine-tuning jobs that have failed, this will
    # contain more information on the cause of the failure.

    fine_tuned_model: Optional[Union[str, None]] = None  # The name of the fine-tuned model that is being created.
    # The value will be null if the fine-tuning job is still running.
    finished_at: Optional[Union[int, None]] = None  # The Unix timestamp (in seconds) for when the fine-tuning job
    ...
    object: str = 'fine_tuning.job'  # The object type, which is always "fine_tuning.job".
    organization_id: str = 'scaletorch'  # The organization that owns the fine-tuning job.

    result_files: List[str] = []  # The compiled results file ID(s) for the fine-tuning job. You can retrieve the
    ...
    trained_tokens: Optional[Union[int, None]] = None  # The total number of billable tokens processed by this
    ...
    validation_file: Optional[Union[str, None]] = None  # The file ID used for validation. You can retrieve the
...
class OpenaiFile(BaseModel):
    ...
    purpose: Literal["fine-tune", "fine-tune-results", "assistants", "assistants_output"]  # The intended purpose of
...
class OpenaiFinishJobIn(BaseModel):
    result_model: Optional[Union[str, None]] = None  # An openai file with resulting lora weights after finetuning
