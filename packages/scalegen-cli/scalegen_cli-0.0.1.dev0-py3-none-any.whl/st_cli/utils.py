...
def read_config_from_yaml(config_path) -> dict:
    ...
    if not os.path.exists(config_path):
        click.echo(click.style(f"{config_path} not found. Please specify config file path", fg='red'), err=True)
        exit()
...
def zip_and_upload(working_directory: str, exclude_dirs: tuple = ()):
    ...
    assert os.path.exists(working_directory) == True, "codeDir specified in config does not exist"
    ...
    all_files = list(filter(lambda x: not os.path.isdir(x), all_files)) # Excluding dir names

    if len(all_files) > MAX_FILES:
        click.echo(click.style("Folder contains more than 2000 files.", fg='red'), err=True, )
        return

    # Check folder size
    folder_size = sum([os.path.getsize(file) for file in all_files]) / (1024 ** 2) # Size in MB
    if folder_size > MAX_SIZE_MB:
        click.echo(click.style(f"Folder size is more than {MAX_SIZE_MB} MB.", fg='red'), err=True)
        return
    ...
    try:
        resp = requests.post(resp_data["data"]["uploadUrl"], headers=headers, files={"archive": (filename, zip_buffer)})
        if resp.status_code == 200:
            spinner_error = False
    except:
        click.echo(click.style(f"Could not upload files", fg='red'), err=True)
    ...
    return CodeCopy(type=CodeCopyType.B2, repo=filename, codeDir=working_directory).model_dump(mode="json")
...
def display_event_logs(job_id: str):
    ...
    try:
        ...
        click.echo(click.style(f"\nTo stop the workload please run `{'scaletorch' if PRODUCT_TYPE=='SCALETORCH' else 'scalegen finetune'} stop {job_id}`", fg='yellow'))
    
    except Exception:
        click.echo(click.style(f"\nFailed to retrieve the latest status. Please run `{'scaletorch' if PRODUCT_TYPE=='SCALETORCH' else 'scalegen finetune'} view {job_id}` to know the status fo the workload", fg='yellow'))
    ...
    # Wait for trial to be running
        ...
        while counter < 10:
            ...
            if trial_resp.status_code == 200:
                non_dapp_trials = list(filter(lambda x: not x["is_dapp_trial"], trial_data))
        ...
        if non_dapp_trials and non_dapp_trials[0]["status"] == "RUNNING":
            ...
            logs_url = f"https://proxy-prod.storch-api.com/logs/{job_id}/{non_dapp_trials[0]['trial_id']}/get-last-lines?num=100&mtime={{}}"
            counter = 0
            ...
            with Live(get_table(), refresh_per_second=4) as live:  # update 4 times a second to feel fluid
                while True:
                    log_resp = requests.get(logs_url.format(round(time.time() * 1000)))
                    if log_resp.status_code == 200:
                        logs = log_resp.json()["content"].split("\n")
...
def is_repo_private():
    ...
    repo_url = run_command("""git config --get remote.origin.url | sed -e 's/:/\//g'| sed -e 's/ssh\/\/\///g'| sed -e 's/git@/https:\/\//g' """)
    try:
        print(repo_url)
