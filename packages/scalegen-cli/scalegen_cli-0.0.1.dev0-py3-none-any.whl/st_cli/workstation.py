...
def create(gpus, count, name, num_workstations, config, quiet, template):
    ...
    if template:
        ...
        if not config_dict:
            click.echo(click.style(f"The template provided: {template} does not exist or is not a Workstation Template", fg='red'), err=True)
            return
        ...
        if not (all([gpus, count, name, num_workstations]) or config):
            click.echo(click.style(f"Should provide (gpus, count, name, num_workstations) or a config file", fg='red'), err=True)
            exit()
        ...
        if config:
            ...
            if config.requirements:
                reqs_path = config.requirements if not config.requirements.startswith("/") else os.path.join(os.getcwd(), config.requirements)
    ...
    if resp.status_code == 201:
        ...
        content = resp.json() if resp.status_code != 500 else resp.content.decode('utf-8')
        click.echo(click.style(f"Could not create workstation: {content}", fg="red"), err=True)
...
def delete(ws_id):
    ...
    if resp.status_code == 200:
        ...
        click.echo(click.style(f"Could not start deletion of workstation", fg="red"), err=True)
    ...
    if resp.status_code == 200:
        ...
        for ws_data in resp.json():
            if all or (not all and (ws_data['status'] in {'RUNNING', 'QUEUED'})):
                table.add_row(ws_data['id'], ws_data['status'], f"$ {round(float(ws_data['cost']), 3)}")
            
            if ws_data["status"] in {"DELETED", "FAILED"} and os.path.exists(os.path.join(SSH_KEY_PATH, ws_data["id"])):
...
def view(ws_id):
    ...
    if resp.status_code != 200:
        click.echo(click.style(f"Could not find workstation: {ws_id}", fg="red"), err=True)
        return
    
    ws_data = resp.json()
    node = None
    if ws_data['nodes']:
        running_vms = list(filter(lambda node: node['status'] == 'RUNNING' and node['role'] == 'WORKSTATION', ws_data['nodes']))
        if running_vms:
    ...
    if node and not os.path.exists(os.path.join(SSH_KEY_PATH, ws_id)):
        resp = send_request("GET", f"/secret/INTERNAL_SSH_PRIVATE_KEY_{node['ssh_key_id']}")
        if resp.status_code == 200:
            with open(os.path.join(SSH_KEY_PATH, ws_id), 'w') as fp:
                fp.write(resp.json()['secret'][f'INTERNAL_SSH_PRIVATE_KEY_{node["ssh_key_id"]}'])
    
    if ws_data["status"] in {"DELETED", "FAILED"} and os.path.exists(os.path.join(SSH_KEY_PATH, ws_data["id"])):
        os.remove(os.path.join(SSH_KEY_PATH, ws_data["id"]))

    console = Console()
    markdown_content = f"[bold][orange_red1]Id[/orange_red1] : [cyan]{ws_data['id']}[/cyan]\n"
    viz_link = 'N/A' if ('viz_page' in ws_data and ws_data['viz_page'] == '') else ws_data['viz_page'] 
    markdown_content += f"[orange_red1]Visualisation[/orange_red1] : [yellow]N/A[/yellow]\n"
    markdown_content += f"[orange_red1]Status[/orange_red1] : [yellow]{ws_data['status']}[/yellow]\n"
...
def restart(ws_id):
    ...
    if resp.status_code == 200:
        ...
        click.echo(click.style(f"Could not restart workstation: {resp.content.decode('utf-8')}", fg="red"), err=True)
...
def stop(ws_id):
    ...
    if resp.status_code == 200:
        ...
        click.echo(click.style(f"Could not stop workstation: {resp.content.decode('utf-8')}", fg="red"), err=True)
