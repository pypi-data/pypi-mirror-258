...
def start(as_standalone: bool = False) -> None:
    if A.U.for_service(SD, as_standalone=as_standalone):
            ...
            if sc == SC.send_event:
                if subscribtion_result.result: # type: ignore
                    ...
                    if event == A.CT_E.NEW_FILE_DETECTED:
                        ...
                        if A.PTH.get_extension(file_path) in (A.CT_F_E.JPEG, A.CT_F_E.JPG):
                            ...
                            if nn(polibase_document):
                                ...
                                if document_name in [
                                    ...
                                    A.L.polibase_document(
                                        js(("Отправленный документ Полибейс:", file_path))
                                    )
                                else:
        ...
        def service_starts_handler() -> None:
            ...
            A.A_PTH.listen(A.PTH.MEDICAL_DATA.VALUE)
            if not A.C_R.accessibility_by_smb_port(A.PTH.MEDICAL_DATA.HOST):
                A.L.it_bot(
                    j(
                        (
                            "При попытке прослушать папку: ",
                            A.PTH.MEDICAL_DATA.VALUE,
                            ", определено, что компьютер ",
                            A.PTH.MEDICAL_DATA.HOST,
                            " не доступен.",
                        )
                    ),
                    A.CT_L_ME_F.ERROR,
