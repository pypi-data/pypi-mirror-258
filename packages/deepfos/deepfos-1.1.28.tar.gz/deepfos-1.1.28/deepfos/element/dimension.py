...
class AsyncDimension(ElementBase[DimensionAPI]):
    ...
    def _maybe_rename_fields(
        ...
        for old, new in chain(
            V10_TO_V12.items(),
            [(DFLT_PNAME_COLUMN, 'parentName'), (SHAREDMEMBER, 'sharedMember')]
        ):
            if old in mbr:
                mbr[new] = mbr.pop(old)
    ...
    async def load_dataframe(
        ...
        **langugage_keys: str
        ...
        valid_columns = list(
            (
                set(mbrcls.__fields__) |
                # include them since they are compatible between versions
    ...
    async def _save_replace(
        ...
        if reorder:
            ...
            for rec in member_list:
                rec = self._maybe_rename_fields(rec)
