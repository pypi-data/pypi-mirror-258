...
def __handle_csv(
    ...
    *,
    ...
    if as_lists:
        ...
        write_csvfile_rows(
            filepath, data=data, mode=mode, include_keys=include_keys, header_row=header_row
        )
    else:
...
def write_csvfile_rows(
    ...
    *,
    ...
    """Write the given data dictionary to a CSV file. If optional <<include_keys>> is True,
    the first item of each row will be dictionary key that maps to that row in <<data>>.
    """
    data_rows = []
