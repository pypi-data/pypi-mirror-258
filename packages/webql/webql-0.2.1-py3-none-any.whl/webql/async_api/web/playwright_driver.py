...
class PlaywrightWebDriver(WebDriver[Locator]):
    ...
    async def scroll_to_bottom(self):
        ...
        await self._current_page.evaluate(scroll_js)
    ...
    async def _get_frame_accessibility_tree(
        ...
        content_frame_document_serialized = await frame_context.evaluate(
    ...
    async def _add_page_event_listeners(self):
        ...
        dom_listener_js = """
            (() => {
                window.myPageObserver = new MutationObserver(mutations => {
                    const now = Date.now();
                    window.localStorage.setItem('lastDomChange', now);
                });
                window.myPageObserver.observe(document.body, { childList: true, subtree: true });
            })();
        """

        try:
            await self._current_page.evaluate(dom_listener_js)
        # If the page is navigating, the evaluate function will raise an error. In this case, we wait for the page to load.
        except Error:
            start_time = time.time()
            while True:
    ...
    async def _remove_page_event_listeners(self):
        ...
        await self._current_page.evaluate(remove_dom_listener_js)
