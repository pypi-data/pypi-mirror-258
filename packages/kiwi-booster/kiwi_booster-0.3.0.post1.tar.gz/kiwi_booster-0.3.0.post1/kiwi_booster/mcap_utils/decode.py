...
from mcap_ros2._dynamic import CompressedImage, Image, NavSatFix, Odometry, TFMessage
...
def decode_compressed_image(msg: CompressedImage) -> Tuple[np.ndarray, dict]:
    """
    ...
    log_time = datetime.fromtimestamp(msg.ros_msg.header.stamp.sec) + timedelta(
        microseconds=msg.ros_msg.header.stamp.nanosec / 1000
    )
...
def decode_image(msg: Image) -> Tuple[np.ndarray, dict]:
    """
...
def decode_navsatfix(msg: NavSatFix) -> dict:
    """
...
def decode_tfmessage(msg: TFMessage) -> List[dict]:
    """
...
def decode_odometry(msg: Odometry) -> dict:
    """
    ...
    """
    return {
        "timestamp": datetime.fromtimestamp(msg.ros_msg.header.stamp.sec)
        + timedelta(microseconds=msg.ros_msg.header.stamp.nanosec / 1000),
        "frame_id": msg.header.frame_id,
