...
class nf_jira:
    ...
    def get_nf_issue_context(self, issue_id):
        try:
            ...
            # issue_data = self._rename_fields(issue_data)
            ...
            # fornecedor = self._get_nf_domain('fornecedor', '32504706000187') ########### TESTE ########### TESTE ########### TESTE ###########
    ...
    def _get_nf_issue_attachment(self, attachment_id):
        try:
            ...
            # attachment_data = self._rename_fields(attachment_data)
    ...
    def _get_nf_domain(self, type, cnpj):
        ...
        if type == 'fornecedor':
            domain_request = requests.get(
                f"{self._api_domain_url}/fornecedores?cnpj={cnpj}"
            )
            domain_request.raise_for_status()
            domain_data = domain_request.json()
        if type == 'centro':
            domain_request = requests.get(
                f"{self._api_domain_url}/centros?cnpj={cnpj}"
            )
            domain_request.raise_for_status()
            domain_data = domain_request.json()
