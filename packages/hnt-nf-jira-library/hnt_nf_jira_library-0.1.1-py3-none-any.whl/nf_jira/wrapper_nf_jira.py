...
class wrapper_jira:
    ...
    def _issue_factory(self, issue):
        ...
        if not issue['allocation_data']:
            ...
            for centro in issue['allocation_data']['centro_custos']:

                validTotalPercents+=issue['json_data']['totalInvoiceAmount']

                item = {
                    'centro' : centro['nome'].split("210")[0],
                    'centro_custo' : centro['nome'],
                    'cod_imposto' : 'C6',
                    'valor_bruto' : issue['json_data']['totalInvoiceAmount'] * centro['porcentagem'] / 100
                }
    ...
    def get_nf_issue_context(self, issue_id):
        try:
            ...
            ##### GET ALLOCATION #####
            allocation = self._get_allocation(attachment['supplierCnpj'], attachment['clientCnpj'], attachment['contractNumber'])
