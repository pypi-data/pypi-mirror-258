...
def process_config(config, config_data):
    ...
    if "taskio" in config_data:
        process_taskio_config_section(config, config_data['taskio'])
...
def process_taskio_config_section(config, taskio_config):
    """ Processes the taskio section from the configuration data dict.

    :param config: The config reference of the object that will hold the
    configuration data from the config_data.
    :param taskio_config: Taskio configuration section from a config data
    dict.
    """
    config.taskio['firenado'] = taskio_config
