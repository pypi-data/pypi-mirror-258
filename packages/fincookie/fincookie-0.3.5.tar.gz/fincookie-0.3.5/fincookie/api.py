...
def get_cookies(appid: str,
    ...
    api = dev_api if MODE == 'DEVELOPER' else cloud_cloud_api if ip[:5] == '10.55' else local_local_api
...
def get_loads() -> Dict:
    ...
    for api in apis:
        api = api + "get_loads"
        res = requests.get(api, timeout=2)
...
def get_dynamic_proxy():
    ip = socket.gethostbyname(socket.gethostname())
    api = cloud_cloud_api if ip[:5] == '10.55' else cloud_local_api
