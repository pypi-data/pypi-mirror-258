...
from switcore.action.schemas import SwitRequest, BaseState, SwitResponse, UserActionType, QueryResource, TaskResource, \
    MessageResource, SettingsResource
...
class AsyncActivityHandlerABC(BaseHandler, ABC):
    async def on_turn(self, swit_request: SwitRequest, state: BaseState, *args, **kwargs) -> SwitResponse:
        if swit_request.user_action.type == UserActionType.view_actions_drop:
            ...
            if not isinstance(swit_request.user_action.resource, SettingsResource):
                raise ValidationError("Must contain SettingsResource", SettingsResource)
            ...
            if not isinstance(swit_request.user_action.resource, QueryResource):
                raise ValidationError("Must contain QueryResource", QueryResource)
            ...
            if not isinstance(swit_request.user_action.resource, TaskResource):
                raise ValidationError("Must contain TaskResource", TaskResource)
            ...
            if not isinstance(swit_request.user_action.resource, MessageResource):
                raise ValidationError("Must contain MessageResource", MessageResource)
