...
class ModelArtifact(ConnectionMixin):
    ...
    def _deploy_model_artifact(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            tar_file_base_name = Path(tmp) / 'model'
            tar_file_path = f'{str(tar_file_base_name)}.tar.gz'

            _file_path = shutil.make_archive(
                base_name=str(tar_file_base_name),
                format='tar',
                root_dir=model_dir,
                base_dir='.',
            )
            os.rename(src=_file_path, dst=tar_file_path)
            ...
            # Upload file
            file = File(path=tar_file_path).upload()
