...
class GenScope(GenScopeBase):
    ...
    def gen_computed(self, type: mm.Type|None):
        return self._gen_arbitrary_computed if type == None or type == mm.Builtins.Any else self._gen_computed.get(type)
    ...
    def gen_agg(self, type: mm.Type|None):
        return self._gen_arbitrary_agg if type == None or type == mm.Builtins.Any else self._gen_agg.get(type)
    ...
    def list_vars(self, type: mm.Type|None):
        ...
        return self.vars if type == None or type == mm.Builtins.Any else self.by_type.get(type, [])
    ...
    def sample_entity(self):
        ...
        vars = self.entities
        if not vars:
            raise ValueError(f"Cannot sample entity vars. No variables of given type in scope.")
    ...
    def gen_value(self, type: mm.Type|None):
        strats = []
        if type in SUPPORTS_CONST_GEN or type == None:
