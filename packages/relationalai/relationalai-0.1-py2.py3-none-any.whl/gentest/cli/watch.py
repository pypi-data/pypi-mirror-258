...
from hupper.reloader import ControlSignal
...
from gentest.cli.repro import Shareable, encode_shareable, export_ir, export_pyrel, get_example, get_fn_file, reproduce, encode, reproduce_blob
...
class State:
    ...
    def get_target(self) -> str:
        if not self.repro_target:
            raise Exception(f"No target failure for operation!")
...
def build_cmd_tree(state: State):
    ...
    def clear_action():
        was_targeted = state.repro_target != None
