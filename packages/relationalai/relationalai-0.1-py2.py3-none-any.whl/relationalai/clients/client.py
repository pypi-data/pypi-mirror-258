...
class Client():
    ...
    def query(self, task:m.Task, rentrant=False, readonly=False, raw_results=False) -> DataFrame|Any:
    ...
    def _install_batch_flush(self):
        if not self.dry_run:
            start = time.perf_counter()
            errors = self.resources.create_models(self._database, self.get_engine(), self._install_batch.flush())
            self.report_errors(errors)
            debugging.time(f"install_batch", time.perf_counter() - start)
