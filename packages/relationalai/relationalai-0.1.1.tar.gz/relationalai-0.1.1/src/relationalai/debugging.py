...
def time(type:str, elapsed:float, results:DataFrame = DataFrame()):
    if DEBUG:
        logger.debug({"event":"time", "type": type, "elapsed": elapsed, "results": {"values": results.head(20).to_json(orient="records"), "count": len(results)}})
        # print(f"{type} | {elapsed:.4f} | {len(results)}")
