...
python -m pip install git+https://git@github.com/RelationalAI/relationalai-python.git

# Alternatively, if you prefer SSH:
python -m pip install git+ssh://git@github.com/RelationalAI/relationalai-python.git
```

> [!NOTE]
> If you encounter authentication errors while installing,
> you may need to create a personal access token or authorize your SSH token for the RelationalAI GitHub organization.
> See [About authentication with SAML single sign-on](https://docs.github.com/en/enterprise-cloud@latest/authentication/authenticating-with-saml-single-sign-on/about-authentication-with-saml-single-sign-on)
...
and configure your project's resources.

> [!CAUTION]
> If you have the [legacy RelationalAI CLI](https://github.com/RelationalAI/rai-cli/) installed
> and encounter an error that `init` is not a valid command, you may need to adjust your PATH
> so that the folder containing the legacy CLI binary comes _after_ your project's folder.
> You can also rename your legacy RAI binary, typically located at `~/usr/local/bin/rai.`
> If you do not need the legacy CLI, the best solution is to delete it.
>
> You may also encounter issues with workflows using the legacy CLI
...
```sh
python -m pip install --force-reinstall git+https://git@github.com/RelationalAI/relationalai-python.git
```
...
Here, `person.friend.name != "Bob"` says that `person` is friends with someone whose name isn't Bob.
