    ...
    def query(self, task: m.Task):
        ...
        for item in task.items:
            ...
            if ent == m.Builtins.Return:
                ...
                for col in cols:
                    has_var = any([isinstance(v, m.Var) and v.value is None for v in col.value])
                    if has_var:
                        requires_union = True
                        break
                if not requires_union:
                    ...
                    for i in range(len(cols[0].value)):
                        row = [self.emit_var(col.value[i], sources) for col in cols]
                ...
                if ent.name in sql_filter:
                    ...
                    vs = " || ".join([self.emit_var(prop, sources) for prop in props[0].value])
                    sources[props[-1]] = f"encode(sha1({vs}), 'base64')"
                elif ent.name == "range":
                    froms.append(f"(VALUES {','.join([f'({i})' for i in range(int(props[0].value), int(props[1].value), int(props[2].value))])}) AS range{item.id}")
