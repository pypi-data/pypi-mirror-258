...
def get_test_hash(test_fn: Callable|HypothesisTestFn):
    ...
    raise Exception(f"Must pass a valid hypothesis test fn to get its key. Did you mean to pass the inner fn to `get_test_inner_key()`?")
...
def expand_short_key(short_key: str, db_path: str):
    if not os.path.isdir(db_path):
        raise Exception(f"No hypothesis database detected.")
    ...
    for cur_key in keys:
        if cur_key.startswith(short_key):
            if key != None:
                raise Exception(f"Hash collision! Specify a longer segment of the key hash.")
            key = cur_key
...
def expand_short_value(key: str, short_value: str, db_path: str):
    ...
    for cur_value in values:
        if cur_value.startswith(short_value):
            if value != None:
                raise Exception(f"Hash collision! Specify a longer segment of the value hash.")
            value = cur_value
