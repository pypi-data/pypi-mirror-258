...
from railib.pb.schema_pb2 import PrimitiveType
import requests
...
import os
import textwrap
from typing import Any, Dict, Iterable, List, Tuple, List, cast
import base64
...
class Resources(ResourceProvider):
    ...
    def create_engine(self, name:str, size:str, compute_pool:str):
        return self._exec(f"call {APP_NAME}.api.create_engine('{name}', '{compute_pool}', '{size}');")

    def delete_engine(self, name:str):
        return self._exec(f"call {APP_NAME}.api.delete_engine('{name}');")
    ...
    def create_graph(self, name: str):
        return self._exec(f"call {APP_NAME}.api.create_database('{name}');")

    def delete_graph(self, name:str):
        return self._exec(f"call {APP_NAME}.api.delete_database('{name}');")
    ...
    def delete_model(self, database:str, engine:str, name:str):
        return self.exec_raw(database, engine, f"def delete:rel:catalog:model[\"{name}\"] = rel:catalog:model[\"{name}\"]", readonly=False)
    ...
    def _exec_async_raw(self, database:str, engine:str, raw_code:str, readonly=True):
        ...
        for ix, rel in enumerate(cast(Any, meta).relations):

            arrow_file = rel.file_name
    ...
    def list_warehouses(self):
        results = self._exec(f"SHOW WAREHOUSES")
        if not results:
            return []
        return [{"name":name}
    ...
    def list_compute_pools(self):
        results = self._exec(f"SHOW COMPUTE POOLS")
        if not results:
            return []
        return [{"name":name}
                for (name, *rest) in results.fetchall()]
    ...
    def list_roles(self):
        results = self._exec(f"SHOW ROLES")
        if not results:
            return []
        return [{"name":name}
                for (name, *rest) in results.fetchall()]
    ...
    def list_apps(self):
        results = self._exec(f"SHOW APPLICATIONS")
        if not results:
            return []
        return [{"name":name}
    ...
    def list_databases(self):
        results = self._exec(f"SHOW DATABASES")
        if not results:
            return []
        return [{"name":name}
                for (time, name, *rest) in results.fetchall()]
