...
def build_kmers(sequence: str, ksize: int, seq_length: int):
    ...
    # convert to list and then np array

    return np.array(list(kmers))
...
def find_shared_kmers(queryKmers: dict, records: list, ksize: int):
    ...
    for record in records:
        subj_kmers_array = build_kmers(record.seq, ksize, record.seq_length)

        # find common kmers
        kmer_info = ""
        for query_id, query_kmer_array in queryKmers.items():
            shared_kmers = np.intersect1d(query_kmer_array, subj_kmers_array)
