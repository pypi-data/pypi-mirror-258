...
class DiscordPyInstrumentor(BaseInstrumentor):
    ...
    def _uninstrument(self, **kwargs):
        ...
        otel_unwrap(commands.Bot, "Bot.process_commands")
    ...
    def _wrapper_process_commands(self, original, instance, args, kwargs):
        ...
        # Assuming `args[0]` is the message object for process_commands
        command_name = getattr(args[0], "content", None)
        with tracer.start_as_current_span(
            "Bot.process_commands", attributes={"command.name": command_name}
        ):
    ...
    def _wrapper_send_message(self, original, instance, args, kwargs):
        ...
        with tracer.start_as_current_span("TextChannel.send") as span:
            try:
                ...
                span.set_status(Status(StatusCode.ERROR, "Error sending message"))
                raise  # Re-raise the exception after recording it
