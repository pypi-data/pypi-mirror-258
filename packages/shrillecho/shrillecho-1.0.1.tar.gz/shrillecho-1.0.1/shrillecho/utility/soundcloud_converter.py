...
class SoundCloudConverter:
    ...
    def __init__(self, sp: Spotify, sc: SoundCloud):
        self.__sp = sp
        self.__sc = sc
    ...
    async def _resolve_soundcloud_tracks(self, sc_link: str) -> ResolvePlaylist:
        ...
        sc_tracks: List[SoundCloudTrack] = await self.__get_soundcloud_tracks(
    ...
    async def async_search(self, queries: List[str], search_type: str) -> List[TrackSearch]:
        ...
        async with httpx.AsyncClient() as client:
            responses = await asyncio.gather(
                *(client.get(f"{self.__spotify_base_url}/search", headers=self.__spotify_headers, params=param) for
