...
def format_divs(md, target, div_specs=None, skip_div_tags=False):
    ...
    while line_no < len(lines):
            ...
            if "title" in comment:
                ...
                if target == "tex":
                    if specs.get("caption", False):
                        div_title = f"\\caption{{{div_title}}}"
                    else:
                        div_title = f"[{div_title}]"
            ...
            if div_type == "algorithm" and target == "html":
                ...
                if div_title != None:
                    if target == "tex":
                        ...
                        result.append(f"`{div_title}`{{=latex}}\n")
                    else:
        ...
        # end of a div
            ...
            div_end_stack.pop()
            if div_type_stack[-1] == "algorithm" and target == "html":
                result.append("\\end{algorithm}")
                result.append("</pre>")
                result.append("```")
                in_algorithm = False
            elif div_type_stack[-1] == "algorithmic" and target == "html":
