...
class TeXMarkdownSerializer(MarkdownSerializer):
    ...
    def md_to_tex(self, md):
        ...
        # divs to latex environment filter (pandoc-latex-environment)
        extra_args = ['--eol=lf', '--filter=pandoc-xnos', '--filter=pandoc-latex-environment']
        # generate standalone HTML files (with HTML header and footer)
        if self._standalone:
            extra_args.append('--standalone')
        ...
        # configure div to LaTeX 
        try:
            tmpfile = tempfile.NamedTemporaryFile(delete=False, mode="w", encoding=default_encoding)
            content = {"pandoc-latex-environment": div_latex_environments}
            yaml.dump(content, tmpfile, allow_unicode=True)
            extra_args.append("--metadata-file={}".format(tmpfile.name))
        except:
            logger.error("Could not create temporary file with pandoc-latex-environment params")
            tmpfile = None
